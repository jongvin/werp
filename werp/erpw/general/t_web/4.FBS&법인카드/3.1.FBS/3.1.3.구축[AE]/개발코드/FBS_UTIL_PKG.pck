CREATE OR REPLACE PACKAGE FBS_UTIL_PKG IS
    ...
    -- 메일발송에서 사용되는 상수
    SMTP_SERVER         VARCHAR2(50) := '52.2.202.41';  -- 임시로 개발서버에 SMTP를 설치한것임. 추후 수정요망.
    SMTP_PORT           NUMBER       := 10;
    ...
    SEND_EDI_COMMAND         VARCHAR2(300) := '/CJCERP/FBS/FB/tcpsend';         -- BATCH로 송신시 처리하는 프로그램 설정
    RECV_EDI_COMMAND         VARCHAR2(300) := '/CJCERP/FBS/FB/tcprecv';         -- BATCH로 수신시 처리하는 프로그램 설정
    /*************************************************************************/
    /*  1. 모듈ID    : pw_encode                                             */
    /*  2. 모듈이름  : 문자열 암호화 모듈                                    */
    /*************************************************************************/
...
CREATE OR REPLACE PACKAGE BODY FBS_UTIL_PKG IS
    ...
    BEGIN
        ...
        ELSIF out_bank_cd = HANA_BANK_CD THEN
            SELECT RPAD( SUBSTR( TRUNC( ( SUBSTR( RTRIM( out_account_no ) , 1 , 12 )  +
                   ...
                   RTRIM( trade_amt ) + TO_CHAR(SYSDATE, 'YYMMDD' ) ) / TO_NUMBER( RTRIM( etc_field1 ) ) ) , -4, 4 ), 6, ' ' )
              INTO v_sign_no
              FROM DUAL;
        ...
        ELSIF out_bank_cd = KIUP_BANK_CD THEN
            -- STEP 1,2 단계 변형을 처리한다.			
            SELECT TO_CHAR( MOD(MOD(TO_NUMBER(SUBSTRB(out_account_no,1,1)) + TO_NUMBER(SUBSTRB(in_account_no,1,1)),10) + to_number(substrb(LPAD(TO_CHAR(trade_amt),6,'0'),1,1)),10)) ||
                   TO_CHAR( MOD(MOD(TO_NUMBER(SUBSTRB(out_account_no,2,1)) + TO_NUMBER(SUBSTRB(in_account_no,2,1)),10) + to_number(substrb(LPAD(TO_CHAR(trade_amt),6,'0'),2,1)),10)) ||
                   TO_CHAR( MOD(MOD(TO_NUMBER(SUBSTRB(out_account_no,3,1)) + TO_NUMBER(SUBSTRB(in_account_no,3,1)),10) + to_number(substrb(LPAD(TO_CHAR(trade_amt),6,'0'),3,1)),10)) ||
                   TO_CHAR( MOD(MOD(TO_NUMBER(SUBSTRB(out_account_no,4,1)) + TO_NUMBER(SUBSTRB(in_account_no,4,1)),10) + to_number(substrb(LPAD(TO_CHAR(trade_amt),6,'0'),4,1)),10)) ||
                   TO_CHAR( MOD(MOD(TO_NUMBER(SUBSTRB(out_account_no,5,1)) + TO_NUMBER(SUBSTRB(in_account_no,5,1)),10) + to_number(substrb(LPAD(TO_CHAR(trade_amt),6,'0'),5,1)),10)) ||
                   TO_CHAR( MOD(MOD(TO_NUMBER(SUBSTRB(out_account_no,6,1)) + TO_NUMBER(SUBSTRB(in_account_no,6,1)),10) + to_number(substrb(LPAD(TO_CHAR(trade_amt),6,'0'),6,1)),10))
              INTO v_sign_no
              FROM DUAL;
            -- STEP 3 단계 변형을 처리한다.			  
            SELECT SUBSTRB(v_sign_no ,4,3)||SUBSTRB(v_sign_no ,1,3)
              INTO v_sign_no
              FROM DUAL;			  
            -- STEP 4단계 변형
    ...
    /*        물리적인 실제 CLASS파일은 FBS폴더 하위의 javaclass？더에 있음  */
    /*                                                                       */
    ...
    BEGIN
        ...
        -- PIPE buffer를 지우고, PIPE HANDLE을 얻는다.
        DBMS_PIPE.RESET_BUFFER;
        v_pipe_name := DBMS_PIPE.UNIQUE_SESSION_NAME;
  
        DBMS_PIPE.PACK_MESSAGE('SYSTEM');
        DBMS_PIPE.PACK_MESSAGE(v_pipe_name);
        DBMS_PIPE.PACK_MESSAGE(p_string);
		
		fbs_util_pkg.write_log('FBS',p_string);
		fbs_util_pkg.write_log('FBS',v_pipe_name);
  
        -- 메시지를 send합니다.
        --v_status := DBMS_PIPE.SEND_MESSAGE('EFT_call_daemon_'||'CJCDB', DEFAULT_TIMEOUT);
		v_status := DBMS_PIPE.SEND_MESSAGE('EFT_call_daemon_'||get_db_name(), DEFAULT_TIMEOUT);

        -- 정상 혹은 오류처리여부를 status로 판단
        IF v_status <> 0 THEN
            write_log('EFT', 'ORA-20010 EFT데몬에게 메시지를 전달하던중 에러 발생! status = ' || v_status);
            RAISE_APPLICATION_ERROR(-20010,'EFT데몬에게 메시지를 전달하던중 에러 발생! status = ' || v_status);
        END IF;
  
        v_status := DBMS_PIPE.RECEIVE_MESSAGE(v_pipe_name, DEFAULT_TIMEOUT);

        IF v_status <> 0 THEN
            write_log('EFT', 'ORA-20011  EFT데몬으로부터 메시지 수신중 에러 발생! status = ' ||  v_status);
            RAISE_APPLICATION_ERROR(-20011, 'EFT데몬으로부터 메시지 수신중 에러 발생! status = ' ||  v_status);
        END IF;
  
        DBMS_PIPE.UNPACK_MESSAGE(v_result);
		fbs_util_pkg.write_log('FBS',v_result);
        RETURN( 'OK' );
  
    EXCEPTION
        WHEN OTHERS THEN
            write_log('EFT', 'ORA-20102  EAP_CALL_DAEMON procedure에서 Other Exception이 발생'||sqlerrm);
            RETURN('ERROR');
...
/*        -- String으로 받은 OS명령어를 수행시킵니다. JAVA Class를 활용
    ...
    BEGIN
        ...
        ELSIF v_gubun_code = 'CHK_CASH_PW' THEN
            IF v_record_exist_yn = 'Y' THEN
                ...
                ELSIF pw_decode(rec_lookup_values.attribute1,4 ,p_password2) <> p_password2 THEN
        ...
        ELSIF v_gubun_code = 'CHK_BILL_PW' THEN
            IF v_record_exist_yn = 'Y' THEN
                ...
                IF pw_decode( rec_lookup_values.lookup_value , 4 ,p_password1) <> SubStrb(p_password1,1,4) THEN
                     v_return_msg := '팀장암호가 틀렸습니다.\n확인 후, 다시 입력하여 주세요.';
                ELSIF pw_decode(rec_lookup_values.attribute2,4 ,p_password2 ) <> p_password2 THEN
    ...
    /*  9. 모듈의 목적 및 주요기능                                           */
    /*                                                                       */
    /*      - 주로 EFT 데몬을 parameter를 셋팅하여 call한다.                 */
    /*      - control  : SYSTEM => 명령어 실행                               */
    /*                   STOP   => 데몬 정지                                 */
    /*                   SQL    => SQL실행                                   */
    /*      - commandline : 실행프로그램과 parameter일체                     */
    /*                                                                       */
    /* 10. 최초작성자: LG CNS 신인철                                         */
    /* 11. 최초작성일: 2005년1월26일                                         */
    /* 12. 최종수정자:                                                       */
    /* 13. 최종수정일:                                                       */
    /*************************************************************************/
/*    PROCEDURE call_daemon( control     IN VARCHAR2,          -- 제어문자열  
                           commandline IN VARCHAR2,          -- 실행시킬 command  
                           timeout     IN NUMBER,            -- Session Timeout시간  
                           result      OUT VARCHAR2,         -- 결과코드
                           gubun       IN  VARCHAR2 DEFAULT 'S' ) IS   -- 구분 S:일반FBS   I:내부사용 (별도EXCEPTION발생안함) 
        v_pipe_name VARCHAR2(30);
        v_status    NUMBER;
        v_result    VARCHAR2(20);
    BEGIN
        ...
        DBMS_PIPE.PACK_MESSAGE(control);
        DBMS_PIPE.PACK_MESSAGE(v_pipe_name);
        DBMS_PIPE.PACK_MESSAGE(commandline);
  
        -- 메시지를 send합니다.
        v_status := DBMS_PIPE.SEND_MESSAGE('EFT_call_daemon_'||get_db_name, timeout);
        ...
        v_status := DBMS_PIPE.RECEIVE_MESSAGE(v_pipe_name, timeout);
    ...
    EXCEPTION
        WHEN OTHERS THEN
            write_log('EFT', 'ORA-20102  EAP_CALL_DAEMON procedure에서 Other Exception이 발생'||sqlerrm);
            IF gubun = 'S' THEN
                RAISE_APPLICATION_ERROR(-20102, 'EAP_CALL_DAEMON procedure에서 Other Exception이 발생'||sqlerrm);
            END IF;
    END call_daemon;    */
