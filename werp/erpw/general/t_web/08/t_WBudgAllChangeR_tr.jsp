		...
		if (CITCommon.isNull(strAct))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_BUDG_ALL_CHANGE_I(?,?,?,?,?)}";
						...
						lrArgData.setValue("ALL_CHG_SEQ", rows[i].getString(dsMAIN.indexOfColumn("ALL_CHG_SEQ")));
						lrArgData.setValue("CRTUSERNO", strUserNo);
						lrArgData.setValue("BUDG_APPLY_YM", rows[i].getString(dsMAIN.indexOfColumn("BUDG_APPLY_YM")));
					}
					else if (rows[i].getJobType() == GauceDataRow.TB_JOB_UPDATE)
					{
						strSql = "{call SP_T_BUDG_ALL_CHANGE_U(?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_BUDG_ALL_CHANGE_D(?,?,?)}";
