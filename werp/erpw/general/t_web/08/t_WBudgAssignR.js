...
function Initialize()
{
	G_addDataSet(dsMASTER, trans, gridMASTER, null, "부서목록",null,null,true);
...
function	confirmDept(asTag)
{
	...
	if(asTag == "T")
	{
		C_msgOk("확정이 정상적으로 수행되었습니다.");
	}
	else
	{
		C_msgOk("확정취소가 정상적으로 수행되었습니다.");
	}
...
function OnColumnChanged(dataset, row, colid)
{
	...
	else if(dataset == dsMAIN)
	{
		if(colid == "BUDG_ITEM_ASSIGN_AMT")
		{
			...
			if(lfAmt != lfSum)
			{
				var lsRet = C_msgYesNo("배정금액과 월별금액이 일치하지 않습니다.<br> 자동으로 분할하시겠습니까?");
				if(lsRet == "Y")
				{
					dvdMonth();
				}
