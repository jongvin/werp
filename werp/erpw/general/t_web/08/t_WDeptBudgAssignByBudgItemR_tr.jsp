...
/**************************************************************************/
	...
	try
	{
		...
		if(!CITCommon.isNull(strAct) && strAct.equals("ITEM_COST"))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					strSql = "{call SP_T_BUDG_COST_AMT_CAL2(?,?,?,?)}";
					...
					lrArgData.setValue("BUDG_CODE_NO", rows[i].getString(dsITEM_COST.indexOfColumn("BUDG_CODE_NO")));
		...
		else if(!CITCommon.isNull(strAct) && strAct.equals("BUDG_ITEM_ALL"))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					strSql = "{call SP_T_BUDG_DEPT_ITEM_H_ALL2(?,?,?,?,?)}";
					...
					lrArgData.setValue("BUDG_CODE_NO", rows[i].getString(dsBUDG_ITEM_ALL.indexOfColumn("BUDG_CODE_NO")));
					lrArgData.setValue("CRTUSERNO", strUserNo);
			...
			{
				if (GauceInfo == null) GauceInfo = new CITGauceInfo(request, response);
				GauceInfo.response.writeException("USER", "900001", "예산항목별 부서일괄 등록 에러" + ex.getMessage());
				throw new Exception(ex.getMessage());
			}
		}
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					if (rows[i].getJobType() == GauceDataRow.TB_JOB_INSERT)
					{
						...
						lrArgData.setValue("BUDG_CODE_NO", rows[i].getString(dsMASTER.indexOfColumn("BUDG_CODE_NO")));
						lrArgData.setValue("RESERVED_SEQ", rows[i].getString(dsMASTER.indexOfColumn("RESERVED_SEQ")));
						lrArgData.setValue("CRTUSERNO", strUserNo);
						lrArgData.setValue("TARGET_DEPT_CODE", rows[i].getString(dsMASTER.indexOfColumn("TARGET_DEPT_CODE")));
						lrArgData.setValue("BUDG_ADD_AMT", rows[i].getString(dsMASTER.indexOfColumn("BUDG_ADD_AMT")));
						lrArgData.setValue("BUDG_ITEM_REQ_AMT", rows[i].getString(dsMASTER.indexOfColumn("BUDG_ITEM_REQ_AMT")));
						lrArgData.setValue("BUDG_ITEM_ASSIGN_AMT", rows[i].getString(dsMASTER.indexOfColumn("BUDG_ITEM_ASSIGN_AMT")));
