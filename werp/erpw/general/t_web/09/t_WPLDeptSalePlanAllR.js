...
function Initialize()
{
	G_addDataSet(dsMAIN, trans, gridMAIN, null, "현장별경영계획계획",null,null,true);
...
{
	if(colid == "AMT_00") return;	//당초금액의 변동시는 계산 안함
	var			liLv = C_convSafeInt(dsMAIN.NameString(row,"LV"));
	if(liLv == 1 || liLv == 2)
	{
		var			liCol = C_convSafeInt(colid.substring(4,6));
	...
	{
		CalcSumOneCol(row,colid);			//원가가 바뀌는 경우는 이후 금액을 재계산 하지 않는다.
	}
}
