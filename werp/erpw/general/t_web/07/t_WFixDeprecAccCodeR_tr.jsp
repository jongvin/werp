		...
		if (CITCommon.isNull(strAct))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_FIX_DEPREC_ACC_CODE_I(?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_FIX_DEPREC_ACC_CODE_U(?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_FIX_DEPREC_ACC_CODE_D(?,?)}";
			...
			catch (Exception ex)
			{
				...
				GauceInfo.response.writeException("USER", "900001", "T_FIX_DEPREC_ACC_CODE 테이블 데이타 갱신 에러" + ex.getMessage());
