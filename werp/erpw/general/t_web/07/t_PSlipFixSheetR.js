...
function Initialize()
{
	...
	//G_addDataSet(dsMAIN2, trans, gridMAIN, sSelectPageName+D_P1("ACT","MAIN2")+ + D_P2("FIX_ASSET_SEQ",vFixAssetSeq), "전표발행대상");
	...
	G_addDataSet(dsAUTO_BILL_FIX_GET_SEQ, null, null, null, "취득_자동전표_작업번호");
	...
	if(vWORK_SLIP_ID=="") CurWorkSlip(); //수정 주석으로 막은 것 풀었음
...
function OnLoadBefore(dataset)
{
		...
		dataset.DataID = sSelectPageName	+ D_P1("ACT","MAIN")
											+ D_P2("COMP_CODE", txtCOMP_CODE.value)
										       + D_P2("ASSET_CLS_CODE",dsASSETCLS.NameString(dsASSETCLS.RowPosition,"ASSET_CLS_CODE"))
									              ...
									              + D_P2("GET_YEAR", cboGET_YEAR.value)
	...
	else if(dataset == dsAUTO_BILL_FIX_GET_SEQ)
	{
		dataset.DataID = sSelectPageName	+ D_P1("ACT","AUTO_BILL_FIX_GET_SEQ");
	}
}
...
function calAmt()
{
	//vGET_AMT =  parseInt(vGET_AMT) + parseInt(dsMAIN.NameValue(dsMAIN.RowPosition, "GET_COST_AMT"));

	//alert( dsMAIN.NameString(dsMAIN.RowPosition, "GET_COST_AMT") );
	//txtVAT_AMT.value = dsMAIN.NameValue(dsMAIN.RowPosition, "GET_COST_AMT")*0.1;
...
function SetIfrmOtherAccPageAmt()
{
	if(dsMAIN.NameString(dsMAIN.RowPosition, "CHK_CLS") =='T')
	{
		...
		txtVAT_AMT.value = parseInt(dsMAIN.NameValue(dsMAIN.RowPosition, "GET_COST_AMT"))*0.1;	
	}
	else if(dsMAIN.NameString(dsMAIN.RowPosition, "CHK_CLS") =='F')
	{
		txtVAT_AMT.value = 0;	
	}
	ifrmOtherAccPage.dsSLIP_D.NameString(1,"CR_AMT") 	  =parseInt(dsMAIN.NameValue(dsMAIN.RowPosition, "GET_COST_AMT")) +  parseInt(txtVAT_AMT.value);
	ifrmOtherAccPage.dsSLIP_D.NameString(1,"CR_AMT_D") = C_toNumberFormatString(parseInt(dsMAIN.NameValue(dsMAIN.RowPosition, "GET_COST_AMT")) +  parseInt(txtVAT_AMT.value));
...
{
	//D_defaultAdd();
	//dsMAIN.NameString(dsMAIN.RowPosition,"MAKE_COMP_CODE") = txtCOMP_CODE.value;
...
function	btnSlipCreate_onClick()
{
	...
	G_Load(dsAUTO_BILL_FIX_GET_SEQ, null); 
	btnSLIP_MAKE_SLIPNO_onClick();
	
	trans.Parameters = "";
	trans.Parameters += "ACT=AUTO_BILL_FIX_GET";
	trans.Parameters += ",AUTO_BILL_FIX_GET_SEQ="+dsAUTO_BILL_FIX_GET_SEQ.NameString(1,"AUTO_BILL_FIX_GET_SEQ");
	...
	//trans.Parameters += ",VAT_AMT="+dsMAIN.NameValue(dsMAIN.RowPosition, "VAT_AMT");
