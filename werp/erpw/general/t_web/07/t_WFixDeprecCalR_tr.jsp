		...
		if(!CITCommon.isNull(strAct) && strAct.equals("REMOVE"))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					strSql = "{call SP_T_FIX_REM_DEPREC_SLIP(?)}";
		...
		if(CITCommon.isNull(strAct))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_FIX_DEPREC_CAL_I(?,?,?,?,?,?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_FIX_DEPREC_CAL_U(?,?,?,?,?,?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_FIX_DEPREC_CAL_D(?)}";
		...
		if(!CITCommon.isNull(strAct) && strAct.equals("APPLY"))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					strSql = "{call SP_T_APPLY_OR_CANCEL_FIX_DEP(?,?, ?, ?, ?,?)}";
