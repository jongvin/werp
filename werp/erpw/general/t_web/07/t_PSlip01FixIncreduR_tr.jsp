...
/**************************************************************************/
	...
	try
	{
		...
		strAUTO_B_F_INCSUB_SEQ = GauceInfo.request.getParameter("AUTO_BILL_FIX_INCSUB_SEQ");
		...
		strACC_REMAIN_POSITION	= GauceInfo.request.getParameter("ACC_REMAIN_POSITION");
		strVAT_CODE				= GauceInfo.request.getParameter("VAT_CODE");
		strVAT_DT				= GauceInfo.request.getParameter("VAT_DT");
		
		strACC_CODE				= GauceInfo.request.getParameter("ACC_CODE");
		strFIX_ASSET_SEQ			= GauceInfo.request.getParameter("FIX_ASSET_SEQ");
		
		strAct = GauceInfo.request.getParameter("ACT");
		
		if(strAct.equals("AUTO_BILL_FIX_INCSUB"))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_AUTO_BILL_FIX_INCSUB_I(?,?,?,?,?)}";
						
						lrArgData.addColumn("AUTO_B_F_INCSUB_SEQ", CITData.NUMBER);
						...
						lrArgData.addRow();
						lrArgData.setValue("AUTO_B_F_INCSUB_SEQ", strAUTO_B_F_INCSUB_SEQ);
					...
					{
						//strSql = "{call SP_T_AUTO_BILL_FIX_INCSUB_D(?,?)}";
						
						//lrArgData.addColumn("AUTO_B_F_INCSUB_SEQ", CITData.NUMBER);
						//lrArgData.addColumn("FIX_ASSET_SEQ", CITData.NUMBER);

						//lrArgData.addRow();
						//lrArgData.setValue("AUTO_B_F_INCSUB_SEQ", rows[i].getString(dsMAIN.indexOfColumn("AUTO_B_F_INCSUB_SEQ")));
						//lrArgData.setValue("FIX_ASSET_SEQ", rows[i].getString(dsMAIN.indexOfColumn("FIX_ASSET_SEQ")));
			...
			catch (Exception ex)
			{
				...
				GauceInfo.response.writeException("USER", "900001", "전표 발행 실패" + ex.getMessage());
		...
		if(strAct.equals("AUTO_BILL_FIX_INCSUB"))
		{
			...
			{
				CITData lrArgData = new CITData();
				strSql = "{call SP_T_AUTO_BILL_FIX_INCSUB_SLIP(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,?)}";
				...
				lrArgData.addColumn("AUTO_BILL_FIX_INCSUB_SEQ", CITData.NUMBER);
				...
				lrArgData.setValue("AUTO_BILL_FIX_INCSUB_SEQ", strAUTO_B_F_INCSUB_SEQ);
		...
		if(strAct.equals("AUTO_SLIP_ID_DEL"))
		{
			try
			{
				...
				lrArgData.addColumn("SLIP_ID", CITData.NUMBER);
				lrArgData.addColumn("ModUser", CITData.VARCHAR2); //
