...
function Initialize()
{
	...
	G_setReadOnlyCol(gridMAIN2,"MAKE_SLIPNO");
	gridMAIN.focus();
...
function	chkSave()
{
	if (G_isChangedDataset(dsMAIN2))
	{
		var ret = C_msgYesNo("변경사항을 저장한 후 작업이 가능합니다.<br>변경된 내용을 저장하시겠습니까?" , "저장");

		if (ret == "Y")
		{
			if(!G_saveAllData(dsMAIN2)) return false;
			return true;
		}
		else
		{
...
function	makeInSlip()
{
	...
	if(!C_isNull(dsMAIN2.NameString(dsMAIN2.RowPosition,"SLIP_ID")))
	{
	...
	lrObject.fix_asset_seq = dsMAIN2.NameString(dsMAIN2.RowPosition,"FIX_ASSET_SEQ");
	lrObject.incredu_seq = dsMAIN2.NameString(dsMAIN2.RowPosition,"INCREDU_SEQ");
     	lrObject.div = "D";
     	var vIncreduCls = dsMAIN2.NameString(dsMAIN2.RowPosition,"INCREDU_CLS");
     	var strPage = '';
     	if(vIncreduCls=='1')//자본적지출
     	{
     	  	strPage = "t_PSlip01FixIncreduR.jsp";
     	}
     	else if(vIncreduCls=='3')//폐기
     	{
     		strPage = "t_PSlip03FixIncreduR.jsp";
     	}
     	else if(vIncreduCls=='4')//매각
     	{
     		strPage = "t_PSlip02FixIncreduR.jsp";
     	}
	var	arrRtn2 = window.showModalDialog("./" + strPage , lrObject, "center:yes; dialogWidth:1024px; dialogHeight:768px; status:no; help:no; scroll:no");
	if(C_isNull(arrRtn2)) return;
	var			liRow = dsMAIN2.RowPosition;
	G_Load(dsMAIN2);
	if(liRow > 0) dsMAIN2.RowPosition = liRow;
}
function	removeSlip()
{
	...
	var pSLIP_ID        = dsMAIN2.NameString(dsMAIN2.RowPosition,"SLIP_ID");
	...
	G_Load(dsREMOVE);
	dsREMOVE.NameString(dsREMOVE.RowPosition,"FIX_ASSET_SEQ") = dsMAIN2.NameString(dsMAIN2.RowPosition,"FIX_ASSET_SEQ");
	dsREMOVE.NameString(dsREMOVE.RowPosition,"INCREDU_SEQ")    = dsMAIN2.NameString(dsMAIN2.RowPosition,"INCREDU_SEQ");
...
//폐기 자동전표발행
function btnAutoSlipMake3_onClick()//폐기
{
	//if (!G_isLoaded("dsMAIN"))
	//{
		//C_msgOk("먼저 조회를 수행하세요");
...
{
	var pSLIP_ID                  = dsMAIN2.NameString(dsMAIN2.RowPosition,"SLIP_ID");
	var pSLIP_IDSEQ            = dsMAIN2.NameString(dsMAIN2.RowPosition,"SLIP_IDSEQ");
	var pMAKE_COMP_CODE = dsMAIN2.NameString(dsMAIN2.RowPosition,"MAKE_COMP_CODE");
	var pMAKE_DT 	       = dsMAIN2.NameString(dsMAIN2.RowPosition,"MAKE_DT_TRANS");
	var pMAKE_SEQ     	       = dsMAIN2.NameString(dsMAIN2.RowPosition,"MAKE_SEQ");
	var pSLIP_KIND_TAG      = dsMAIN2.NameString(dsMAIN2.RowPosition,"SLIP_KIND_TAG");
	var pREADONLY_TF        = "F";
	if(C_isNull(pSLIP_ID))
	{
		C_msgOk("전표가 발행된 것이 없습니다.");
