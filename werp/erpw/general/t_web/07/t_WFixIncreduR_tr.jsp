		...
		if(!CITCommon.isNull(strAct) && strAct.equals("REMOVE"))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					strSql = "{call SP_T_FIX_REM_INCREDU_SLIP(?,?)}";
					...
					lrArgData.setValue("INCREDU_SEQ", rows[i].getString(dsREMOVE.indexOfColumn("INCREDU_SEQ")));
		...
		if (CITCommon.isNull(strAct))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_FIX_INCREDU_I(?,?,?,?,?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_FIX_INCREDU_U(?,?,?,?,?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_FIX_INCREDU_D(?,?)}";
