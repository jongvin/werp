...
function Initialize()
{
	...
	G_addDataSet(dsMAIN,  trans,   gridMAIN, sSelectPageName+D_P1("ACT","MAIN"), "안분분류코드");
	...
	G_addRelCol(dsSUB01,"DIV_CODE","DIV_CODE");
...
function OnLoadBefore(dataset)
{
	if(dataset == dsMAIN)
	{
		dataset.DataID = sSelectPageName + D_P1("ACT","MAIN")
										 + D_P2("DIV_COMP_CODE",txtCOMP_CODE.value);
	}
	else if(dataset == dsSUB01)
	{
		dataset.DataID = sSelectPageName + D_P1("ACT","SUB01")
										 + D_P2("DIV_COMP_CODE",dsMAIN.NameString(dsMAIN.RowPosition, "DIV_COMP_CODE"))
										 + D_P2("DIV_CODE",dsMAIN.NameString(dsMAIN.RowPosition, "DIV_CODE"));
	}
}
...
function btnadd_onclick()
{
	if(G_FocusDataset==dsMAIN) {
		...
		if(dsMAIN.CountRow==0) {
			C_msgOk("안분분류코드를 먼저 선택하세요.", "확인");
			return;
		}
		var lrArgs = new C_Dictionary();
		var lrRet = null;
	
		lrArgs.set("COMP_CODE1", dsMAIN.NameString(dsMAIN.RowPosition, "DIV_COMP_CODE"));
		lrArgs.set("COMP_CODE2", dsMAIN.NameString(dsMAIN.RowPosition, "DIV_COMP_CODE"));
		lrArgs.set("SEARCH_CONDITION", "");
	
		lrRet = C_LOV_NEW("T_DEPT_CODE5", null, lrArgs, "T", "T");
		...
		for (idx = 0; idx < lrRet.length; idx++)
		{
			...
			//D_defaultAdd();
			G_addRow(dsSUB01);
			dsSUB01.NameString(dsSUB01.RowPosition, "DIV_DEPT_CODE") = lrArgs.get("DIV_DEPT_CODE");
			dsSUB01.NameString(dsSUB01.RowPosition, "DIV_DEPT_NAME") = lrArgs.get("DIV_DEPT_NAME");
		}
	}
}
