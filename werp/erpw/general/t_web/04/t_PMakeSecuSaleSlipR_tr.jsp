		...
		if(strAct.equals("MAKE_DEFAULT"))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_MAKE_DFLT_SECU_SALE_SLIP(?,?,?,?,?,?)}";
						...
						lrArgData.setValue("SECU_NO", rows[i].getString(dsWORK_SLIP_IDSEQ.indexOfColumn("SECU_NO")));
		...
		if(strAct.equals("MAKE_SLIP"))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					if (rows[i].getJobType() == GauceDataRow.TB_JOB_UPDATE)
					{
						strSql = "{call SP_T_FIN_MAKE_SECU_SALE_SLIP(?,?,?,?,?,?,?)}";
						...
						lrArgData.setValue("SECU_NO", rows[i].getString(dsMAKE_SLIP.indexOfColumn("SECU_NO")));
						lrArgData.setValue("CRTUSERNO", strUserNo);
					}
					else
					{
