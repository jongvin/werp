...
/**************************************************************************/
	...
	try
	{
		...
		if(!CITCommon.isNull(strAct) && strAct.equals("COPY"))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					strSql = "{call SP_T_FL_GET_DAY_EXEC(?,?,?,?)}";
		...
		else if(!CITCommon.isNull(strAct) && strAct.equals("REMOVE"))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					strSql = "{call SP_T_FL_REMOVE_DAY_EXEC_B(?,?,?)}";
		...
		else if (CITCommon.isNull(strAct))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					if (rows[i].getJobType() == GauceDataRow.TB_JOB_UPDATE)
					{
						...
						strWORK_DT = rows[i].getString(dsMAIN.indexOfColumn("WORK_DT"));


						strSql = "{call SP_T_FL_DAY_EXEC_B(?,?,?,?,?,?)}";
				...
				if(iCount > 0)
				{
					...
					strSql = "{call SP_T_FL_DAY_EXEC_B_SUM(?,?,?)}";
			...
			{
				if (GauceInfo == null) GauceInfo = new CITGauceInfo(request, response);
				GauceInfo.response.writeException("USER", "900001", "SP_T_FL_DAY_EXEC_B_SUM 테이블 데이타 갱신 에러" + ex.getMessage());
				throw new Exception(ex.getMessage());
			}
		}
		conn.commit();
