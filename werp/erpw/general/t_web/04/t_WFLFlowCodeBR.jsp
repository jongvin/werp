...
<%
/**************************************************************************/
/* 1. 프 로 그 램 id : t_WFLFlowCodeBR(본사자금수지분류코드등록)
/* 2. 유형(시나리오) : 
/* 3. 기  능  정  의 : 프로그램 설명 
...
/**************************************************************************/
	...
	CITData lrReturnDataB1 = null;
	CITData lrReturnDataB2 = null;
	CITData lrReturnDataB3 = null;
//디자인 외 부분은 여기만 손대자!!!!!
//파일명	
	String strFileName = "./t_WFLFlowCodeBR";
//저장 액션
	String strUpdateKeyValue = "Service1(I:dsMAIN=dsMAIN,I:dsMAIN_D=dsMAIN_D,I:dsSUB01=dsSUB01,I:dsSUB02=dsSUB02)";
	...
	try
	{
		...
		try
		{
			...
			strSql = 
				"Select"+"\n"+
				"	a.COMP_PLN_FUNC_NO CODE,"+"\n"+
				"	a.FUNC_TITLE NAME"+"\n"+
				"From	T_FL_COMP_PLN_FUNCTIONS a"+"\n"+
				"Order By"+"\n"+
				"	a.FUNC_TITLE"+"\n";
		...
		try
		{
			...
			strSql = 
				"Select"+"\n"+
				"	a.COMP_EXE_FUNC_NO CODE,"+"\n"+
				"	a.FUNC_TITLE NAME"+"\n"+
				"From	T_FL_COMP_EXE_FUNCTIONS a"+"\n"+
				"Order By"+"\n"+
				"	a.FUNC_TITLE"+"\n";
		...
		try
		{
			...
			strSql = 
				"Select"+"\n"+
				"	a.COMP_FOR_FUNC_NO CODE,"+"\n"+
				"	a.FUNC_TITLE NAME"+"\n"+
				"From	T_FL_COMP_FORE_FUNCTIONS a"+"\n"+
				"Order By"+"\n"+
				"	a.FUNC_TITLE"+"\n";
		...
		try
		{
			...
			strSql = 
				"Select"+"\n"+
				"	a.COMP_PLN_FUNC_NO_B CODE,"+"\n"+
				"	a.FUNC_TITLE NAME"+"\n"+
				"From	T_FL_COMP_PLN_FUNCTIONS_B a"+"\n"+
				"Order By"+"\n"+
				"	a.FUNC_TITLE"+"\n";
			lrReturnDataB1 = CITDatabase.selectQuery(strSql, lrArgData);
		}
		catch (Exception ex)
		{
			throw new Exception("SYS-100001:페이지 초기화 오류 -> " + ex.getMessage());
		...
		try
		{
			...
			strSql = 
				"Select"+"\n"+
				"	a.COMP_EXE_FUNC_NO_B CODE,"+"\n"+
				"	a.FUNC_TITLE NAME"+"\n"+
				"From	T_FL_COMP_EXE_FUNCTIONS_B a"+"\n"+
				"Order By"+"\n"+
				"	a.FUNC_TITLE"+"\n";
			lrReturnDataB2 = CITDatabase.selectQuery(strSql, lrArgData);
		}
		catch (Exception ex)
		{
			throw new Exception("SYS-100001:페이지 초기화 오류 -> " + ex.getMessage());
		...
		try
		{
			...
			strSql = 
				"Select"+"\n"+
				"	a.COMP_FOR_FUNC_NO_B CODE,"+"\n"+
				"	a.FUNC_TITLE NAME"+"\n"+
				"From	T_FL_COMP_FORE_FUNCTIONS_B a"+"\n"+
				"Order By"+"\n"+
				"	a.FUNC_TITLE"+"\n";
			lrReturnDataB3 = CITDatabase.selectQuery(strSql, lrArgData);
		}
		catch (Exception ex)
		{
			throw new Exception("SYS-100001:페이지 초기화 오류 -> " + ex.getMessage());
...
%>			
			...
			arCODEB1 = new Array();
<%
			for(int i = 0 ; i < lrReturnDataB1.getRowsCount(); ++i)
			{
				String		sCode = lrReturnDataB1.toString(i, "CODE");
				String		sName = lrReturnDataB1.toString(i, "NAME");
%>
				arCODEB1.push(new F_CodeName("<%=sCode%>","<%=sName%>"));
<%
			}
%>			
			arCODEB2 = new Array();
<%
			for(int i = 0 ; i < lrReturnDataB2.getRowsCount(); ++i)
			{
				String		sCode = lrReturnDataB2.toString(i, "CODE");
				String		sName = lrReturnDataB2.toString(i, "NAME");
%>
				arCODEB2.push(new F_CodeName("<%=sCode%>","<%=sName%>"));
<%
			}
%>			
			arCODEB3 = new Array();
<%
			for(int i = 0 ; i < lrReturnDataB3.getRowsCount(); ++i)
			{
				String		sCode = lrReturnDataB3.toString(i, "CODE");
				String		sName = lrReturnDataB3.toString(i, "NAME");
%>
				arCODEB3.push(new F_CodeName("<%=sCode%>","<%=sName%>"));
<%
			}
%>			
		...
		<!--CONVPAGE_HEAD 지우지마세요//--> <comment id="__NSID__4"> <!--CONVPAGE_TAIL 지우지마세요//--><object id=dsMAIN_D classid=CLSID:3267EA0D-B5D8-11D2-A4F9-00608CEBEE49>
		...
		<!--CONVPAGE_HEAD 지우지마세요//--> <comment id="__NSID__107"> <!--CONVPAGE_TAIL 지우지마세요//--><object id=dsACC_CODE classid=CLSID:3267EA0D-B5D8-11D2-A4F9-00608CEBEE49>
		</object><!--CONVPAGE_HEAD 지우지마세요//--></comment> <script> __WS__(__NSID__107); </script> <!--CONVPAGE_TAIL 지우지마세요//-->
											...
											<td width="100%">
												<table width="100%" border="0" cellspacing="0" cellpadding="0">
													<tr>
														...
														<td class="font_green_bold"> 본사자금수지항목</td>
														<td align="right" width="*">
															...
															<input id="btnDeleteNode" type="button" class="img_btn4_1" value="삭제" title="Alt+D" onclick="btnDelete_onClick()" onmouseover="this.style.color=MOUSE_OVER;" onmouseout="this.style.color=MOUSE_OUT;">
											...
											<td width="50%">
												...
												<table width="100%" border="0" cellspacing="0" cellpadding="0">
													<tr height="23">
														...
														<td width="160" id="tab1" align="center" background="../images/Content_tab_bgimage_r.gif" class="font_tab"><a href="javascript:selectTab(1, 2);" onfocus="this.blur()">실적집계 기준 계정정의</a></td>
														...
														<td width="160" id="tab2" align="center" background="../images/Content_tab_bgimage.gif" class="font_tab"><a href="javascript:selectTab(2, 2);" onfocus="this.blur()">관련사업장 정의</a></td>
														...
														<tr>
															<td width="100%" height="100%">
																<!--CONVPAGE_HEAD 지우지마세요//--> <comment id="__NSID__13"> <!--CONVPAGE_TAIL 지우지마세요//--><OBJECT id=gridSUB02 WIDTH="100%" HEIGHT="100%" classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49 VIEWASTEXT>
																	...
																	<PARAM NAME="ColSizing" VALUE="-1">
																	<PARAM NAME="Format" VALUE=" 
																		<C> Name=사업장코드 ID=COMP_CODE Align=Center Width=90
																		</C> 
																		<C> Name=사업장명 ID=COMPANY_NAME Align=Left Width=300
																		</C> 
																		">
