...
/**************************************************************************/
	...
	try
	{
		...
		if (strAct.equals("MAIN"))
		{
			...
			strSql  = "  \n";
			strSql += " select lpad(' ', (level-1) *2 ) || flow_item_name flow_item_name,  \n";
			...
			strSql += " 	   c_plan_rate, \n";
			strSql += " 	   b_rate, \n";
			strSql += " 	   n_y_forecast_amt, \n";
			strSql += " 	   n_rate  \n";
			strSql += " from t_fl_flow_code_b a, \n";
			strSql += " 	 (select  \n";
			strSql += " 	 		 b2.flow_code_b, \n";
			...
			strSql += " 			 decode(nvl(n_y_forecast_amt,0),0, 0,  round(n_y_forecast_amt,0)) n_y_forecast_amt, \n";
			strSql += " 			 decode(nvl(n_y_forecast_amt,0), 0, 0, round(((nvl(c_y_exec_amt19,0) + nvl(forecast_amt10,0) + nvl(forecast_amt11,0) + nvl(forecast_amt12,0))/nvl(n_y_forecast_amt,0))*100,1)) n_rate \n";
			strSql += " 	  from  \n";
			strSql += " 			 (select comp_code, clse_acc_id + 1 clse_acc_id, flow_code_b,  b_y_exec_amt \n";
			strSql += " 			  from  \n";
			strSql += " 				 (select comp_code, clse_acc_id, flow_code_b, sum(exec_amt) b_y_exec_amt \n";
			strSql += " 				  from t_fl_month_plan_exec_b \n";
			strSql += " 				  where comp_code =  ?  \n";
			strSql += " 				  and	clse_acc_id =  ?  - 1 \n";
			strSql += " 				  group by comp_code, clse_acc_id, flow_code_b) \n";
			strSql += " 			  ) b1, \n";
			strSql += " 			 (select comp_code, clse_acc_id, flow_code_b, sum(exec_amt) c_y_exec_amt19 \n";
			strSql += " 			  from t_fl_month_plan_exec_b \n";
			strSql += " 			  where comp_code =  ?  \n";
			strSql += " 			  and	clse_acc_id =  ?  \n";
			strSql += " 			  and substr(work_ym, 5,6) in ('01', '02', '03','04','05','06','07','08','09') \n";
			strSql += " 			  group by comp_code, clse_acc_id, flow_code_b) b2, \n";
			strSql += " 			  (select comp_code, clse_acc_id, flow_code_b,  \n";
			...
			strSql += " 			  group by comp_code, clse_acc_id, flow_code_b \n";
			strSql += " 			  ) b3, \n";
			strSql += " 			 (select comp_code, clse_acc_id, flow_code_b, sum(plan_amt) c_y_plan_amt \n";
			strSql += " 			  from t_fl_month_plan_exec_b \n";
			strSql += " 			  where comp_code =  ?  \n";
			strSql += " 			  and	clse_acc_id =  ?  \n";
			strSql += " 			  group by comp_code, clse_acc_id, flow_code_b) b4, \n";
			strSql += " 			  (select comp_code, clse_acc_id, flow_code_b, sum(forecast_amt) c_y_forecast_amt \n";
			strSql += " 			  from t_fl_month_plan_exec_b \n";
			strSql += " 			  where comp_code =  ?  \n";
			strSql += " 			  and	clse_acc_id =  ?  \n";
			strSql += " 			  group by comp_code, clse_acc_id, flow_code_b) b5, \n";
			strSql += " 			  (select comp_code, clse_acc_id - 1 clse_acc_id, flow_code_b, n_y_forecast_amt \n";
			strSql += " 			   from \n";
			strSql += " 				  (select comp_code, clse_acc_id, flow_code_b, sum(plan_amt) n_y_forecast_amt \n";
			strSql += " 				  from t_fl_month_plan_exec_b \n";
			strSql += " 				  where comp_code =  ?  \n";
			strSql += " 				  and	clse_acc_id =  ?  + 1 \n";
			strSql += " 				  group by comp_code, clse_acc_id, flow_code_b) \n";
			...
			strSql += " 	  and	  b2.comp_code = b4.comp_code(+) \n";
			strSql += " 	  and	  b2.comp_code = b5.comp_code(+) \n";
			strSql += " 	  and	  b2.comp_code = b6.comp_code(+) \n";
			strSql += " 	  and	  b2.clse_acc_id = b1.clse_acc_id(+) \n";
			strSql += " 	  and	  b2.clse_acc_id = b3.clse_acc_id(+)  \n";
			strSql += " 	  and	  b2.clse_acc_id = b4.clse_acc_id(+)  \n";
			strSql += " 	  and	  b2.clse_acc_id = b5.clse_acc_id(+) \n";
			strSql += " 	  and	  b2.clse_acc_id = b6.clse_acc_id(+) \n";
			strSql += " 	  and	  b2.flow_code_b = b1.flow_code_b(+) \n";
			strSql += " 	  and	  b2.flow_code_b = b3.flow_code_b(+) \n";
			strSql += " 	  and	  b2.flow_code_b = b4.flow_code_b(+) \n";
			strSql += " 	  and	  b2.flow_code_b = b5.flow_code_b(+) \n";
			strSql += " 	  and	  b2.flow_code_b = b6.flow_code_b(+) \n";
			strSql += " 	  ) b	   \n";
			strSql += " where a.flow_code_b = b.flow_code_b(+) \n";
			strSql += " start with p_no= 0 \n";
			strSql += " connect by prior a.flow_code_b = p_no \n";
			strSql += " order siblings by flow_item_kind, level_seq ";
