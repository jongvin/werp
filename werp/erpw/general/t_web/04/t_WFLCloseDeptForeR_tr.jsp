		...
		if (CITCommon.isNull(strAct))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_FL_COMP_FORECAST_CLOSE_I(?,?,?,?,?,?)}";
						...
						lrArgData.setValue("QUARTER_YEAR", rows[i].getString(dsMASTER.indexOfColumn("QUARTER_YEAR")));
					...
					{
						strSql = "{call SP_T_FL_COMP_FORECAST_CLOSE_U(?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_FL_COMP_FORECAST_CLOSE_D(?,?,?)}";
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					if (rows[i].getJobType() == GauceDataRow.TB_JOB_INSERT)
					{
						strSql = "{call SP_T_FL_DEPT_FORECAST_CLOSE_I(?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_FL_DEPT_FORECAST_CLOSE_U(?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_FL_DEPT_FORECAST_CLOSE_D(?,?,?,?)}";
