/**************************************************************************/
/* 1. 프 로 그 램 id : t_WFLComPlanR(본사자금수지전망수립)
/* 2. 유형(시나리오) : 
/* 3. 기  능  정  의 : 프로그램 설명 
...
"	</C> \n" +
"	<C> Name=구분 ID=SUBTITLE_NAME Align=Center Width=60   \n" +
"	</C> \n" +
"	</FG> \n" +
"	<G> Name='전월실적' id='G2' \n" +
...
"		</C> \n" +
"		<C> Name='전망금액' ID=FORECAST_AMT_01	Width=95 BgColor={decode(LV,1,'#DCDCDC',2,'#DCDCDC',3,decode(IS_LEAF_TAG,'T','white','F','#EFEFEF'),4,'#87CEEB')} \n" +
"		</C> \n" +
...
"		</C> \n" +
"		<C> Name='전망금액' ID=FORECAST_AMT_02	Width=95 BgColor={decode(LV,1,'#DCDCDC',2,'#DCDCDC',3,decode(IS_LEAF_TAG,'T','white','F','#EFEFEF'),4,'#87CEEB')} \n" +
"		</C> \n" +
...
"		</C> \n" +
"		<C> Name='전망금액' ID=FORECAST_AMT_03	Width=95 BgColor={decode(LV,1,'#DCDCDC',2,'#DCDCDC',3,decode(IS_LEAF_TAG,'T','white','F','#EFEFEF'),4,'#87CEEB')} \n" +
"		</C> \n" +
...
function Initialize()
{
	G_addDataSet(dsMAIN, trans, gridMAIN, null, "본사자금수지전망",null,null,true);
...
{
	if(colid != 'LEVELED_FLOW_ITEM_NAME' && colid != 'SUBTITLE_NAME' && colid != 'EXEC_AMT' && colid != 'PLAN_AMT_01' && colid != 'PLAN_AMT_02' && colid != 'PLAN_AMT_03')
	{
		if(dataset.NameString(row,"IS_LEAF_TAG") == "T" && dataset.NameString(row,"LV") == "3")
		{
...
function	btnGetConsInfo_onClick()
{
	...
	dsCOPY.NameString(dsCOPY.RowPosition,"QUARTER_YEAR") = cboQUARTER_YEAR.value;
	transCopy.Parameters = "ACT=COPY";
	if(!G_saveData(dsCOPY))return;
	C_msgOk("현장별수지에서 가져오기가 정상적으로 수행되었습니다.");
	G_Load(dsMAIN);
}
function	btnRemoveAll()
{
	if(!checkConditions()) return;
	if(checkClose()) return;
	var	ret = C_msgYesNo("이 작업을 수행하시면 현재 선택된 사업장의 자금수지 전망이 완전히 삭제됩니다.<br> 정말 작업을 수행하시겠습니까?","경고");
	if(ret == "N") return;
	G_Load(dsREMOVE);
	...
	dsREMOVE.NameString(dsREMOVE.RowPosition,"QUARTER_YEAR") = cboQUARTER_YEAR.value;
	transRemove.Parameters = "ACT=REMOVE";
	if(!G_saveData(dsREMOVE))return;
	C_msgOk("자금수지전망삭제가 정상적으로 수행되었습니다.");
	G_Load(dsMAIN);
}
