		...
		if(!CITCommon.isNull(strAct) && strAct.equals("SUM"))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					strSql = "{call SP_T_MAKE_PERS_COST(?,?,?,?,?)}";
					...
					lrArgData.setValue("DEPT_CODE", rows[i].getString(dsSUM.indexOfColumn("DEPT_CODE")));
					lrArgData.setValue("WORK_DT", rows[i].getString(dsSUM.indexOfColumn("WORK_DT")));
					lrArgData.setValue("WORK_NO", rows[i].getString(dsSUM.indexOfColumn("WORK_NO")));
	...
	//복사시작
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_FIN_PERSON_COST_I(?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_FIN_PERSON_COST_U(?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_FIN_PERSON_COST_D(?,?)}";
			...
			{
				if (GauceInfo == null) GauceInfo = new CITGauceInfo(request, response);
				GauceInfo.response.writeException("USER", "900001", "T_FIN_PERSON_COST 테이블 데이타 갱신 에러" + ex.getMessage());
				throw new Exception(ex.getMessage());
			}
	//복사끝
	//복사시작
		...
		if(!CITCommon.isNull(strAct) && strAct.equals("REMOVE"))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					strSql = "{call SP_T_REMOVE_PER_COST_SLIP(?,?,?,?)}";
