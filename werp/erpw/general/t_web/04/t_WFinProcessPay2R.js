/**************************************************************************/
/* 1. 프 로 그 램 id : t_WFinProcessPay1R.jsp(대금지급관리거래처실물어음)
...
function Initialize()
{
	G_addDataSet(dsMAIN, trans, gridMAIN, sSelectPageName+D_P1("ACT","MAIN"), "대금지급관리거래처실물어음");
	...
	G_setReadOnlyCol(gridMAIN,"MASKED_CUST_CODE");
	...
	G_setLovCol(gridMAIN,"CHK_BILL_NO");
...
function	btnSelectDt_onClick()
{
	for(var i = 1 ; i <= dsMAIN.CountRow ; ++ i)
	{
		if(dsMAIN.NameString(i,"EXPR_DT") == txtDT_APPLY.value)
		{
