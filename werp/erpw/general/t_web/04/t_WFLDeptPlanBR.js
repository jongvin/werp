...
function Initialize()
{
	G_addDataSet(dsMAIN, trans, gridMAIN, null, "본사자금수지현장별계획",null,null,true);
...
function	btnGetConsInfo_onClick()
{
	...
	C_msgOk("시공계획에서 가져오기가 정상적으로 수행되었습니다.");
	G_Load(dsMAIN);
}
function	btnRemoveAll()
{
	if(!checkConditions()) return;
	if(checkClose()) return;
	var	ret = C_msgYesNo("이 작업을 수행하시면 현재 선택된 현장의 자금수지 계획이 완전히 삭제됩니다.<br> 정말 작업을 수행하시겠습니까?","경고");
	if(ret == "N") return;
	G_Load(dsREMOVE);
