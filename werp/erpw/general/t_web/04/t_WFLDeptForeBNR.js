/**************************************************************************/
/* 1. 프 로 그 램 id : t_WFLDeptForeBR(본사자금수지 현장별전망수립)
/* 2. 유형(시나리오) : 
/* 3. 기  능  정  의 : 프로그램 설명 
...
function Initialize()
{
	G_addDataSet(dsMAIN, trans, gridMAIN, null, "현장별자금수지전망",null,null,true);
...
function btnDEPT_CODE_onClick()
{
	...
	lrRet = C_LOV("T_VIRRTUAL_DEPT", lrArgs,"T");
...
function	btnRemoveAll()
{
	if(!checkConditions()) return;
	if(checkClose()) return;
	var	ret = C_msgYesNo("이 작업을 수행하시면 현재 선택된 현장의 자금수지 전망이 완전히 삭제됩니다.<br> 정말 작업을 수행하시겠습니까?","경고");
	if(ret == "N") return;
	G_Load(dsREMOVE);
