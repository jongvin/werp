/**************************************************************************/
/* 1. 프 로 그 램 id : t_WFLFlowCodeR(본사자금수지분류코드등록)
/* 2. 유형(시나리오) : 
/* 3. 기  능  정  의 : 프로그램 설명 
...
function Initialize()
{
	G_addDataSet(dsMAIN, trans, null, sSelectPageName+D_P1("ACT","MAIN"), "본사자금수지코드");
	G_addDataSet(dsMAIN_D, trans, null, sSelectPageName+D_P1("ACT","MAIN_D"), "본사자금수지코드삭제");
	G_addDataSet(dsSUB01, trans, gridSUB01, sSelectPageName+D_P1("ACT","SUB01"), "실적집계기준계정정의");
	G_addDataSet(dsSUB02, trans,  gridSUB02, sSelectPageName+D_P1("ACT","SUB02"), "관련사업장정의");
	G_addDataSet(dsLOV, null, null, sSelectPageName+D_P1("ACT","LOV"), "LOV");
	G_addDataSet(dsFLOW_CODE, null, null, sSelectPageName+D_P1("ACT","FLOW_CODE"), "본사자금수지코드번호");
	...
	fgManager = F_addFlexGrid(fg,dsMAIN,dsMAIN_D,1,"FLOW_ITEM_NAME","LV","LEVEL_SEQ","FLOW_CODE","P_NO","신규자금수지항목");
	...
	fg.FrozenCols = 1;
	F_setCellValue(fg,0,fgManager.getColumnIndex("FLOW_ITEM_NAME"),"자금수지항목명");
	F_setCellValue(fg,0,fgManager.getColumnIndex("MNG_CODE"),"관리코드");
	F_setCellValue(fg,0,fgManager.getColumnIndex("FLOW_ITEM_KIND"),"항목구분");
	F_setCellValue(fg,0,fgManager.getColumnIndex("COMP_PLN_FUNC_NO_B"),"계획본사집계방법");
	F_setCellValue(fg,0,fgManager.getColumnIndex("COMP_EXE_FUNC_NO_B"),"실적본사집계방법");
	F_setCellValue(fg,0,fgManager.getColumnIndex("COMP_FOR_FUNC_NO_B"),"전망본사집계방법");
