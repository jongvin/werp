...
/**************************************************************************/
	...
	GauceDataSet dsMAIN05 = null;
	GauceDataSet dsLIST05 = null;
	...
	try
	{
		...
		if (CITCommon.isNull(strAct))
		{
			...
			dsMAIN05 = GauceInfo.request.getGauceDataSet("dsMAIN05");
			
			if (dsMAIN05 == null) throw new Exception("dsMAIN05이(가) 널(Null)입니다.");
			
			rows = dsMAIN05.getDataRows();
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					if (rows[i].getJobType() == GauceDataRow.TB_JOB_INSERT)
					{
						...
						lrArgData.addRow();
						lrArgData.setValue("CHK_BILL_NO", rows[i].getString(dsMAIN05.indexOfColumn("CHK_BILL_NO")));
						lrArgData.setValue("CRTUSERNO", strUserNo);
						lrArgData.setValue("CHK_BILL_CLS", rows[i].getString(dsMAIN05.indexOfColumn("CHK_BILL_CLS")));
						lrArgData.setValue("BILL_KIND", rows[i].getString(dsMAIN05.indexOfColumn("BILL_KIND")));
						lrArgData.setValue("STAT_CLS", rows[i].getString(dsMAIN05.indexOfColumn("STAT_CLS")));
						lrArgData.setValue("COMP_CODE", rows[i].getString(dsMAIN05.indexOfColumn("COMP_CODE")));
						lrArgData.setValue("BANK_CODE", rows[i].getString(dsMAIN05.indexOfColumn("BANK_CODE")));
						lrArgData.setValue("CUST_SEQ", rows[i].getString(dsMAIN05.indexOfColumn("CUST_SEQ")));
						lrArgData.setValue("ACPT_DT", rows[i].getString(dsMAIN05.indexOfColumn("ACPT_DT")));
						lrArgData.setValue("PUBL_DT", rows[i].getString(dsMAIN05.indexOfColumn("PUBL_DT")));
						lrArgData.setValue("PUBL_AMT", rows[i].getString(dsMAIN05.indexOfColumn("PUBL_AMT")));
						lrArgData.setValue("EXPR_DT", rows[i].getString(dsMAIN05.indexOfColumn("EXPR_DT")));
						lrArgData.setValue("CHG_EXPR_DT", rows[i].getString(dsMAIN05.indexOfColumn("CHG_EXPR_DT")));
						lrArgData.setValue("DUSE_DT", rows[i].getString(dsMAIN05.indexOfColumn("DUSE_DT")));
						lrArgData.setValue("CUST_DOUT_DT", rows[i].getString(dsMAIN05.indexOfColumn("CUST_DOUT_DT")));
						lrArgData.setValue("COLL_DT", rows[i].getString(dsMAIN05.indexOfColumn("COLL_DT")));
						lrArgData.setValue("RETURN_DT", rows[i].getString(dsMAIN05.indexOfColumn("RETURN_DT")));
						lrArgData.setValue("DISC_RAT", rows[i].getString(dsMAIN05.indexOfColumn("DISC_RAT")));
						lrArgData.setValue("DISC_AMT", rows[i].getString(dsMAIN05.indexOfColumn("DISC_AMT")));
						lrArgData.setValue("SLIP_ID", rows[i].getString(dsMAIN05.indexOfColumn("SLIP_ID")));
						lrArgData.setValue("SLIP_IDSEQ", rows[i].getString(dsMAIN05.indexOfColumn("SLIP_IDSEQ")));
						lrArgData.setValue("RESET_SLIP_ID", rows[i].getString(dsMAIN05.indexOfColumn("RESET_SLIP_ID")));
						lrArgData.setValue("RESET_SLIP_IDSEQ", rows[i].getString(dsMAIN05.indexOfColumn("RESET_SLIP_IDSEQ")));
						lrArgData.setValue("REMARKS", rows[i].getString(dsMAIN05.indexOfColumn("REMARKS")));
					}
					else if (rows[i].getJobType() == GauceDataRow.TB_JOB_UPDATE)
					{
						strSql = "{call SP_T_FIN_PAY_CHK_BILL_U(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
			...
			dsLIST05 = GauceInfo.request.getGauceDataSet("dsLIST05");
			
			if (dsLIST05 == null) throw new Exception("dsLIST05이(가) 널(Null)입니다.");
			
			rows = dsLIST05.getDataRows();
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					if (rows[i].getJobType() == GauceDataRow.TB_JOB_INSERT)
					{
						strSql = "{call SP_T_FIN_BILL_CHG_LIST_I(?,?,?,?,?,?)}";
						
						lrArgData.addColumn("CHK_BILL_NO", CITData.VARCHAR2);
						lrArgData.addColumn("CHG_SEQ", CITData.NUMBER);
						lrArgData.addColumn("CRTUSERNO", CITData.VARCHAR2);
						lrArgData.addColumn("CHG_DT", CITData.VARCHAR2);
						lrArgData.addColumn("CHG_EXPR_DT", CITData.VARCHAR2);
						lrArgData.addColumn("REMARKS", CITData.VARCHAR2);

						lrArgData.addRow();
						lrArgData.setValue("CHK_BILL_NO", rows[i].getString(dsLIST05.indexOfColumn("CHK_BILL_NO")));
						lrArgData.setValue("CHG_SEQ", rows[i].getString(dsLIST05.indexOfColumn("CHG_SEQ")));
						lrArgData.setValue("CRTUSERNO", strUserNo);
						lrArgData.setValue("CHG_DT", rows[i].getString(dsLIST05.indexOfColumn("CHG_DT")));
						lrArgData.setValue("CHG_EXPR_DT", rows[i].getString(dsLIST05.indexOfColumn("CHG_EXPR_DT")));
						lrArgData.setValue("REMARKS", rows[i].getString(dsLIST05.indexOfColumn("REMARKS")));
					}
					else if (rows[i].getJobType() == GauceDataRow.TB_JOB_UPDATE)
					{
						strSql = "{call SP_T_FIN_BILL_CHG_LIST_U(?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_FIN_BILL_CHG_LIST_D(?,?)}";
			...
			{
				if (GauceInfo == null) GauceInfo = new CITGauceInfo(request, response);
				GauceInfo.response.writeException("USER", "900001", "T_FIN_BILL_CHG_LIST 테이블 데이타 갱신 에러" + ex.getMessage());
				throw new Exception(ex.getMessage());
			}
		}
		conn.commit();
