		...
		if (CITCommon.isNull(strAct))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_FL_FLOW_CODE_B_I(?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_FL_FLOW_CODE_B_U(?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_FL_FLOW_CODE_B_D(?)}";
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					if (rows[i].getJobType() == GauceDataRow.TB_JOB_INSERT)
					{
						strSql = "{call SP_T_FL_FLOW_ACC_CODE_B_I(?,?,?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_FL_FLOW_ACC_CODE_B_U(?,?,?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_FL_FLOW_ACC_CODE_B_D(?,?)}";
	...
	//복사시작
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					if (rows[i].getJobType() == GauceDataRow.TB_JOB_INSERT)
					{
						strSql = "{call SP_T_FL_FLOW_COMP_B_I(?,?,?)}";
						...
						lrArgData.addRow();
						lrArgData.setValue("FLOW_CODE", rows[i].getString(dsSUB02.indexOfColumn("FLOW_CODE")));
						lrArgData.setValue("COMP_CODE", rows[i].getString(dsSUB02.indexOfColumn("COMP_CODE")));
						lrArgData.setValue("CRTUSERNO", strUserNo);
					}
					else if (rows[i].getJobType() == GauceDataRow.TB_JOB_UPDATE)
					{
						strSql = "{call SP_T_FL_FLOW_COMP_B_U(?,?,?)}";
					...
					{
						strSql = "{call SP_T_FL_FLOW_COMP_B_D(?,?)}";
			...
			{
				if (GauceInfo == null) GauceInfo = new CITGauceInfo(request, response);
				GauceInfo.response.writeException("USER", "900001", "T_FL_FLOW_COMP_B 테이블 데이타 갱신 에러" + ex.getMessage());
				throw new Exception(ex.getMessage());
			}
	//복사끝
		}
		if(iCnt > 0)
		{
			...
			{
				CITData lrArgData = new CITData();
				strSql = "{call SP_T_SET_FLOW_CODE_LEAF_B()}";
