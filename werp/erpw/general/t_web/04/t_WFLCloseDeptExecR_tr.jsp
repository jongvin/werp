		...
		if (CITCommon.isNull(strAct))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_FL_COMP_EXEC_CLOSE_I(?,?,?,?,?,?)}";
						...
						lrArgData.setValue("WORK_YM", rows[i].getString(dsMASTER.indexOfColumn("WORK_YM")));
					...
					{
						strSql = "{call SP_T_FL_COMP_EXEC_CLOSE_U(?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_FL_COMP_EXEC_CLOSE_D(?,?,?)}";
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					if (rows[i].getJobType() == GauceDataRow.TB_JOB_INSERT)
					{
						strSql = "{call SP_T_FL_DEPT_EXEC_CLOSE_I(?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_FL_DEPT_EXEC_CLOSE_U(?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_FL_DEPT_EXEC_CLOSE_D(?,?,?,?)}";
