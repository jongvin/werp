...
function btnsave_proc_onclick()
{
	...
	//alert('전표프로그램 수정중!!! = '+dsSLIP_H.NameString(1,"MAKE_SEQ"));
	//return;
	...
	// 신규저장, 전표번호 구성항목 수정일 경우만, 실 전표순번을 가져와 다시 조회한다.
...
{
	// if( txtMAKE_SEQ.readOnly == false && txtMAKE_SLIPNO.value == "") {
	if (dsSLIP_H.CountRow == 0) {
		C_msgOk("전표입력상태가 아닙니다.<br>전표조회나 추가를 실행한후 다시 실행하십시오", "확인");
		return;
	}
...
//전표번호변경
function btnSLIP_NO_CHG_onClick()
{
	if( !chkDayClose(true) ) return;
	...
	if (dsSLIP_H.NameString(dsSLIP_H.RowPosition, "KEEP_CLS") == "T") {
		C_msgOk("확정전표는 발의일자를 수정할 수 없습니다.", "확인");
		return;
	}
	...
	var	arrRtn = window.showModalDialog(
		"t_PSlipNoChangeR.jsp?TYPE=CONF_T"+
		"&SLIP_ID="+vSlipId+
		"&MAKE_SLIPNO="+vMakeSlipNo+
		"&MAKE_DT="+C_toDateFormatString(vMAKE_DT),
		myObject,
		"center:yes; dialogWidth:406px;	dialogHeight:215px;	status:no; help:no;	scroll:no"
	);
	...
	else if(arrRtn.RESULT=="변경완료")
	{
		txtMAKE_COMP_CODE.value	= arrRtn.MAKE_COMP_CODE;
		txtMAKE_DT.value		= arrRtn.MAKE_DT;
		txtMAKE_DT_TRANS.value	= arrRtn.MAKE_DT_TRANS;
		txtMAKE_SEQ.value		= arrRtn.MAKE_SEQ;
		cboSLIP_KIND_TAG.value	= arrRtn.SLIP_KIND_TAG;
...
function Confirm_Trans_Button()
{
	...
	if (sSlip_Trans_Cls == "T")
	{
		if (dsSLIP_H.NameString(dsSLIP_H.RowPosition, "KEEP_CLS") == "T")
		{
			//document.all.btnSLIP_NO_CHG.style.display	= "none";
		...
		{
			//document.all.btnSLIP_NO_CHG.style.display	= "";
