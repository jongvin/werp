...
function btnDETAIL_COPY_onClick()
{
	...
	for(i=0;i<arSelectRow.length;i++){
		opener.G_addRow(opener.dsSLIP_D);
		orgIdx = arSelectRow[i];
		curIdx = opener.dsSLIP_D.RowPosition;
		// AccSlipDataCopy(formDS, fromROW, toDS, toROW)
		opener.AccSlipDataCopy(dsMAIN, orgIdx, opener.dsSLIP_D, curIdx);
	}
		
	opener.Input_Detail_Display();
	opener.Input_Detail_Clear();
	opener.Input_Color_ReadOnly();
	opener.Input_Clear();
}