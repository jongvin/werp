...
function OnLoadBefore(dataset)
{
	if (dataset == dsMAIN)	//시스템명 가져오기
	{
		dataset.DataID = sSelectPageName	+ D_P1("ACT","MAIN")
											...
											+ D_P2("CASH_ACC_CODE",vCashAccCode)
											+ D_P2("COMP_CODE",txtCOMP_CODE.value)
											+ D_P2("MAKE_DEPT_CODE",txtMAKE_DEPT_CODE.value)
											+ D_P2("ADJUST",chkAdjust.checked?'T':'F');
	}
}
...
function btnquery_prt_onclick()
{
	...
	strTemp  += "&&pADJUST__" + (chkAdjust.checked?'T':'F');
	strTemp += "&&pACC_CODE__111010100";
...
function OnLoadCompleted(dataset, rowcnt)
{
	...
	{
		G_addRow(dsMAIN);
		var row = dsMAIN.CountRow;
		dsMAIN.NameString(row,"LINE_FLAG") = "type5"
		dsMAIN.NameString(row,"ACC_NAME") = "          총    계"
		dsMAIN.NameValue(row,"SUM_DB_AMT") = dsMAIN.NameValue(row-1,"SUM_DB_AMT")+dsMAIN.NameValue(row-2,"SUM_DB_AMT")
		dsMAIN.NameValue(row,"CUR_DB_AMT") = dsMAIN.NameValue(row-1,"CUR_DB_AMT")+dsMAIN.NameValue(row-2,"CUR_DB_AMT")
		dsMAIN.NameValue(row,"REP_DB_AMT") =  dsMAIN.NameValue(row-1,"REP_DB_AMT")+dsMAIN.NameValue(row-2,"REP_DB_AMT")
		dsMAIN.NameValue(row,"SUM_CR_AMT") = dsMAIN.NameValue(row-1,"SUM_CR_AMT")+dsMAIN.NameValue(row-2,"SUM_CR_AMT")
		dsMAIN.NameValue(row,"CUR_CR_AMT") = dsMAIN.NameValue(row-1,"CUR_CR_AMT")+dsMAIN.NameValue(row-2,"CUR_CR_AMT")
		dsMAIN.NameValue(row,"REP_CR_AMT") =   dsMAIN.NameValue(row-1,"REP_CR_AMT")+dsMAIN.NameValue(row-2,"REP_CR_AMT")
		dsMAIN.ResetStatus();
	}
}
