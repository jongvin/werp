...
(
	Select
		...
		a.MNG_CODE,
		Sum( Case When InStr(c.GB1,b.COMP_CODE) > 0 And b.WORK_YM = :CLSE_ACC_ID_BF ||:MM Then b.EXEC_AMT Else Null End ) / :BASE_AMT EXEC_AMT_TOT_BF1,
		Sum( Case When InStr(c.GB1,b.COMP_CODE) > 0 And b.WORK_YM = :CLSE_ACC_ID_NW ||:MM  Then b.EXEC_AMT Else Null End ) / :BASE_AMT EXEC_AMT_TOT1,
		Sum( Case When InStr(c.GB1,b.COMP_CODE) > 0 And b.WORK_YM = :CLSE_ACC_ID_NW ||:MM  Then b.PLAN_AMT Else Null End ) / :BASE_AMT PLAN_AMT_TOT1,
		Sum( Case When InStr(c.GB1,b.COMP_CODE) > 0 And b.WORK_YM = :CLSE_ACC_ID_BF ||:MM  And a.MNG_CODE = '매출액' Then b.EXEC_AMT Else Null End ) / :BASE_AMT BASE_EXEC_AMT_TOT_BF1,
		Sum( Case When InStr(c.GB1,b.COMP_CODE) > 0 And b.WORK_YM = :CLSE_ACC_ID_NW ||:MM  And a.MNG_CODE = '매출액' Then b.PLAN_AMT Else Null End ) / :BASE_AMT BASE_PLAN_AMT_TOT1,
		Sum( Case When InStr(c.GB1,b.COMP_CODE) > 0 And b.WORK_YM = :CLSE_ACC_ID_NW ||:MM  And a.MNG_CODE = '매출액' Then b.EXEC_AMT Else Null End ) / :BASE_AMT BASE_EXEC_AMT_TOT1,
		Sum( Case When InStr(c.GB2,b.COMP_CODE) > 0 And b.WORK_YM = :CLSE_ACC_ID_BF ||:MM  Then b.EXEC_AMT Else Null End ) / :BASE_AMT EXEC_AMT_TOT_BF2,
		Sum( Case When InStr(c.GB2,b.COMP_CODE) > 0 And b.WORK_YM = :CLSE_ACC_ID_NW ||:MM  Then b.EXEC_AMT Else Null End ) / :BASE_AMT EXEC_AMT_TOT2,
		Sum( Case When InStr(c.GB2,b.COMP_CODE) > 0 And b.WORK_YM = :CLSE_ACC_ID_NW ||:MM  Then b.PLAN_AMT Else Null End ) / :BASE_AMT PLAN_AMT_TOT2,
		Sum( Case When InStr(c.GB2,b.COMP_CODE) > 0 And b.WORK_YM = :CLSE_ACC_ID_BF ||:MM  And a.MNG_CODE = '매출액' Then b.EXEC_AMT Else Null End ) / :BASE_AMT BASE_EXEC_AMT_TOT_BF2,
		Sum( Case When InStr(c.GB2,b.COMP_CODE) > 0 And b.WORK_YM = :CLSE_ACC_ID_NW ||:MM  And a.MNG_CODE = '매출액' Then b.PLAN_AMT Else Null End ) / :BASE_AMT BASE_PLAN_AMT_TOT2,
		Sum( Case When InStr(c.GB2,b.COMP_CODE) > 0 And b.WORK_YM = :CLSE_ACC_ID_NW ||:MM  And a.MNG_CODE = '매출액' Then b.EXEC_AMT Else Null End ) / :BASE_AMT BASE_EXEC_AMT_TOT2,
		Sum( Case When InStr(c.GB3,b.COMP_CODE) > 0 And b.WORK_YM = :CLSE_ACC_ID_BF ||:MM  Then b.EXEC_AMT Else Null End ) / :BASE_AMT EXEC_AMT_TOT_BF3,
		Sum( Case When InStr(c.GB3,b.COMP_CODE) > 0 And b.WORK_YM = :CLSE_ACC_ID_NW ||:MM  Then b.EXEC_AMT Else Null End ) / :BASE_AMT EXEC_AMT_TOT3,
		Sum( Case When InStr(c.GB3,b.COMP_CODE) > 0 And b.WORK_YM = :CLSE_ACC_ID_NW ||:MM  Then b.PLAN_AMT Else Null End ) / :BASE_AMT PLAN_AMT_TOT3,
		Sum( Case When InStr(c.GB3,b.COMP_CODE) > 0 And b.WORK_YM = :CLSE_ACC_ID_BF ||:MM  And a.MNG_CODE = '매출액' Then b.EXEC_AMT Else Null End ) / :BASE_AMT BASE_EXEC_AMT_TOT_BF3,
		Sum( Case When InStr(c.GB3,b.COMP_CODE) > 0 And b.WORK_YM = :CLSE_ACC_ID_NW ||:MM  And a.MNG_CODE = '매출액' Then b.PLAN_AMT Else Null End ) / :BASE_AMT BASE_PLAN_AMT_TOT3,
		Sum( Case When InStr(c.GB3,b.COMP_CODE) > 0 And b.WORK_YM = :CLSE_ACC_ID_NW ||:MM  And a.MNG_CODE = '매출액' Then b.EXEC_AMT Else Null End ) / :BASE_AMT BASE_EXEC_AMT_TOT3
	From
		(
			Select
				a.BIZ_PLAN_ITEM_NO,
