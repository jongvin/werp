			...
			try
			{
				...
				if (f != null) {
					...
					for(int i=0;i<nRows;i++)
					{
						...
						//cell = arrCell[j];
						//arrCell[0].getContents() //거래일자
						//arrCell[1].getContents() //상품코드
						//arrCell[2].getContents() //상품명
						//arrCell[3].getContents() //단위
						//arrCell[4].getContents() //수량
						//arrCell[5].getContents() //단가
						//arrCell[6].getContents() //공급가
						//arrCell[7].getContents() //세액
						//arrCell[8].getContents() //비고
						...
						lrArgData.addRow();
						lrArgData.setValue("TAX_GENDATE", 	arrCell[0].getContents().trim());
						lrArgData.setValue("ITEM_CODE", 		arrCell[1].getContents().trim());
						lrArgData.setValue("ITEM_NAME", 		arrCell[2].getContents().trim());
						lrArgData.setValue("ITEM_UNIT", 		arrCell[3].getContents().trim());
						lrArgData.setValue("ITEM_NUM", 			arrCell[4].getContents().trim());
						lrArgData.setValue("ITEM_DANGA", 		arrCell[5].getContents().trim());						
						lrArgData.setValue("TAX_SUPPRICE", 	arrCell[6].getContents().trim());
						lrArgData.setValue("TAX_TAXPRICE", 	arrCell[7].getContents().trim());
						lrArgData.setValue("ITEM_BIGO", 		arrCell[8].getContents().trim());
