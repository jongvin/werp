...
function OnLoadBefore(dataset)
{
	if(dataset == dsMAIN)
	{
		dataset.DataID = sSelectPageName	+ D_P1("ACT","MAIN")
											...
											+ D_P2("STATUS",C_Trim(cboSTATUS.value))
											+ D_P2("GEN_TM",C_Trim(chkINS.value))
											+ D_P2("DLV_TM",C_Trim(chkACK.value));
	}
}
...
{
	if ( dataset.countrow == 0 )  return;
	if ( dataset.NameString(row ,"GEN_TM" ) == "" ) return;
	// 프로그래머 유효성검사 추가
	// 보고서 디렉토리
...
function	btnSUP_REGNUM_onClick()
{
	...
	lrRet = C_LOV("TB_WT_COMPANY_PLANT", lrArgs,"F");
...
function	btnSUP_REGNUM_onblur()
{
	...
	lrRet = C_AutoLov(dsLOV,"TB_WT_COMPANY_PLANT", lrArgs,"T");
