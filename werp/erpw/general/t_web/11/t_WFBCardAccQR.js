...
function OnLoadBefore(dataset)
{
	if(dataset == dsMAIN)
	{
		dataset.DataID = sSelectPageName + D_P1("ACT","MAIN")
										 ...
										 + D_P2("CARDNUMBER",txtCARD_CODE.value);
	}
...
function btnCARD_CODE_onClick()
{
	...
	lrArgs.set("CARDNO", "");
	lrArgs.set("CARD_OWNER", "");

	lrRet = C_LOV("T_ACC_CREDCARD3", lrArgs,"F");
	
	if (lrRet == null) return;

	txtCARD_CODE.value	= lrRet.get("CARDNO");
	txtCARD_NAME.value	= lrRet.get("CARD_OWNER");
}
function txtCARD_CODE_onblur()
{
	...
	lrArgs.set("CARD_CLS", "1");
	lrArgs.set("BANK_MAIN_CODE", "");
	lrArgs.set("CARDNO", txtCARD_CODE.value);
	lrArgs.set("CARD_OWNER", "");

	lrRet = C_AutoLov(dsLOV,"T_ACC_CREDCARD3", lrArgs,"T");
