/**************************************************************************/
/* 1. 프 로 그 램 id : t_WBudgResultAmtR.js(예산실적조회)
/* 2. 유형(시나리오) : 스크립트소스
/* 3. 기  능  정  의 : 예산실적조회
...
function makeGridTitle(grid)
{
	...
	if(vDeptFlag !="ALL")
	{
		gridTitle += "<FC> Name="+vDeptFlagName+" ID=DEPT_TITLE Sort=true Align=Left HeadAlign=Center     Width=150 BgColor={Decode(P_BUDG_CODE_NO,'-1', '#E0F4FF', Decode(ACC_TITLE, '소계' , '#FFFCE0')) } suppress=2";
		gridTitle += "</FC>  ";
	}
	
	gridTitle += "<FC> Name=분류계정  ID=CLS_ACC_TITLE Sort=true Align=Left HeadAlign=Center    Width=100   suppress=1 BgColor={Decode(P_BUDG_CODE_NO,'-1', '#E0F4FF', Decode(ACC_TITLE, '소계' , '#FFFCE0')) } </FC>          ";  //#E0F4FF #FFFCE0
	if(vAccFlag !="CLS_ACC")
	{                                                                                                                         
		gridTitle += "<FC> Name=계정명칭 ID=ACC_TITLE Sort=true Align=Left HeadAlign=Center    Width=180 BgColor={Decode(P_BUDG_CODE_NO,'-1', '#E0F4FF', Decode(ACC_TITLE, '소계' , '#FFFCE0')) }               ";
		gridTitle += "</FC>";
	}                                                                                                                           
	gridTitle += "	<C> Name="+vBudgFlagName+" ID=budg_month_req_amt Align=Right HeadAlign=Center  Width=100 BgColor={Decode(P_BUDG_CODE_NO,'-1', '#E0F4FF', Decode(ACC_TITLE, '소계' , '#FFFCE0')) }</C>   ";
	gridTitle += "	<C> Name=배정액             ID=budg_month_assign_amt Align=Right HeadAlign=Center  Width=100 BgColor={Decode(P_BUDG_CODE_NO,'-1', '#E0F4FF', Decode(ACC_TITLE, '소계' , '#FFFCE0')) }</C>   ";
	gridTitle += "	<C> Name=추가신청액       ID=chg_amt Align=Right HeadAlign=Center  Width=100 BgColor={Decode(P_BUDG_CODE_NO,'-1', '#E0F4FF', Decode(ACC_TITLE, '소계' , '#FFFCE0')) }</C>     ";
	gridTitle += "	<C> Name=확정실적액       ID=conf_sil_amt Align=Right HeadAlign=Center  Width=100 BgColor={Decode(P_BUDG_CODE_NO,'-1', '#E0F4FF', Decode(ACC_TITLE, '소계' , '#FFFCE0')) }</C> ";
	gridTitle += "	<C> Name=미확정실적액    ID=notconf_sil_amt Align=Right HeadAlign=Center  Width=100 BgColor={Decode(P_BUDG_CODE_NO,'-1', '#E0F4FF', Decode(ACC_TITLE, '소계' , '#FFFCE0')) }</C>    ";
	gridTitle += "	<C> Name=잔액   		    ID=JAN Align=Right HeadAlign=Center  Width=100 BgColor={Decode(P_BUDG_CODE_NO,'-1', '#E0F4FF', Decode(ACC_TITLE, '소계' , '#FFFCE0')) }</C>   ";

	grid.Format = gridTitle;
}

function checkDtFromTo()
{
	var vBudgYyyyMmFr    = txtBUDG_YYYY_MM_FR.value;
   	var vBudgYyyyMmTo   = txtBUDG_YYYY_MM_TO.value;
   	
   	vBudgYyyyFr = vBudgYyyyMmFr.substring(0.4);
   	vBudgYyyyTo = vBudgYyyyMmTo.substring(0.4);
...
//------------------------------------------------------------------------
{
	...
	var vCompCode  = txtCOMP_CODE.value;
  	...
  	var arg_temp;
   	...
   	var reportFile ="";
   	if (vDeptFlag =='ALL' && vAccFlag=='CLS_ACC')  
   	{
   		reportFile ="r_t_080051";  //전체 , 분류계정
   	} 
   	else if (vDeptFlag =='ALL' && vAccFlag=='BUDG_ACC')  
   	{
   		reportFile ="r_t_080052";  //전체 , 예산계정
   	}
   	else if (vDeptFlag =='CHK_DEPT' && vAccFlag=='CLS_ACC')
   	{
   		reportFile ="r_t_080053"; //통제부서별, 분류계정
   	}
   	else if (vDeptFlag =='DEPT' && vAccFlag=='BUDG_ACC') 
   	{
   		reportFile ="r_t_080054"; //부서별, 예산계정
   	} 
   	else if (vDeptFlag =='CHK_DEPT' && vAccFlag=='BUDG_ACC')  
   	{
   		reportFile ="r_t_080055"; //통제부서별, 예산계정
   	}
   	else if (vDeptFlag =='DEPT' && vAccFlag=='CLS_ACC') 
   	{
   		reportFile ="r_t_080056"; //부서별, 분류계정
   	}
   	
   	arg_temp = arg_temp + reportFile ;   // 보고서명 
   	
   	arg_temp = arg_temp +  "&RptParams=" + vBudgYyyyMmFr + "&RptParams=" + vBudgYyyyMmTo+   
   	                      "&RptParams=" + vCompCode + "&RptParams=" + vDeptCode +  "&RptParams=" + vDeptFlag +  "&RptParams=" + vAccFlag;
