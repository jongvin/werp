...
/**************************************************************************/
	...
	//복사시작
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_PL_MA_DVD_CODE_I(?,?,?,?)}";
						
						lrArgData.addColumn("DVD_CODE", CITData.VARCHAR2);
						lrArgData.addColumn("CRTUSERNO", CITData.VARCHAR2);
						lrArgData.addColumn("DVD_NAME", CITData.VARCHAR2);
						lrArgData.addColumn("FUNC_NAME", CITData.VARCHAR2);

						lrArgData.addRow();
						lrArgData.setValue("DVD_CODE", rows[i].getString(dsMAIN.indexOfColumn("DVD_CODE")));
						lrArgData.setValue("CRTUSERNO", strUserNo);
						lrArgData.setValue("DVD_NAME", rows[i].getString(dsMAIN.indexOfColumn("DVD_NAME")));
						lrArgData.setValue("FUNC_NAME", rows[i].getString(dsMAIN.indexOfColumn("FUNC_NAME")));
					}
					else if (rows[i].getJobType() == GauceDataRow.TB_JOB_UPDATE)
					{
						strSql = "{call SP_T_PL_MA_DVD_CODE_U(?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_PL_MA_DVD_CODE_D(?)}";
			...
			catch (Exception ex)
			{
				...
				GauceInfo.response.writeException("USER", "900001", "T_PL_MA_DVD_CODE 테이블 데이타 갱신 에러" + ex.getMessage());
	...
	//복사시작
			...
			{
				if (GauceInfo == null) GauceInfo = new CITGauceInfo(request, response);
				GauceInfo.response.writeException("USER", "900001", "T_PL_MA_DVD_TAR_DEPT 테이블 데이타 갱신 에러" + ex.getMessage());
				throw new Exception(ex.getMessage());
			}
	//복사끝
	//여기에 붙여넣으세요
