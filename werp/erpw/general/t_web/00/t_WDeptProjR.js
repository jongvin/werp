...
function Initialize()
{
	...
	G_setLovCol(gridMAIN,"BUDG_APPL_DT");
...
function	btnSetInfoCost1_onClick()
{
	for(var i = 1 ; i <= dsMAIN.CountRow ; ++i)
	{
		if(dsMAIN.RowMark(i) == 1)
...
function	btnSetInfoCost2_onClick()
{
	for(var i = 1 ; i <= dsMAIN.CountRow ; ++i)
	{
		ApplyInfo2(i);
...
function	btnSetInfoHist_onClick()
{
	...
	lrRet = C_LOV("T_DEPT_BUDG_HIST", lrArgs,"T");
	
}
function	btnSetInfoHist2_onClick()
{
	if(dsMAIN.RowPosition < 1) return;
	var lrArgs = new C_Dictionary();
	var lrRet = null;
	...
	lrRet = C_LOV("T_DEPT_COST_HIST", lrArgs,"T");
}
function	btnSetInfo3_onClick()
{
	for(var i = 1 ; i <= dsMAIN.CountRow ; ++i)
	{
		if(dsMAIN.RowMark(i) == 1)
