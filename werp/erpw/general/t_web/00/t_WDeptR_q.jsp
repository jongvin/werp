...
/**************************************************************************/
	...
	try
	{
		...
		if (strAct.equals("MAIN"))
		{
			...
			strSql += " AND		NVL(A.DEPT_PROJ_TAG,' ') LIKE  '%'||   ?   || '%'  \n";
		...
		else if (strAct.equals("COST_CONV_CODE"))
		{
			
			strSql  = "Select \n";
			strSql += "		a.COST_CONV_CODE CODE, \n";
			strSql += "		a.COST_CONV_NAME NAME \n";
			strSql += "From	T_SET_COST_CONV_CODE a \n";
			strSql += " union all select '',' ' from dual  \n";
			strSql += "Order By 1 ";
			...
			{
				if (GauceInfo == null) GauceInfo = new CITGauceInfo(request, response);
				GauceInfo.response.writeException("USER", "900001","COST_CONV_CODE Select 오류-> "+ ex.getMessage());
			}

		}
		else if (strAct.equals("DEPT_KIND_TAG"))
		{
			
			strSql  = "Select \n";
			strSql += "		a.DEPT_KIND_TAG CODE, \n";
			strSql += "		a.DEPT_KIND_NAME NAME \n";
			strSql += "From	T_DEPT_LEVEL a \n";
			strSql += " union all select '',' ' from dual  \n";
			strSql += "Order By 1 ";
			...
			{
				if (GauceInfo == null) GauceInfo = new CITGauceInfo(request, response);
				GauceInfo.response.writeException("USER", "900001","DEPT_KIND_TAG Select 오류-> "+ ex.getMessage());
			}
