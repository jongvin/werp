		...
		if (CITCommon.isNull(strAct))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_PL_PLAN_DEPT_I(?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_PL_PLAN_DEPT_U(?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_PL_PLAN_DEPT_D(?,?,?)}";
			...
			catch (Exception ex)
			{
				...
				GauceInfo.response.writeException("USER", "900001", "T_PL_PLAN_DEPT 테이블 데이타 갱신 에러" + ex.getMessage());
