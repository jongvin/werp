Create Or Replace Procedure SP_T_MAKE_DFLT_REFUND_SLIP
(
...
Begin
	...
	End If;
	If lrRec.ITR_TAG = '5' Then		--선취 원금상환
		lnAmt := Nvl(lrRec.REFUND_AMT,0) + Nvl(lrRec.INTR_AMT,0) ;		--선취의 경우 이자금액을 계산하지 않는다.
	ElsIf lrRec.ITR_TAG = '6' Then	--후취 원금상환
		lnAmt := Nvl(lrRec.REFUND_AMT,0) + Nvl(lrRec.INTR_AMT,0);		--후취는 이자 포함한다
	Else
