CREATE OR REPLACE PROCEDURE Sp_T_Acc_Slip_Body_I
(
	AR_SLIP_ID                                             NUMBER,
...
IS
/**************************************************************************/
/* 1. 프 로 그 램 id : SP_T_ACC_SLIP_BODY_I
/* 2. 유형(시나리오) : Procedure
/* 3. 기  능  정  의 : T_ACC_SLIP_BODY 테이블 Insert
...
LN_RESET_SLIP_ID                                       NUMBER := AR_RESET_SLIP_ID;
LN_RESET_SLIP_IDSEQ                                    NUMBER := AR_RESET_SLIP_IDSEQ;
BEGIN
	BEGIN
		SELECT
			*
	...
	IF NVL(lrT_ACC_CODE.ACC_REMAIN_MNG,'F') = 'T' THEN
		...
		IF (NVL(lrT_ACC_CODE.ACC_REMAIN_POSITION,'D') = 'D' AND NVL(Ar_DB_AMT, 0) > 0) OR
			(NVL(lrT_ACC_CODE.ACC_REMAIN_POSITION,'D') = 'C' AND NVL(Ar_CR_AMT, 0) > 0) THEN
			--잔액관리 설정
			IF ( NVL(LN_RESET_SLIP_ID, 0)=0 OR NVL(LN_RESET_SLIP_IDSEQ, 0)=0 ) THEN
				LN_RESET_SLIP_ID := Ar_SLIP_ID;
				LN_RESET_SLIP_IDSEQ := Ar_SLIP_IDSEQ;
			END IF;
			IF (
				NVL(LN_RESET_SLIP_ID, 0)<>NVL(Ar_SLIP_ID, 0)
				OR
				NVL(LN_RESET_SLIP_IDSEQ, 0)<>NVL(Ar_SLIP_IDSEQ, 0)
			) THEN
				LN_RESET_SLIP_ID := Ar_SLIP_ID;
				LN_RESET_SLIP_IDSEQ := Ar_SLIP_IDSEQ;
			END IF;
	...
	(
		...
		SYSDATE,
      	...
      	AR_BANK_CODE ,
      	AR_ACCNO ,
      	DECODE(LN_RESET_SLIP_ID, 0, NULL, LN_RESET_SLIP_ID) ,
      	DECODE(LN_RESET_SLIP_IDSEQ, 0, NULL, LN_RESET_SLIP_IDSEQ) ,
      	AR_SUMMARY1 ,
      	AR_SUMMARY2 ,
	AR_BRAIN_GRP_SEQ,
	AR_BRAIN_SLIP_SEQ1,
	...
	DECODE(AR_CARD_SEQ_B,0,NULL,AR_CARD_SEQ_B),
      	AR_CHK_NO ,
      	AR_BILL_NO ,
      	AR_REC_BILL_NO ,
      	AR_CP_NO ,
	...
	AR_FIX_ASSET_SEQ,
      	...
      	AR_PAY_CON_CASH ,
      	AR_PAY_CON_BILL ,
      	F_T_Stringtodate(AR_PAY_CON_BILL_DT) ,
      	AR_PAY_CON_BILL_DAYS,
      	AR_EMP_NO ,
      	F_T_Stringtodate(AR_ANTICIPATION_DT) ,
      	AR_MNG_ITEM_CHAR1 ,
      	AR_MNG_ITEM_CHAR2 ,
      	AR_MNG_ITEM_CHAR3 ,
      	AR_MNG_ITEM_CHAR4 ,
      	...
      	AR_MNG_ITEM_NUM3 ,
      	AR_MNG_ITEM_NUM4 ,
      	F_T_Stringtodate(AR_MNG_ITEM_DT1) ,
      	F_T_Stringtodate(AR_MNG_ITEM_DT2) ,
      	F_T_Stringtodate(AR_MNG_ITEM_DT3) ,
      	F_T_Stringtodate(AR_MNG_ITEM_DT4)
	);
