Create Or Replace Procedure SP_T_FIN_MAKE_SECU_SALE_SLIP
(
...
Begin
	...
	If PKG_MAKE_SLIP.IsCashAccCode(lrOther.ACC_CODE) Then
		lrHead := PKG_MAKE_SLIP.New_Head(Ar_Make_Comp_Code,Ar_Dept_Code,lrRec.SALE_DT,'2','B',Ar_CrtUserNo,Ar_Work_Code,lsChargePers,Ar_CrtUserNo);
	Else
		lrHead := PKG_MAKE_SLIP.New_Head(Ar_Make_Comp_Code,Ar_Dept_Code,lrRec.SALE_DT,'1','B',Ar_CrtUserNo,Ar_Work_Code,lsChargePers,Ar_CrtUserNo);
	End If;
	...
	End;
	--유가증권 처분 손 또는 익의 처리
	If Nvl(lrRec.SALE_AMT,0) - Nvl(lrRec.INCOME_AMT,0) > 0 Then
		--처분익
		Begin
			Select
				a.CODE_LIST_ID
			...
			Where	a.CODE_GROUP_ID = 'SECU_SALE_PROF_ACC'
			And		RowNum < 2;
		Exception When No_Data_Found Then
			Raise_Application_Error(-20009,'공통코드에 매도가능증권 처분익 계정이 등록되어 있지 않습니다.(SECU_SALE_PROF_ACC).');
		End;
		lnMakeSlipLine := lnMakeSlipLine + 1;
		lsClassCode := PKG_MAKE_SLIP.Get_Class_Code(Ar_Dept_Code);
		lrBody1 := PKG_MAKE_SLIP.New_Body(lrHead,lnMakeSlipLine,lsAccCode,lsClassCode,Null);
		lrBody1.CR_AMT := Nvl(lrRec.SALE_AMT,0) - Nvl(lrRec.INCOME_AMT,0);
		lrBody1.TAX_COMP_CODE := Ar_Make_Comp_Code;
		lrBody1.COMP_CODE := Ar_Make_Comp_Code;
		lrBody1.DEPT_CODE := Ar_Dept_Code;
		lrBody1.SUMMARY1 := '유가 증권 처분시 처분익';
		lrBody1.CUST_SEQ := lnCustSeq;
		lrBody1.CUST_NAME := lsCustName;
		lrBody1.SECU_NO := Ar_Secu_No;
		PKG_MAKE_SLIP.Insert_Body(lrBody1);
		Update	T_FIN_SECURTY
		Set		SALE_LOSS = - lrBody1.CR_AMT
		Where	SECU_NO = Ar_Secu_No;
	ElsIf Nvl(lrRec.SALE_AMT,0) - Nvl(lrRec.INCOME_AMT,0) < 0 Then
		--처분손
		Begin
			Select
				a.CODE_LIST_ID
			Into
				lsAccCode
			From	V_T_CODE_LIST a
			Where	a.CODE_GROUP_ID = 'SECU_SALE_LOSS_ACC'
			And		RowNum < 2;
		Exception When No_Data_Found Then
			Raise_Application_Error(-20009,'공통코드에 매도가능증권 처분손 계정이 등록되어 있지 않습니다.(SECU_SALE_LOSS_ACC).');
		End;
		lnMakeSlipLine := lnMakeSlipLine + 1;
		lsClassCode := PKG_MAKE_SLIP.Get_Class_Code(Ar_Dept_Code);
		lrBody1 := PKG_MAKE_SLIP.New_Body(lrHead,lnMakeSlipLine,lsAccCode,lsClassCode,Null);
		lrBody1.DB_AMT := Nvl(lrRec.INCOME_AMT,0) - Nvl(lrRec.SALE_AMT,0);
		lrBody1.TAX_COMP_CODE := Ar_Make_Comp_Code;
		lrBody1.COMP_CODE := Ar_Make_Comp_Code;
		lrBody1.DEPT_CODE := Ar_Dept_Code;
		lrBody1.SUMMARY1 := '유가 증권 처분시 처분손';
		lrBody1.CUST_SEQ := lnCustSeq;
		lrBody1.CUST_NAME := lsCustName;
		lrBody1.SECU_NO := Ar_Secu_No;
		PKG_MAKE_SLIP.Insert_Body(lrBody1);
		Update	T_FIN_SECURTY
		Set		SALE_LOSS = lrBody1.DB_AMT
		Where	SECU_NO = Ar_Secu_No;
	End If;
	--만약 미수이자가 있다면
	If lnItrAmt <> 0 Then
		...
		lrBody1.COMP_CODE := Ar_Make_Comp_Code;
		lrBody1.DEPT_CODE := Ar_Dept_Code;
		lrBody1.SUMMARY1 := '유가 증권 처분시 미수이자';
		lrBody1.CUST_SEQ := lnCustSeq;
		lrBody1.CUST_NAME := lsCustName;
		lrBody1.SECU_NO := Ar_Secu_No;
		PKG_MAKE_SLIP.Insert_Body(lrBody1);
	...
	If Nvl(lrRec.SALE_ITR_AMT,0) - lnItrAmt <> 0 Then
		...
		lrBody1.COMP_CODE := Ar_Make_Comp_Code;
		lrBody1.DEPT_CODE := Ar_Dept_Code;
		lrBody1.SUMMARY1 := '유가 증권 처분시 이자수익';
		lrBody1.CUST_SEQ := lnCustSeq;
		lrBody1.CUST_NAME := lsCustName;
		lrBody1.SECU_NO := Ar_Secu_No;
		PKG_MAKE_SLIP.Insert_Body(lrBody1);
	...
	If Nvl(lrRec.SALE_TAX,0) <> 0 Then
		...
		lrBody1.COMP_CODE := Ar_Make_Comp_Code;
		lrBody1.DEPT_CODE := Ar_Dept_Code;
		lrBody1.SUMMARY1 := '유가 증권 처분시 이자 법인세';
		lrBody1.CUST_SEQ := lnCustSeq;
		lrBody1.CUST_NAME := lsCustName;
		lrBody1.SECU_NO := Ar_Secu_No;
	...
	lrBody2.DB_AMT := Nvl(lrRec.SALE_AMT,0) + Nvl(lrRec.SALE_ITR_AMT,0) - Nvl(lrRec.SALE_TAX,0);
