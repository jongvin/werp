Create Or Replace Procedure SP_T_FIN_PAY_EXEC_LIST_N_U
(
...
Begin
	If Nvl(AR_EXEC_AMT,0) = 0 Then
		return;
	End If;
	If AR_OUT_ACC_NO Is Null Then
		Raise_Application_Error(-20009,'당사 인출계좌를 선택하여야 합니다.');
	End If;
	If AR_IN_BANK_MAIN_CODE Is Null Then
		Raise_Application_Error(-20009,'입금은행을 선택하여야 합니다.');
	End If;
	If AR_IN_ACC_NO Is Null Then
		Raise_Application_Error(-20009,'입금계좌 선택하여야 합니다.');
	End If;
	If AR_ACCNO_OWNER Is Null Then
		Raise_Application_Error(-20009,'예금주명을 입력하여야 합니다.');
	End If;
