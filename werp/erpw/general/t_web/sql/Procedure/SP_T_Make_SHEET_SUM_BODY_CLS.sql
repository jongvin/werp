Create Or Replace Procedure SP_T_Make_SHEET_SUM_BODY_CLS
(
...
-- 이 프로그램은 재무제표 생성 프로그램입니다.
	...
	Type	t_ArraySEQ_TYPE_NAME Is Table Of T_SHEET_SEQ_TYPE_NAME.SEQ_TYPE_NAME%Type
		Index By Binary_Integer;
	Type	t_ArrayITEM_CODE Is Table Of T_SHEET_SUM_BODY_CLASS.ITEM_CODE%Type
		Index By Binary_Integer;
	Type	t_ArrayITEM_NAME Is Table Of T_SHEET_SUM_BODY_CLASS.ITEM_NAME%Type
		Index By Binary_Integer;
	Type	t_ArrayITEM_ENG_NAME Is Table Of T_SHEET_SUM_BODY_CLASS.ITEM_ENG_NAME%Type
		Index By Binary_Integer;
	Type	t_ArrayITEM_LVL Is Table Of T_SHEET_EXPR_HEAD.ITEM_LVL%Type
		Index By Binary_Integer;
	Type	t_ArrayOUT_CLS Is Table Of T_SHEET_SUM_BODY_CLASS.OUT_CLS%Type
		Index By Binary_Integer;
	lrT_SHEET_SUM_HEAD_CLASS		T_SHEET_SUM_HEAD_CLASS%RowType;
...
Begin
	...
	Select
		*
	Into
		lrT_SHEET_SUM_HEAD_CLASS
	From	T_SHEET_SUM_HEAD_CLASS
	Where	Sheet_Code = Ar_Sheet_Code
	And		COMP_CODE = Ar_COMP_CODE
	And		CLASS_CODE = Ar_CLASS_CODE;
	lsStartDt := To_Char(lrT_SHEET_SUM_HEAD_CLASS.CURR_ACC_FDT,'YYYYMMDD');
	lsEndDt := To_Char(lrT_SHEET_SUM_HEAD_CLASS.CURR_ACC_EDT,'YYYYMMDD');
	lsStartDtCarried := To_Char(lrT_SHEET_SUM_HEAD_CLASS.CURR_ACC_FDT,'YYYY')||'0100';
	lsStartDtPre := To_Char(lrT_SHEET_SUM_HEAD_CLASS.PAST_ACC_FDT,'YYYYMMDD');
	lsEndDtPre := To_Char(lrT_SHEET_SUM_HEAD_CLASS.PAST_ACC_EDT,'YYYYMMDD');
	lsStartDtPreCarried := To_Char(lrT_SHEET_SUM_HEAD_CLASS.PAST_ACC_FDT,'YYYY')||'0100';
	--먼저 기존 자료 삭제합니다.
	Delete	T_SHEET_SUM_BODY_CLASS
	Where	Sheet_Code = Ar_Sheet_Code
	And		COMP_CODE = Ar_COMP_CODE
	And		CLASS_CODE = Ar_CLASS_CODE;
	--재무제표자료를 일단 만들어 놓습니다.
	Insert Into T_SHEET_SUM_BODY_CLASS
	(
		SHEET_CODE,
		COMP_CODE,
		CLASS_CODE,
	...
	Where	Sheet_Code = Ar_Sheet_Code;
	--먼저 전표에서 가져온단다.
	If Ar_COMP_CODE <> '%' And Ar_CLASS_CODE <> '%' Then	-- 특정 현장 집계
		Merge Into T_SHEET_SUM_BODY_CLASS t
		Using
		(
			Select
				x1.SHEET_CODE ,
				Ar_COMP_CODE COMP_CODE,
				Ar_CLASS_CODE CLASS_CODE,
				x1.ITEM_CODE,
				Sum(
				...
				(
					...
					From	T_SHEET_EXPR_HEAD a,
							...
							(
								...
								And		c.CLASS_CODE = Ar_CLASS_CODE
								Group By
									c.ACC_CODE
							) c,
							(
		...
		When Not Matched Then
			Insert (SHEET_CODE,COMP_CODE,CLASS_CODE,ITEM_CODE)
			Values (f.SHEET_CODE,f.COMP_CODE,f.CLASS_CODE,f.ITEM_CODE);
	ElsIf Ar_COMP_CODE <> '%' And Ar_CLASS_CODE = '%' Then	-- 특정 사업장 전체 집계
		Merge Into T_SHEET_SUM_BODY_CLASS t
		Using
		(
			Select
					...
					From	T_SHEET_EXPR_HEAD a,
							...
							(
								...
								From	T_ACC_TRANS_DAILY_SUM c,
										T_CLASS_CODE tp
								Where	c.CONF_YMD Between lsStartDtCarried And lsEndDt
								And		c.CLASS_CODE = tp.CLASS_CODE
								Group By
									c.ACC_CODE
							) c,
							(
	...
	if lnAmtUnit <> 1 Then
		If lrT_SHEET_CODE.ROUND_CLS = '1' Then		--반올림
			Update	T_SHEET_SUM_BODY_CLASS a
			...
			And		a.CLASS_CODE = Ar_CLASS_CODE;
	...
	If lrT_SHEET_CODE.ZERO_CLS = '2' Then		--당기기준
		Delete	T_SHEET_SUM_BODY_CLASS a
		Where	a.SHEET_CODE = Ar_Sheet_Code
		And		a.COMP_CODE = Ar_COMP_CODE
		And		a.CLASS_CODE = Ar_CLASS_CODE
