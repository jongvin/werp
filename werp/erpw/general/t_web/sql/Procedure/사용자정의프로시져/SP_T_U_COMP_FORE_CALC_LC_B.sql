Create Or Replace Procedure SP_T_U_COMP_FORE_CALC_LC_B
Is
...
Begin
	...
	And		RowNum < 2;
	For liI In 0..2 Loop
		ls_Work_Ym := F_T_AddMonths(lsBegin,liI);
		--기초시재
		If liI = 0 Then
			Select
				Sum(b.FORECAST_AMT)
		...
		Else
			...
			Exception When Dup_Val_On_Index Then
				...
				Set		SUM_FORECAST_AMT = ln_Last_Cash,
						MOD_FORECAST_AMT = 0,
						FORECAST_AMT = Nvl(B_MOD_FORECAST_AMT,0) + Nvl(ln_Last_Cash,0)
		...
		Exception When Dup_Val_On_Index Then
			Update	T_FL_MONTH_PLAN_EXEC_B
			Set		SUM_FORECAST_AMT = ln_HL_Amt,
					MOD_FORECAST_AMT = 0,
					FORECAST_AMT = Nvl(B_MOD_FORECAST_AMT,0) + Nvl(ln_HL_Amt,0)
			Where	COMP_CODE = ls_Comp_Code
			And		CLSE_ACC_ID = ls_Clse_Acc_Id
			And		WORK_YM = ls_Work_Ym
