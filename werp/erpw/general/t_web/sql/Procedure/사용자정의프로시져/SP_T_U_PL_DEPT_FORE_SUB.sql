Create Or Replace Procedure SP_T_U_PL_DEPT_FORE_SUB
(
...
Begin
	...
	(
		Select
			...
			Sum(
			Case b.MNG_CODE
			When Ar_Mng_Code1 Then a.SUM_FORECAST_AMT
			When Ar_Mng_Code2 Then - a.SUM_FORECAST_AMT
			Else 0
			End) SUM_FORECAST_AMT,
			Sum(
			Case b.MNG_CODE
			When Ar_Mng_Code1 Then a.MOD_FORECAST_AMT
			When Ar_Mng_Code2 Then - a.MOD_FORECAST_AMT
			Else 0
			End) MOD_FORECAST_AMT,
			Sum(
			Case b.MNG_CODE
			When Ar_Mng_Code1 Then a.FORECAST_AMT
			When Ar_Mng_Code2 Then - a.FORECAST_AMT
			Else 0
			End) FORECAST_AMT
	...
	When Matched Then
	Update
	Set		t.SUM_FORECAST_AMT = Nvl(s.SUM_FORECAST_AMT,0) + Nvl(s.MOD_FORECAST_AMT,0) ,
			t.MOD_FORECAST_AMT = 0,
			t.FORECAST_AMT = Nvl(s.SUM_FORECAST_AMT,0) + Nvl(s.MOD_FORECAST_AMT,0),
			t.MODDATE = s.CRTDATE,
	...
	Values
	(s.COMP_CODE,s.CLSE_ACC_ID,s.CHG_SEQ,s.DEPT_CODE,s.WORK_YM,s.BIZ_PLAN_ITEM_NO,s.CRTDATE,s.CRTUSERNO,Nvl(s.SUM_FORECAST_AMT,0) + Nvl(s.MOD_FORECAST_AMT,0),0,Nvl(s.SUM_FORECAST_AMT,0) + Nvl(s.MOD_FORECAST_AMT,0));
End;
/
