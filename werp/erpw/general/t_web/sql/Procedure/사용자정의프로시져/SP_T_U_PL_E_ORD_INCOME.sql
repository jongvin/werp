Create Or Replace Procedure SP_T_U_PL_E_ORD_INCOME
--경영계획 경상이익 실적 산출
Is
...
Begin
	...
	(
		Select
			...
			SysDate CRTDATE,
			Sum(Decode(b.ITEM_TAG,'7',-1,1) * Nvl(a.SUM_EXEC_AMT,0)) SUM_EXEC_AMT,
			Sum(Decode(b.ITEM_TAG,'7',-1,1) * Nvl(a.MOD_EXEC_AMT,0)) MOD_EXEC_AMT,
			Sum(Decode(b.ITEM_TAG,'7',-1,1) * Nvl(a.B_MOD_EXEC_AMT,0)) B_MOD_EXEC_AMT,
			Sum(Decode(b.ITEM_TAG,'7',-1,1) * Nvl(a.EXEC_AMT,0)) EXEC_AMT
		From	T_PL_COMP_PLAN_EXEC a,
				T_PL_ITEM b
		Where	a.BIZ_PLAN_ITEM_NO = b.BIZ_PLAN_ITEM_NO
		And		b.ITEM_TAG In ('5','6','7')		--영업이익,영업외수익,영업외비용을 가져온다.
		And		b.IS_LEAF_TAG = 'T'
