Create Or Replace Procedure SP_T_U_Dept_Fore_Get_Est_Amt
Is
...
Begin
	...
	(
		Select
			...
			a.DEPT_CODE,
			To_Char(a.PLAN_DATE,'YYYYMM') WORK_YM,
			lnFlowCode FLOW_CODE,
			lsCrtUserNo CRTUSERNO,
			SysDate CRTDATE,
			Nvl(Sum(a.CASH_AMT),0) + Nvl(Sum(a.BILL_AMT),0) SUM_FORECAST_AMT
		From	R_CONTRACT_COLLECTION_PLAN a
		Where	a.DEPT_CODE = lsDeptCode
		And		a.PLAN_DATE Between To_Date(lsBegin ||'01','YYYYMMDD')
						And Last_Day(To_Date(lsEnd ||'01','YYYYMMDD'))
		Group By
			a.DEPT_CODE,
			To_Char(a.PLAN_DATE,'YYYYMM')
	) s
	On
	(
