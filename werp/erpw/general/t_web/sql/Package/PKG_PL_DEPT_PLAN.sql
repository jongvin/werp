Create Or Replace Package PKG_PL_DEPT_PLAN
Is
...
End;
/
Create Or Replace Package Body PKG_PL_DEPT_PLAN
Is
	Type	T_FUNCTTIONS	Is Table Of T_PL_COMP_PLN_FUNCTIONS.FUNC_NAME%Type
	...
	Begin
		G_FUNCTIONS.Delete;
		For lrA In (Select COMP_PLN_FUNC_NO,FUNC_NAME From T_PL_COMP_PLN_FUNCTIONS) Loop
	...
	Is
		...
		liFirst						Number;
		liLast						Number;
		lsFuncName					T_PL_COMP_PLN_FUNCTIONS.FUNC_NAME%Type;
		lnFuncNo					Number;
	Begin
		LoadFunctions;
		SetCalcInfo(Ar_COMP_CODE,Ar_CLSE_ACC_ID,Ar_CHG_SEQ,Ar_DEPT_CODE,Ar_CRTUSERNO);
		ClearData(Ar_COMP_CODE,Ar_CLSE_ACC_ID,Ar_CHG_SEQ,Ar_DEPT_CODE);
		...
		From
			(
				...
				(
					Select
						b.COMP_PLN_FUNC_NO
					From	T_PL_COMP_PLN_FUNCTIONS b
	...
	Begin
		...
		Values
		(s.COMP_CODE,s.CLSE_ACC_ID,s.CHG_SEQ,s.DEPT_CODE,s.WORK_YM,s.BIZ_PLAN_ITEM_NO,s.CRTDATE,s.CRTUSERNO,s.SUM_PLAN_AMT,s.MOD_PLAN_AMT,s.PLAN_AMT);
	End;
End;
/
