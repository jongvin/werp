Create Or Replace Package PKG_FL_DEPT_PLAN
Is
...
End;
/
Create Or Replace Package Body PKG_FL_DEPT_PLAN
Is
	Type	T_FUNCTTIONS	Is Table Of T_FL_DEPT_PLN_FUNCTIONS.FUNC_NAME%Type
	...
	Begin
		G_FUNCTIONS.Delete;
		For lrA In (Select DEPT_PLN_FUNC_NO,FUNC_NAME From T_FL_DEPT_PLN_FUNCTIONS) Loop
			G_FUNCTIONS(lrA.DEPT_PLN_FUNC_NO) := lrA.FUNC_NAME;
		End Loop;
	End;
	...
	Is
		...
		liFirst						Number;
		liLast						Number;
		lsFuncName					T_FL_DEPT_PLN_FUNCTIONS.FUNC_NAME%Type;
		lnFuncNo					Number;
	Begin
		LoadFunctions;
		...
		From
			(
				Select
					...
					RowNum rn
				From	T_FL_FLOW_CODE a
				Where	a.DEPT_PLN_FUNC_NO In
				(
					Select
						b.DEPT_PLN_FUNC_NO
					From	T_FL_DEPT_PLN_FUNCTIONS b
	...
	Begin
		...
		Insert
		(COMP_CODE,CLSE_ACC_ID,DEPT_CODE,WORK_YM,FLOW_CODE,CRTDATE,CRTUSERNO,SUM_PLAN_AMT,MOD_PLAN_AMT,PLAN_AMT)
		Values
		(s.COMP_CODE,s.CLSE_ACC_ID,s.DEPT_CODE,s.WORK_YM,s.FLOW_CODE,s.CRTDATE,s.CRTUSERNO,s.SUM_PLAN_AMT,s.MOD_PLAN_AMT,s.PLAN_AMT);
	End;
	Procedure	CalcCurrentInOut
	Is
	Begin
		...
		When Not Matched Then
		Insert 
		(COMP_CODE,CLSE_ACC_ID,DEPT_CODE,WORK_YM,FLOW_CODE,CRTUSERNO,CRTDATE,SUM_PLAN_AMT,MOD_PLAN_AMT,PLAN_AMT)
		Values
		(s.COMP_CODE,s.CLSE_ACC_ID,s.DEPT_CODE,s.WORK_YM,s.FLOW_CODE,s.CRTUSERNO,s.CRTDATE,s.SUM_PLAN_AMT,s.MOD_PLAN_AMT,s.PLAN_AMT);
	End;
	Procedure	CalcSpecialInOut
	Is
	Begin
	...
	Begin
		...
		For liI In 1..12 Loop
			lsWorkYm := G_CLSE_ACC_ID || To_Char(liI,'FM00');
			Select
				Nvl(Sum(Case
...
*/
			...
			Exception When Dup_Val_On_Index Then
				Update	T_FL_MONTH_PLAN_EXEC
					Set	SUM_PLAN_AMT = lnInItrAmt - lnOutItrAmt,
						PLAN_AMT = Nvl(MOD_PLAN_AMT,0) + lnInItrAmt - lnOutItrAmt,
			...
			Update
			Set		SUM_PLAN_AMT = s.SUM_PLAN_AMT,
					PLAN_AMT = Nvl(s.SUM_PLAN_AMT,0) + Nvl(t.MOD_PLAN_AMT,0) ,
					MODDATE = s.CRTDATE,
					MODUSERNO = s.CRTUSERNO
			When Not Matched Then
			Insert 
			...
			(
				Select
					...
					G_CRTUSERNO CRTUSERNO,
					SysDate CRTDATE,
					lnOutItrAmt SUM_PLAN_AMT,
					0 MOD_PLAN_AMT,
					lnOutItrAmt PLAN_AMT
				From	T_FL_FLOW_CODE a
		...
		Begin
			G_CALCITRAMT := True;
			SP_T_FL_MONTH_PLAN_SUM(G_COMP_CODE,G_CLSE_ACC_ID,G_DEPT_CODE);
