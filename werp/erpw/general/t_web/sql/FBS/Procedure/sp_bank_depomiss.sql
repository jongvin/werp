    ...
    /* 13. 최종수정일:                                                       */
    /*************************************************************************/  
    Create Or Replace Procedure sp_bank_depomiss(p_param IN VARCHAR2 DEFAULT NULL ) IS
        ...
        v_max_docu_numc            NUMBER;
		    ...
		    FB_DOCU_DEPO_TR_B   VARCHAR2(7) := FBS_MAIN_PKG.FB_DOCU_DEPO_TR_B;   /*  예금거래명세통지(은행=>업체)  */
		    FB_DOCU_DEPO_MISS_T VARCHAR2(7) := FBS_MAIN_PKG.FB_DOCU_DEPO_MISS_T; /*  예금거래결번    (업체=>은행)  */
		    FB_DOCU_DEPO_MISS_B VARCHAR2(7) := FBS_MAIN_PKG.FB_DOCU_DEPO_MISS_B; /*  예금거래결번    (은행=>업체)  */
        	                            
    BEGIN
        /***************************************************************/         
        /*  예금거래명세 결번을 확인하여, 자동으로 결번요청            */
        /***************************************************************/         
         ...
         IF v_hour >= 9 AND v_hour <= 21 THEN
            ...
            FOR rec_gubun IN ( SELECT DISTINCT TRAN_CODE, ENTE_CODE ,BANK_CODE
                               ...
                               AND    UPMU_CODE = SUBSTRB(FB_DOCU_DEPO_TR_B,5,3) ) LOOP
                ...
                FOR index_num IN 1..v_max_docu_numc LOOP 
                    ...
                    IF v_chk = 0 THEN   
                        ...
                        AND    ENTE_CODE = rec_gubun.ENTE_CODE
                        AND    BANK_CODE = rec_gubun.BANK_CODE
                        AND    DOCU_CODE = SUBSTRB(FB_DOCU_DEPO_MISS_B,1,4) 
                        ...
                        IF v_chk = 0 THEN
                            ...
                            -- 결번요청 전문을 송신합니다.
														sp_depo_miss_req( v_company_cd	 ,	                      -- 사업장코드
