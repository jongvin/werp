...
function G_DateCol(agrid,acolid,adatetag)
{
	...
	{
		C_msgOk("G_DateCol 에러 : " + e.message, "에러");
	}
}
...
function	G_searchDateCol(grid,colid)
{
	...
	try
	{
		...
		{
			C_msgOk("G_searchDateCol 에러 : 그리드 인자가 null입니다.", "에러");
			return null;
		}
		if (G_arrDateCols == null) G_arrDateCols = new Array();
	...
	{
		C_msgOk("G_searchDateCol 에러 : " + e.message, "에러");
	}
...
function G_DataSetInfo(aDataset, aTrans, aGrid, aServiceURL, aDataSetTitle, aNotNullColName, aSort, aClearChild , aControlNoDataMessage)
{
	try
	{
		// DataSet
		...
		this.ControlNoDataMessage = aControlNoDataMessage == null ? false : aControlNoDataMessage;
	}
	catch (e)
	{
...
function G_OnLoadCompleted(dataset, rowcnt)
{
	...
	try
	{
		...
		/*
		if (rowcnt < 1 && grid != null && datasetinfo.masterDataSet == null && ! datasetinfo.ControlNoDataMessage)	//관련 그리드가 있고 부모가 없고 자료없음 flag가 꺼져 있으면
		{
...
function G_OnExit(dataset, grid, row, colid, olddata)
{
	try
	{
		...
		if(objdateinfo != null)
		{
			...
			{
				C_msgOk("G_OnExit 에러 : 알 수 없는 날짜 형식입니다.", "에러");
			}
		}
	}
	catch (e)
	{
		C_msgOk("G_OnExit 에러 : " + e.message, "에러");
	}
...
function G_OnPopup(dataset, grid, row, colid, data)
{
	...
	{
		C_msgOk("G_OnPopup 에러 : " + e.message, "에러");
	}
...
// DataSet 등록
function G_addDataSet(aDataset, aTrans, aGrid, aServiceURL, aDataSetTitle, aNotNullColName, aSort, aClearChild , aControlNoDataMessage)
{
	if (aDataset == null)
	{
	...
	try
	{
		var objDataSetInfo = new G_DataSetInfo(aDataset, aTrans, aGrid, aServiceURL, aDataSetTitle, aNotNullColName, aSort, aClearChild , aControlNoDataMessage);
...
function G_setLovCol(aGrid, aColName, isBgColor)
{
	...
	try
	{
		...
		aGrid.RejectEnterkeyOnPopupStyle = true;		//그리드에서 enter 키 입력시 popup을 뜨지 않게한다.
	}
	catch (e)
	{
...
function	btndup_onclick()
{
	...
	{
		var			grid = C_EventObject;
		if(grid.DataID == undefined || grid.DataID == null) return;
		var			dataset = G_searchDataSet(grid.DataID);
...
function	G_autoLovColInfo(aLovName,aGrid, aDataset , aColName, aInputArgs , aReturnValues)
{
	...
	{
		C_msgOk("G_autoLovColInfo 에러 : " + e.message, "에러");
	}
}
//자동 LOV 설정
//aLovName : LOV명
//aGrid : 그리드
//aDataset : 자동 검색에 사용될 데이타셑 ex> dsLOV
//aColName : LOV를 표시할 컬럼명
//aInputArgs : LOV로 넘겨줄 입력 인자 설정 ex> "ACC_CODE:SEARCH_CONDITION,COMP_CODE:COMP_CODE" => "컬럼명1:LOV인자명1,컬럼명2:LOV인자명2....."
//aReturnValues : LOV에서 넘겨받을 데이타 설정 ex> "ITR_ACC_CODE:ACC_CODE,ITR_ACC_NAME:ACC_NAME" => "컬럼명1:LOV컬럼명1,컬럼명2:LOV컬럼명2....."
//isBgColor : LOV 백그라운드 컬러
...
function	G_searchAutoLovCol(grid,colid)
{
	...
	try
	{
		if (grid == null || C_isNull(grid))
		{
			C_msgOk("G_searchAutoLovCol 에러 : 그리드 인자가 null입니다.", "에러");
			return null;
		}
		if (G_arrAutoLovs == null) G_arrAutoLovs = new Array();
...
function	G_processLov(dataset, grid, row, colid, data)
{
	...
	{
		C_msgOk("G_processLov 에러 : " + e.message, "에러");
	}
}
...
function	G_raiseEvent_OnBeforeLov(dataset,grid,row,colid,olddata)
{
	...
	catch (e)
	{
		...
		{
			C_msgOk("OnBeforeGridLov 이벤트의 개발자 정의 함수 Call 에러 : " + e.message, "에러");
			return false
		}
	}
...
function	G_raiseEvent_OnSuccessLov(dataset,grid,row,colid,olddata)
{
	...
	catch (e)
	{
		...
		{
			C_msgOk("OnSuccessGridLov 이벤트의 개발자 정의 함수 Call 에러 : " + e.message, "에러");
		}
	}
}
function	G_raiseEvent_OnCancelLov(dataset,grid,row,colid,olddata)
{
	...
	catch (e)
	{
		...
		{
			C_msgOk("OnCancelGridLov 이벤트의 개발자 정의 함수 Call 에러 : " + e.message, "에러");
		}
	}
}
function	G_defaultOnDuplicateColumn(dataset,grid,row,colid)
{
	...
	{
		C_msgOk("G_defaultOnDuplicateColumn 에러 : " + e.message, "에러");
	}
}
