		...
		if(strAct.equals("MAKE_DEFAULT"))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_MAKE_DFLT_EMP_PAY_SLIP(?,?,?,?,?,?,?,?)}";
						...
						lrArgData.setValue("WORK_DT", rows[i].getString(dsWORK_SLIP_IDSEQ.indexOfColumn("WORK_DT")));
						lrArgData.setValue("WORK_SEQ", rows[i].getString(dsWORK_SLIP_IDSEQ.indexOfColumn("WORK_SEQ")));
		...
		if(strAct.equals("MAKE_SLIP"))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					if (rows[i].getJobType() == GauceDataRow.TB_JOB_UPDATE)
					{
						strSql = "{call SP_T_FIN_MAKE_EMP_PAY_SLIP(?,?,?,?,?,?,?,?)}";
						...
						lrArgData.setValue("WORK_SEQ", rows[i].getString(dsMAKE_SLIP.indexOfColumn("WORK_SEQ")));
						lrArgData.setValue("CRTUSERNO", strUserNo);
					}
					else
					{
