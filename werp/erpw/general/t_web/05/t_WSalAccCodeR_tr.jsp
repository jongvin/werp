...
/**************************************************************************/
	...
	//복사시작
			...
			catch (Exception ex)
			{
				...
				GauceInfo.response.writeException("USER", "900001", "T_FIN_SAL_ITEM 테이블 데이타 갱신 에러" + ex.getMessage());
	...
	//복사시작
			...
			{
				if (GauceInfo == null) GauceInfo = new CITGauceInfo(request, response);
				GauceInfo.response.writeException("USER", "900001", "T_FIN_SAL_ACC_CODE 테이블 데이타 갱신 에러" + ex.getMessage());
				throw new Exception(ex.getMessage());
			}
	//복사끝
		}
