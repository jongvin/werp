		...
		if (CITCommon.isNull(strAct))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_FIN_PAY_EXEC_LIST_N2_U(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
						...
						lrArgData.setValue("CHK_BILL_NO", "");
						lrArgData.setValue("ACCNO_OWNER", "");
						lrArgData.setValue("EXPR_DT", "");
					}
					else
					{
						continue;
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					strSql = "{call SP_T_MAKE_EMP_TAX_SLIP(?,?,?,?,?,?,?,?,?,?,?,?,?)}";
