		...
		public void run()
		{
			try
			{
				...
				try
				{
					...
					strSql  = " Select SCC_REPORT_REQUEST_NO.nextval as REPORT_REQUEST_NO ";
					strSql += " From   DUAL ";
				...
				{
					throw new Exception("SCC_REPORT_REQUEST_NO Sequence Select 오류 : " + ex.getMessage());
				}
				...
				if (this.RunTag.equals("2"))
				{
					try
					{
						...
						strSql = "{call SPCC_USER_REPORT_REQUEST_I(?,?,?,?,?,?,?)}";
				...
				if (this.RunTag.equals("2"))
				{
					try
					{
						...
						strSql = "{call SPCC_USER_REPORT_REQUEST_U(?,?,?,?,?)}";
...
<%
	...
	try
	{
		...
		//strUserNo = session.getAttribute("user_no") == null ? "0" : session.getAttribute("user_no").toString();
		//strUserId = session.getAttribute("user_id") == null ? "테스트ID" : session.getAttribute("user_id").toString();
		strUserName = session.getAttribute("user_nm") == null ? "테스트유저" : CITCommon.URLEncoding(session.getAttribute("user_nm").toString());
		...
		strReportRequestUrl = "http://52.10.123.200/WIIS_REPORT/ServiceReport.aspx";
		strReportResponseUrl ="http://52.10.123.200/WIIS_REPORT/Reports_Temp";
		...
		{
			if (CITCommon.isNull(strReportNo)) throw new Exception("보고서 번호가 없습니다");
...
<html>
	...
	<body onload="C_Initialize()">
		<form name="frmList" action="WCcReportRequestProcess.jsp" target="_self" method="post">
