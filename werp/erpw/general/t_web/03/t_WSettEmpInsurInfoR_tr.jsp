	...
	//복사시작
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_SET_EMP_INSUR_INFO_I(?,?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_SET_EMP_INSUR_INFO_U(?,?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_SET_EMP_INSUR_INFO_D(?,?)}";
			...
			catch (Exception ex)
			{
				...
				GauceInfo.response.writeException("USER", "900001", "T_SET_EMP_INSUR_INFO 테이블 데이타 갱신 에러" + ex.getMessage());
