	...
	//복사시작
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_SET_COST_CONV_SLIP_I(?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_SET_COST_CONV_SLIP_U(?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_SET_COST_CONV_SLIP_D(?,?)}";
			...
			catch (Exception ex)
			{
				...
				GauceInfo.response.writeException("USER", "900001", "T_SET_COST_CONV_SLIP 테이블 데이타 갱신 에러" + ex.getMessage());
		...
		if(!CITCommon.isNull(strAct) && strAct.equals("REMOVE"))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					strSql = "{call SP_T_REMOVE_COST_CONV_SLIP(?,?,?)}";
		...
		if(!CITCommon.isNull(strAct) && strAct.equals("MAKE"))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					strSql = "{call SP_T_MAKE_COST_CONV_SLIP(?,?,?,?)}";
