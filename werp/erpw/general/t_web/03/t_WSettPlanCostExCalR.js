...
function Initialize()
{
	G_addDataSet(dsMAIN, trans, gridMAIN, sSelectPageName+D_P1("ACT","MAIN"), "결산예산원가정산작업목록");
	G_addDataSet(dsSUB01, trans, gridSUB01, sSelectPageName+D_P1("ACT","SUB01"), "결산예정원가정산목록");
	
	G_addDataSet(dsLOV, null, null, null, "LOV");
	G_addDataSet(dsWORK_NO, null, null, null, "작업번호");
	G_addDataSet(dsLIST_NO, null, null, null, "목록번호");

	G_addDataSet(dsMAKE_SLIP, transMAKE_SLIP, null, sSelectPageName+D_P1("ACT","MAKE_SLIP"), "예정원가정산전표발행");
	G_addDataSet(dsREMOVE_SLIP, transREMOVE_SLIP, null, sSelectPageName+D_P1("ACT","MAKE_SLIP"), "예정원가정산전표삭제");
...
function btnFileLoad_onClick()
{
	...
	{
			C_msgOk("결산예산원가정산작업 목록을 먼저 등록하십시오.","확인");
			return;
	}
	...
	fileUploadForm.action =	"./t_WSettPlanCostExCalR_u.jsp?ACT=FILE"+
