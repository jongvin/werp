		...
		if(!CITCommon.isNull(strAct) && strAct.equals("MAKE"))
		{
			...
			try
			{
				if(rows.length > 0)
				{
					...
					strSql = "{call SP_T_MAKE_SHEET_SUM_START(?,?,?,?,?)}";
					...
					lrArgData.setValue("DEPT_CODE",  rows[0].getString(dsMAKE.indexOfColumn("DEPT_CODE")));
		...
		else if (CITCommon.isNull(strAct))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_SHEET_SUM_HEAD_I(?,?,?,?,?,?,?,?,?,?,?,?,?)}";
						...
						lrArgData.setValue("DEPT_CODE", rows[i].getString(dsHEAD.indexOfColumn("DEPT_CODE")));
					...
					{
						strSql = "{call SP_T_SHEET_SUM_HEAD_U(?,?,?,?,?,?,?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_SHEET_SUM_HEAD_D(?,?,?)}";
