	...
	//복사시작
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_SET_PREV_COST_I(?,?,?,?,?,?,?,?,?,?,?)}";
						...
						lrArgData.addColumn("R_SLIP_ID", CITData.NUMBER);
						lrArgData.addColumn("R_SLIP_IDSEQ", CITData.NUMBER);
						...
						lrArgData.setValue("R_SLIP_ID", rows[i].getString(dsMAIN.indexOfColumn("R_SLIP_ID")));
						lrArgData.setValue("R_SLIP_IDSEQ", rows[i].getString(dsMAIN.indexOfColumn("R_SLIP_IDSEQ")));
						lrArgData.setValue("REMARKS", rows[i].getString(dsMAIN.indexOfColumn("REMARKS")));
					}
					else if (rows[i].getJobType() == GauceDataRow.TB_JOB_UPDATE)
					{
						strSql = "{call SP_T_SET_PREV_COST_U(?,?,?,?,?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_SET_PREV_COST_D(?,?)}";
	...
	//복사시작
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					if (rows[i].getJobType() == GauceDataRow.TB_JOB_INSERT)
					{
						strSql = "{call SP_T_SET_PREV_COST_LIST_I(?,?,?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_SET_PREV_COST_LIST_U(?,?,?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_SET_PREV_COST_LIST_D(?,?,?)}";
	...
	//복사시작
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_SET_MAKE_PRECODST_SLIP(?,?,?,?)}";
			...
			{
				if (GauceInfo == null) GauceInfo = new CITGauceInfo(request, response);
				GauceInfo.response.writeException("USER", "900001", "SP_T_SET_MAKE_PRECODST_SLIP 테이블 데이타 갱신 에러" + ex.getMessage());
				throw new Exception(ex.getMessage());
			}
	//복사끝
		}
	...
	//복사시작
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					if (rows[i].getJobType() == GauceDataRow.TB_JOB_UPDATE)
					{
						strSql = "{call SP_T_SET_REMOVE_PRECOST_SLIP(?,?,?)}";
