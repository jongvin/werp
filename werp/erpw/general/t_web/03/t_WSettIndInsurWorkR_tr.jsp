	...
	//복사시작
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_SET_MAKE_IND_INSU_DATA(?,?,?)}";
	...
	//복사시작
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					if (rows[i].getJobType() == GauceDataRow.TB_JOB_UPDATE)
					{
						strSql = "{call SP_T_SET_MAKE_IND_INS_SLIP(?,?,?,?)}";
	...
	//복사시작
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					if (rows[i].getJobType() == GauceDataRow.TB_JOB_UPDATE)
					{
						strSql = "{call SP_T_SET_REMOVE_IND_INS_SLIP(?,?,?)}";
	...
	//복사시작
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_SET_IND_INSUR_WORK_I(?,?,?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_SET_IND_INSUR_WORK_U(?,?,?,?,?,?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_SET_IND_INSUR_WORK_D(?,?)}";
			...
			{
				if (GauceInfo == null) GauceInfo = new CITGauceInfo(request, response);
				GauceInfo.response.writeException("USER", "900001", "T_SET_IND_INSUR_WORK 테이블 데이타 갱신 에러" + ex.getMessage());
				throw new Exception(ex.getMessage());
			}
	//복사끝
	//복사시작
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					if (rows[i].getJobType() == GauceDataRow.TB_JOB_INSERT)
					{
						strSql = "{call SP_T_SET_IND_INSUR_TARGET_SL_I(?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_SET_IND_INSUR_TARGET_SL_U(?,?,?,?)}";
					...
					{
						strSql = "{call SP_T_SET_IND_INSUR_TARGET_SL_D(?,?,?,?)}";
			...
			{
				if (GauceInfo == null) GauceInfo = new CITGauceInfo(request, response);
				GauceInfo.response.writeException("USER", "900001", "T_SET_IND_INSUR_TARGET_SLIP 테이블 데이타 갱신 에러" + ex.getMessage());
				throw new Exception(ex.getMessage());
			}
	//복사끝
	//여기에 붙여넣으세요
