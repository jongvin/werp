...
function Initialize()
{
	...
	G_addDataSet(dsDEPT_CODE, null, null, null, "부서코드");
	...
	autopopval = new AutoPopWhenChange(dsSEARCH,"T_DEPT_CODE3",txtPROJ_NAME,"autopopval",true);
	autopopval.SetArgBind("COMP_CODE",txtCOMP_CODE,"value");
	autopopval.SetArgBind("SEARCH_CONDITION",txtPROJ_NAME,"value");
	autopopval.SetReturnBind("DEPT_CODE",txtDEPT_CODE,"value");
	autopopval.SetReturnBind("DEPT_NAME",txtPROJ_NAME,"value");

	autopopval2 = new AutoPopWhenChange(dsSEARCH,"T_DEPT_CODE3",txtDEPT_CODE,"autopopval2",true);
	autopopval2.SetArgBind("COMP_CODE",txtCOMP_CODE,"value");
	autopopval2.SetArgBind("SEARCH_CONDITION",txtDEPT_CODE,"value");
	autopopval2.SetReturnBind("DEPT_CODE",txtDEPT_CODE,"value");
	autopopval2.SetReturnBind("DEPT_NAME",txtPROJ_NAME,"value");

	txtDEPT_CODE.attachEvent ("onblur", txtDEPT_CODE_onBlur);
	txtPROJ_NAME.attachEvent ("onblur", txtPROJ_NAME_onBlur);
	...
	if(chkDEPT_CODE.checked)
	{
		txtDEPT_CODE.value ="";
		txtPROJ_NAME.value ="";
		txtDEPT_CODE.readOnly=true;
		txtDEPT_CODE.style.backgroundColor = "#EFEFEF";
	...
	{
		txtDEPT_CODE.readOnly=false;
		txtDEPT_CODE.style.backgroundColor = "#ffffff";
...
function OnLoadBefore(dataset)
{
	if(dataset == dsMAIN)
	{
		dataset.DataID = sSelectPageName + D_P1("ACT","MAIN")
										...
										+ D_P2("DEPT_CODE",(C_isNull(txtDEPT_CODE.value)? "%":txtDEPT_CODE.value));
	}
	...
	else if(dataset == dsDEPT_CODE)
	{
		dataset.DataID = sSelectPageName + D_P1("ACT","DEPT_CODE")
										+ D_P2("COMP_CODE",txtCOMP_CODE.value);
	}
...
function	InsertIntoHead()
{
	...
	if(chkDEPT_CODE.checked)
	{
		for(i = 1; i <= dsDEPT_CODE.CountRow; i++)
		{
			G_addRow(dsHEAD);
			dsHEAD.NameString(dsHEAD.RowPosition,"DEPT_CODE")     = dsDEPT_CODE.NameString(i,"DEPT_CODE");
		...
		G_addRow(dsHEAD);
		dsHEAD.NameString(dsHEAD.RowPosition,"DEPT_CODE")     = "%";
		dsHEAD.NameString(dsHEAD.RowPosition,"CURR_ACC_ID")   = txtCURR_ACC_ID.value;
		dsHEAD.NameString(dsHEAD.RowPosition,"CURR_ACC_FDT")  = txtCURR_ACC_FDT.value;
		dsHEAD.NameString(dsHEAD.RowPosition,"CURR_ACC_EDT")  = txtCURR_ACC_EDT.value;
		dsHEAD.NameString(dsHEAD.RowPosition,"PAST_ACC_ID")   = txtPAST_ACC_ID.value;
	...
	else
	{
		...
		G_addRow(dsHEAD);
		dsHEAD.NameString(dsHEAD.CountRow,"DEPT_CODE") = C_isNull(txtDEPT_CODE.value)? "%":txtDEPT_CODE.value ;
...
function	OnRowPosChanged(dataset, row)
{
	if(dataset == dsSHEET_CODE)
	{
		...
		if (dsSHEET_CODE.NameString(dsSHEET_CODE.RowPosition,"SHEET_TYPE") == "4")
		{
			...
			chkDEPT_CODE.disabled  = false;
		}
		else
		{
			chkDEPT_CODE.checked   = false;
			chkDEPT_CODE.disabled  = true;
		}
		if(chkDEPT_CODE.checked)
		{
			txtDEPT_CODE.value ="";
...
function	btnMake_onClick()
{
	...
	if(chkDEPT_CODE.checked)
	{
		dsMAKE.NameString(dsMAKE.RowPosition,"DEPT_CODE") = "%";
	}
	else
	{
		dsMAKE.NameString(dsMAKE.RowPosition,"DEPT_CODE") = dsHEAD.NameString(dsHEAD.RowPosition,"DEPT_CODE");
	}
...
function	btnSearchProj_onClick()
{
	...
	txtPROJ_NAME.value	= lrRet.get("DEPT_NAME");
}
...
{
	var			strDEPT_CODE = C_isNull(txtDEPT_CODE.value)?"%":txtDEPT_CODE.value;

	if(strDEPT_CODE != dsHEAD.NameString(dsHEAD.RowPosition,"DEPT_CODE"))
	{
		Retrieve();
	}
}
function	txtPROJ_NAME_onBlur()
{
	var			strDEPT_CODE = C_isNull(txtDEPT_CODE.value)?"%":txtDEPT_CODE.value;
