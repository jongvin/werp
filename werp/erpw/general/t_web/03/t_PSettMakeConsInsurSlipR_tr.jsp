		...
		if(strAct.equals("MAKE_DEFAULT"))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					{
						strSql = "{call SP_T_MAKE_DFLT_INSUR_SLIP(?,?,?,?,?,?,?,?)}";
						...
						lrArgData.addColumn("WORK_DEPT_CODE", CITData.VARCHAR2);
						...
						lrArgData.setValue("COMP_CODE", rows[i].getString(dsWORK_SLIP_IDSEQ.indexOfColumn("COMP_CODE")));
						lrArgData.setValue("DEPT_CODE", rows[i].getString(dsWORK_SLIP_IDSEQ.indexOfColumn("DEPT_CODE")));
						lrArgData.setValue("WORK_DEPT_CODE", rows[i].getString(dsWORK_SLIP_IDSEQ.indexOfColumn("WORK_DEPT_CODE")));
						lrArgData.setValue("INSUR_NO", rows[i].getString(dsWORK_SLIP_IDSEQ.indexOfColumn("INSUR_NO")));
		...
		if(strAct.equals("MAKE_SLIP"))
		{
			...
			try
			{
				for	(int i = 0;	i <	rows.length; i++)
				{
					...
					if (rows[i].getJobType() == GauceDataRow.TB_JOB_UPDATE)
					{
						strSql = "{call SP_T_FIN_MAKE_INSUR_SLIP(?,?,?,?,?,?,?,?)}";
						...
						lrArgData.setValue("WORK_DEPT_CODE", rows[i].getString(dsMAKE_SLIP.indexOfColumn("WORK_DEPT_CODE")));
						lrArgData.setValue("INSUR_NO", rows[i].getString(dsMAKE_SLIP.indexOfColumn("INSUR_NO")));
						lrArgData.setValue("CRTUSERNO", strUserNo);
					}
					else
					{
