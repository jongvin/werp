...
function Initialize()
{
	...
	G_addDataSet(dsMAIN,  trans,   gridMAIN, sSelectPageName+D_P1("ACT","MAIN"), "원가배부코드");
	...
	G_addRelCol(dsSUB01,"DVD_TAR_CODE","DVD_TAR_CODE");
...
function OnLoadBefore(dataset)
{
	...
	else if(dataset == dsSUB01)
	{
		dataset.DataID = sSelectPageName + D_P1("ACT","SUB01")
										 + D_P2("DVD_TAR_CODE",dsMAIN.NameString(dsMAIN.RowPosition, "DVD_TAR_CODE"));
	}
}
...
function btnadd_onclick()
{
	if(G_FocusDataset==dsMAIN) {
		...
		if(dsMAIN.CountRow==0) {
			C_msgOk("원가배부코드를 먼저 선택하세요.", "확인");
			return;
		}
		var lrArgs = new C_Dictionary();
		var lrRet = null;

		lrArgs.set("DVD_TAR_CODE", dsMAIN.NameValue(dsMAIN.RowPosition,"DVD_TAR_CODE"));
		lrArgs.set("COME_CODE", sCompCode);
		lrArgs.set("COST_DEPT_KND_TAG", '2');
		lrArgs.set("SEARCH_CONDITION", "");
	
		lrRet = C_LOV_NEW("T_DEPT_CODE9", null, lrArgs, "T", "T");
