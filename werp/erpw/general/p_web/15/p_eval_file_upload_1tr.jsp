...
String dir = (String) session.getAttribute("dir");
  ...
  for(int i=0; i< rowCnt; i++){
      ...
      stmt.close(); 
       ...
       String d_file_code  = rows.getString(idx_file_code);
       String d_name  = rows.getString(idx_d_name_file); 
      
		 InputStream is = (InputStream) rowx[i].getInputStream(idx_cdir);

		 FileOutputStream os = new FileOutputStream(dir + d_self_evaluator + '_' + d_file_code +  d_name );
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "update p_eval_file_upload set " + 
                            ...
                            "url=?  where eval_code=? and eval_degree=? and self_evaluator=? and file_code=? ";  
      stmt = conn.prepareStatement(sSql); 
      gpstatement.gp_stmt = stmt;
      gpstatement.bindColumn(1, idx_eval_code);
      gpstatement.bindColumn(2, idx_eval_degree);
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "delete from p_eval_file_upload where eval_code=? and eval_degree=? and self_evaluator=? and file_code=? "; 
      stmt = conn.prepareStatement(sSql); 
      gpstatement.gp_stmt = stmt;
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
      gpstatement.bindColumn(1, idx_eval_code);
