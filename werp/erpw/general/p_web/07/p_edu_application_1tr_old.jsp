<%@ page session="true" import="com.gauce.*,com.gauce.io.*,com.gauce.common.*,com.gauce.log.*,com.gauce.db.*,java.sql.*"%><%@
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "update p_edu_emp set " + 
                            ...
                            "edu_place=?,  " + 
                    			 ...
                    			 "remark," + 
                            "finish_tag=?,  " + 
                            "result=?  where emp_no=? and edu_code=? and edu_year=? edu_degree=? and spec_no_seq=? ";  
      stmt = conn.prepareStatement(sSql); 
      gpstatement.gp_stmt = stmt;
      gpstatement.bindColumn(1, idx_emp_no);
      gpstatement.bindColumn(2, idx_edu_code);
 ...
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
      ...
      gpstatement.bindColumn(22, idx_edu_code);
      gpstatement.bindColumn(23, idx_edu_year);
      gpstatement.bindColumn(24, idx_edu_degree);
      gpstatement.bindColumn(25, idx_spec_no_seq);
      stmt.executeUpdate();
      stmt.close();
   }else if(rows.getJobType() ==  GauceDataRow.TB_JOB_DELETE){
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "delete from p_edu_emp where emp_no=? and edu_code=? and edu_year=? edu_degree=? and spec_no_seq=? "; 
      stmt = conn.prepareStatement(sSql); 
      gpstatement.gp_stmt = stmt;
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
      gpstatement.bindColumn(1, idx_emp_no);
