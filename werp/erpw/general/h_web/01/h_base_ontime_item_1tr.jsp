...
include file ="../../../comm_function/conn_tr_pool.jsp"%><%
     GauceDataSet dSet = req.getGauceDataSet("h_base_ontime_item_1tr"); gpstatement.gp_dataset = dSet;
   ...
   int idx_pyong = dSet.indexOfColumn("pyong");
	...
	if(rows.getJobType() == GauceDataRow.TB_JOB_INSERT){
      ...
      gpstatement.bindColumn(3, idx_pyong);
      gpstatement.bindColumn(4, idx_opt_name);
      gpstatement.bindColumn(5, idx_opt_base);
      gpstatement.bindColumn(6, idx_opt_finish);
      gpstatement.bindColumn(7, idx_opt_ref);
      gpstatement.bindColumn(8, idx_amt_norm);
	  gpstatement.bindColumn(9, idx_amt_norm_vat);
	  gpstatement.bindColumn(10, idx_amt_prem);
	  gpstatement.bindColumn(11, idx_amt_prem_vat);
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "update h_base_ontime_item set " + 
					...
					" amt_prem=?," +
					" amt_prem_vat=?," +
							" note=? where dept_code=?  and sell_codd=? " +
 ...
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
      ...
      gpstatement.bindColumn(14, idx_old_pyong);
      gpstatement.bindColumn(15, idx_old_opt_name);
      gpstatement.bindColumn(16, idx_old_opt_base);
      gpstatement.bindColumn(17, idx_old_opt_finish);
      gpstatement.bindColumn(18, idx_old_opt_ref);
      stmt.executeUpdate();
      stmt.close();
   }else if(rows.getJobType() ==  GauceDataRow.TB_JOB_DELETE){
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "delete from h_base_ontime_item where dept_code=?  and sell_code=?" +
               " and pyong=? and   opt_name=? and opt_base=? and opt_finish=? and  opt_ref=? ";  
      stmt = conn.prepareStatement(sSql); 
      gpstatement.gp_stmt = stmt;
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
      gpstatement.bindColumn(1, idx_dept_code);
      gpstatement.bindColumn(2, idx_sell_code);
      gpstatement.bindColumn(3, idx_old_pyong);
      gpstatement.bindColumn(4, idx_old_opt_name);
      gpstatement.bindColumn(5, idx_old_opt_base);
      gpstatement.bindColumn(6, idx_old_opt_finish);
      gpstatement.bindColumn(7, idx_old_opt_ref);
      stmt.executeUpdate();
      stmt.close();
    }
     }
