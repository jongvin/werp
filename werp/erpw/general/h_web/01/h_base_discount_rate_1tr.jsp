...
include file ="../../../comm_function/conn_tr_pool.jsp"%><%
     GauceDataSet dSet = req.getGauceDataSet("h_base_discount_rate_1tr"); gpstatement.gp_dataset = dSet;
	...
	if(rows.getJobType() == GauceDataRow.TB_JOB_INSERT){
		String sSql = " INSERT INTO h_base_discount_rate ( dept_code," + 
                    "sell_code," + 
                    "rate_kind," + 
                    "s_date," + 
                    "e_date," + 
      ...
      gpstatement.bindColumn(7, idx_cutoff_std);
      gpstatement.bindColumn(8, idx_cutoff_unit);
      stmt.executeUpdate();
      stmt.close();
 
   }else if(rows.getJobType() ==  GauceDataRow.TB_JOB_UPDATE){
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "update h_base_discount_rate set " + 
                            "dept_code=?,  " + 
                            "sell_code=?,  " + 
                            "rate_kind=?,  " + 
                            "s_date=?,  " + 
                            ...
                            "rate=?,  " + 
                            "cutoff_std=?,  " + 
                            "cutoff_unit=?  where dept_code=? and sell_code=? and rate_kind=? and s_date=? ";  
      stmt = conn.prepareStatement(sSql); 
      gpstatement.gp_stmt = stmt;
      gpstatement.bindColumn(1, idx_dept_code);
      gpstatement.bindColumn(2, idx_sell_code);
 ...
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
      ...
      gpstatement.bindColumn(11, idx_old_rate_kind);
      gpstatement.bindColumn(12, idx_old_s_date);
      stmt.executeUpdate();
      stmt.close();
   }else if(rows.getJobType() ==  GauceDataRow.TB_JOB_DELETE){
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "delete from h_base_discount_rate where dept_code=? and sell_code=? and rate_kind=? and s_date=?  "; 
      stmt = conn.prepareStatement(sSql); 
      gpstatement.gp_stmt = stmt;
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
      gpstatement.bindColumn(1, idx_dept_code);
      ...
      gpstatement.bindColumn(4, idx_old_s_date);
      stmt.executeUpdate();
      stmt.close();
    }
     }
