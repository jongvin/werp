<%@ page session="true" import="com.gauce.*,com.gauce.io.*,com.gauce.common.*,com.gauce.log.*,com.gauce.db.*,java.sql.*"%><%@
 include file ="../../../comm_function/conn_tr_pool.jsp"%><%
     GauceDataSet dSet = req.getGauceDataSet("h_contract_detail_3tr");
 ...
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
      gpstatement.bindColumn(15, idx_dept_code);
	   gpstatement.bindColumn(16, idx_sell_code);
		...
		gpstatement.bindColumn(19, idx_pyong);
		gpstatement.bindColumn(20, idx_opt_name);
		gpstatement.bindColumn(21, idx_opt_base);
		gpstatement.bindColumn(22, idx_opt_finish);
		gpstatement.bindColumn(23, idx_opt_ref);
      stmt.executeUpdate();
      stmt.close();
   }else if(rows.getJobType() ==  GauceDataRow.TB_JOB_DELETE){
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "delete from h_sale_ontime_master "+
	                            "         where dept_code=? " +
	                            "             and sell_code=? "+
	                         "                and dongho=? "+
	                         "                and seq=? " +
