CREATE OR REPLACE package body h_calc_income_grt is
...
-- LOOP 시작
		LOOP
	-- 일수 및 약정금액을 구한다
			select nvl((agree_date - as_input_date) * -1,0),nvl(AMT,0),agree_date
			  into C_DAYS,C_LEASE_AMT,C_AGREE_DATE
			  from H_LEASE_GRT_AGREE
			 where dept_code     = as_dept_code
				and sell_code     = as_sell_code
				and cont_date     = as_cont_date
...
--							C_TEMP_AMT  := c_input_amt;
	...
	-- 계산된 값을 분양_보증금납입에 넣어준다.
			INSERT INTO H_LEASE_GRT_INCOME
				VALUES ( as_dept_code,as_sell_code,as_cont_date,as_cont_seq,C_LAST_DEGREE,C_LAST_SEQ,
							as_input_date,as_daily_seq, as_input_deposit,C_TEMP_AMT,
...
-------------------------------------------------------------
-- 공통 변수
   C_RECEIPT_DATE      H_LEASE_GRT_INCOME_DAILY.RECEIPT_DATE%TYPE;
	C_DAILY_SEQ         H_LEASE_GRT_INCOME_DAILY.DAILY_SEQ%TYPE;
	C_AMT                H_LEASE_GRT_INCOME_DAILY.AMT%TYPE;
	C_DEPOSIT_NO         H_LEASE_GRT_INCOME_DAILY.DEPOSIT_NO%TYPE;
...
BEGIN
	...
	IF C_RECEIPT_DATE > '1900.01.01' AND C_DAILY_SEQ > 0 THEN
	   --------------------------------이후자료삭제
		h_calc_income_grt.y_sp_h_delete_daily_grt(as_dept_code, as_sell_code, as_cont_date, as_cont_seq,
	...
	select nvl(max(daily_seq), 0) + 1
	 ...
	 where dept_code     = as_dept_code
		...
		and cont_seq      = as_cont_seq
		and receipt_date  = as_input_date;
	INSERT INTO H_LEASE_GRT_INCOME_DAILY   --일자별 납입금 테이블에 INSERT한다
	...
	h_calc_income_grt.y_sp_h_grt_calc(as_dept_code,as_sell_code,as_cont_date ,as_cont_seq ,as_input_date,C_DAILY_SEQ,
	...
	LOOP
		...
		-----------------------------------이후자료 재입금
		h_calc_income_grt.y_sp_h_grt_cmpt(as_dept_code, as_sell_code, as_cont_date, as_cont_seq, C_RECEIPT_DATE, C_AMT,
		                                      C_DEPOSIT_NO, '재계산');
	END LOOP;
...
-----------------------------------------삭제처리
		LOOP
			...
			update h_lease_grt_agree
			  set (tot_amt,f_pay_yn)=
				   (select agree.tot_amt - in_sum.amt,
				           case when agree.tot_amt - in_sum.amt = agree.amt
				                then 'Y'
								 	 else 'N'
							  end case
					   from h_lease_grt_agree agree,
					       (select income.degree_code,
					               sum(nvl(income.r_amt, 0)) amt
					          from h_lease_grt_income income
					  ...
					  where agree.dept_code = as_dept_code
					    ...
					    and agree.degree_code = in_sum.degree_code
						 and h_lease_grt_agree.degree_code = in_sum.degree_code)
			where h_lease_grt_agree.dept_code = as_dept_code
			  and h_lease_grt_agree.sell_code = as_sell_code
			  and h_lease_grt_agree.cont_date    = as_cont_date
			  and h_lease_grt_agree.cont_seq       = as_cont_seq
			  and h_lease_grt_agree.degree_code in (select income.degree_code
												          from h_lease_grt_income income
														   where income.dept_code = as_dept_code
														     and income.sell_code = as_sell_code
															  and income.cont_date    = as_cont_date
	     ...
	     delete from h_lease_grt_income income
		   where income.dept_code = as_dept_code
		     and income.sell_code = as_sell_code
			  and income.cont_date = as_cont_date
			  and income.cont_seq       = as_cont_seq
			  and income.receipt_date = C_RECEIPT_DATE
	        and income.daily_seq    = C_DAILY_SEQ;
		  delete from h_lease_grt_income_daily daily
  ...
  BEGIN
	begin
      ...
      IF C_WORK_AMT > 0 THEN
		...
		IF R_AMT_CHG_TAG = 'Y' THEN
		   SELECT SUM(NVL(R_AMT+DELAY_AMT-DISCOUNT_AMT,0))
			...
			IF C_R_AMT_OVER > 0 THEN
			   h_calc_income_grt.y_sp_h_grt_calc(as_dept_code,as_sell_code,as_cont_date ,as_cont_seq ,as_receipt_date,as_daily_seq,
		...
		END IF;
      delete h_lease_grt_income income
		 where income.dept_code = as_dept_code
	      and income.sell_code = as_sell_code
		   and income.cont_date = as_cont_date
		   and income.cont_seq  = as_cont_seq
		...
		update h_lease_grt_agree
		  set (tot_amt,f_pay_yn)=
			   (select in_sum.amt,
			           case when in_sum.amt = agree.amt
			                then 'Y'
							 	 else 'N'
						  end case
				   from h_lease_grt_agree agree,
		...
		where h_lease_grt_agree.dept_code = as_dept_code
		  ...
		  and h_lease_grt_agree.cont_date = as_cont_date
		  and h_lease_grt_agree.cont_seq  = as_cont_seq
		  and h_lease_grt_agree.degree_code = as_degree_code;

		select amt,  deposit_no
		  into C_DAILY_AMT, C_DEPOSIT_NO

		  from h_lease_grt_income_daily daily
		 where daily.dept_code = as_dept_code
	      and daily.sell_code = as_sell_code
		   and daily.cont_date = as_cont_date
		   and daily.cont_seq       = as_cont_seq
