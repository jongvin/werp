CREATE OR REPLACE PROCEDURE y_sp_h_income_rent_cmpt_tempbk(as_spec_unq_num IN NUMBER,
...
-- 공통 변수
   C_LAST_DEGREE       H_LEASE_rent_AGREE_temp.DEGREE_CODE%TYPE;    -- 마지막약정차수
   C_MAX_DEGREE        H_LEASE_rent_AGREE_temp.DEGREE_CODE%TYPE;    -- 마지막약정차수(잔금,인상분 포함)
   C_LAST_SEQ          H_LEASE_rent_INCOME_temp.DEGREE_SEQ%TYPE;    -- 마지막회수
   C_AGREE_DATE        H_LEASE_rent_AGREE_temp.AGREE_DATE%TYPE;     -- 약정일자
   C_RENT_AMT         H_LEASE_RENT_AGREE_temp.rent_AMT%TYPE;       -- 약정금액
   C_DELAY_AMT         H_LEASE_rent_AGREE_temp.rent_AMT%TYPE;       -- 연체료
   C_DISCOUNT_AMT      H_LEASE_rent_AGREE_temp.rent_AMT%TYPE;       -- 할인료
   C_DELAY_DAY         H_LEASE_rent_INCOME_temp.DISCOUNT_DAYS%TYPE;       -- 연체일자
   C_DISCOUNT_DAY      H_LEASE_rent_INCOME_temp.DISCOUNT_DAYS%TYPE;       -- 할인일자
   C_R_AMT             H_LEASE_rent_AGREE_temp.rent_AMT%TYPE;       -- 납입금액
   C_WORK_AMT          H_LEASE_rent_AGREE_temp.rent_AMT%TYPE;       -- 약정대상금액
   C_TEMP_AMT          H_LEASE_rent_AGREE_temp.rent_AMT%TYPE;       -- 약정계산금액
   C_S_DATE        	  H_LEASE_rent_AGREE_temp.AGREE_DATE%TYPE;     -- 시작일자
   C_E_DATE        	  H_LEASE_rent_AGREE_temp.AGREE_DATE%TYPE;     -- 종료일자
   C_FR_DATE        	  H_LEASE_rent_AGREE_temp.AGREE_DATE%TYPE;     -- 입주개시일
   C_TO_DATE        	  H_LEASE_rent_AGREE_temp.AGREE_DATE%TYPE;     -- 입주종료일
...
-- LOOP 시작
		LOOP
	-- 일수 및 약정금액을 구한다
			select nvl((agree_date - as_input_date) * -1,0),nvl(rent_AMT_tmp,0),agree_date, vat_yn
...
--							C_TEMP_AMT  := c_input_amt;
	...
	-- 계산된 값을 분양_보증금납입에 넣어준다.
	      if C_LAST_SEQ < 70 THEN
				INSERT INTO H_LEASE_rent_INCOME_temp
					VALUES ( as_spec_unq_num,as_dept_code,as_sell_code,as_cont_date,as_cont_seq,C_LAST_DEGREE,C_LAST_SEQ,
								as_input_date, as_input_deposit,C_TEMP_AMT,C_RENT_SUPPLY, C_RENT_VAT,
	...
	-- 약정사항에 입금완료구분값과 입금합계값을 넣어준다.
			UPDATE H_LEASE_rent_AGREE_temp
...
END y_sp_h_income_rent_cmpt_tempbk;
/
