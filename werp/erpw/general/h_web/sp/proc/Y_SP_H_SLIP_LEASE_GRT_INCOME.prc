CREATE OR REPLACE PROCEDURE Y_Sp_H_Slip_Lease_Grt_Income (ai_save_num IN NUMBER,
...
-- User Define Error
   ...
   C_CATEGORY          VARCHAR2(50)  DEFAULT '임대계약';
...
BEGIN
 BEGIN
   ...
   e_msg      := 'EFIN_INVOICE_HEADER_ITF Insert 실패 '||C_GROUP_ID||','||ai_save_num||','||'A';
	...
	OPEN	DETAIL_CUR;
	LOOP
      FETCH DETAIL_CUR INTO rec_slip_grt_income.cont_date, rec_slip_grt_income.cont_seq, 
                            rec_slip_lease_master.cont_type, rec_slip_grt_income.degree_code, rec_slip_grt_income.receipt_date,C_FOLDER_CODE, 
		...
		c_h_data := trim(TO_CHAR(rec_slip_grt_income.cont_date,'yyyymm'))||trim(TO_CHAR(rec_slip_grt_income.cont_seq,'000'))||trim(rec_slip_grt_income.degree_code);
      C_DESC := trim(TO_CHAR(rec_slip_grt_income.cont_date,'yyyymm'))||'-'||trim(TO_CHAR(rec_slip_grt_income.cont_seq,'00'))||'-'||rec_slip_cust.cust_name||'-'||trim(rec_slip_grt_income.degree_code);
      ...
      v_delay_amt     := rec_slip_grt_income.delay_amt;
      v_discount_amt  := rec_slip_grt_income.discount_amt;
      v_sell_amt      := rec_slip_grt_income.r_amt;
      ...
      Y_Sp_Comm_Slip_Gl_Invoice(C_COMP_CODE,C_GROUP_ID,as_approval_name,rec_slip_grt_income.receipt_date,
...
END Y_Sp_H_Slip_Lease_Grt_Income;
/
