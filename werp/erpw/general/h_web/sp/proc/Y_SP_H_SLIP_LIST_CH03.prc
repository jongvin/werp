...
BEGIN
   BEGIN
		...
		select substr(code_name, 1, 8)
		  ...
		  from h_code_common
		 where code_div = '25'
		  and  CODE     = '23' ;        -- 미지급비용(해약분)
		if v_acntcode_02 is null then
			e_msg := '미지급비용(해약분) 계정 계정 없음 ch03';
			Wk_errflag := -20020;
         GOTO EXIT_ROUTINE;
      end if;
		OPEN	DETAIL_CUR;
		LOOP
			...
			IF C_CNT < 1 THEN
			   e_msg      := '전표번호 생성 실패! ch03 [Line No: -1]';
			...
			ELSE
		   	 ...
		   	 where work_date = v_occ_date;
				C_SEQ := C_SEQ + 1;
			   e_msg      := '전표번호 수정 실패! ch03 [Line No: -2]';
			...
			IF v_cust_code is not null THEN
				...
				IF C_CNT < 1 THEN
					e_msg      := '거래처 생성 실패! ch03 [Line No: -3]' || v_cust_code;
			...
			IF C_START_SEQ = 0 THEN
				C_START_SEQ := C_SEQ;
			END IF;
		   e_msg      := '가전표 마스타 생성 실패! ch03 [Line No: 4]';
...
-- 가전표_디테일을 입력한다
	        ...
	        ;
			v_desc     := v_cust_name || ' 청약금 환불';
		   e_msg      := '가전표 디테일 생성 실패! ch03 [Line No: 5]'|| C_SEQ || ' 1';
			C_DT_SEQ := 1;
		   ...
		   e_msg      := '가전표 디테일 생성 실패! ch03 [Line No: 6]'|| C_SEQ || ' 2';
			C_DT_SEQ := C_DT_SEQ + 1;
			INSERT INTO erpc.AM_WORK_DETAIL
			VALUES ( v_occ_date,C_SEQ,C_DT_SEQ,C_DT_SEQ,as_dept,C_KIND_CODE,'01',as_dept_code,v_deptdetail_code,
