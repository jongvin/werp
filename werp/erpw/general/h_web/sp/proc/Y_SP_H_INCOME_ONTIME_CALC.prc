CREATE OR REPLACE PROCEDURE Y_Sp_H_Income_Ontime_Calc(as_dept_code     IN   VARCHAR2,
...
-------------------------------------------------------------
-- 공통 변수
   C_LAST_DEGREE       H_SALE_ONTIME_AGREE.DEGREE_CODE%TYPE;    -- 마지막약정차수
   C_MAX_DEGREE        H_SALE_ONTIME_AGREE.DEGREE_CODE%TYPE;    -- 마지막약정차수(잔금,인상분 포함)
   C_LAST_SEQ          H_SALE_ONTIME_INCOME.DEGREE_SEQ%TYPE;    -- 마지막회수
   C_AGREE_DATE        H_SALE_ONTIME_AGREE.AGREE_DATE%TYPE;     -- 약정일자
   C_AGREE_AMT         H_SALE_ONTIME_AGREE.AGREE_AMT%TYPE;       -- 약정금액
   C_DELAY_AMT         H_SALE_ONTIME_AGREE.AGREE_AMT%TYPE;       -- 연체료
   C_DISCOUNT_AMT      H_SALE_ONTIME_AGREE.AGREE_AMT%TYPE;       -- 할인료
   C_DELAY_DAY         H_SALE_ONTIME_INCOME.DISCOUNT_DAYS%TYPE;       -- 연체일자
   C_DISCOUNT_DAY      H_SALE_ONTIME_INCOME.DISCOUNT_DAYS%TYPE;       -- 할인일자
   C_R_AMT             H_SALE_ONTIME_AGREE.AGREE_AMT%TYPE;       -- 납입금액
   C_WORK_AMT          H_SALE_ONTIME_AGREE.AGREE_AMT%TYPE;       -- 약정대상금액
   C_TEMP_AMT          H_SALE_ONTIME_AGREE.AGREE_AMT%TYPE;       -- 약정계산금액
   C_S_DATE        	  H_SALE_ONTIME_AGREE.AGREE_DATE%TYPE;     -- 시작일자
   C_E_DATE        	  H_SALE_ONTIME_AGREE.AGREE_DATE%TYPE;     -- 종료일자
   C_FR_DATE        	  H_SALE_ONTIME_AGREE.AGREE_DATE%TYPE;     -- 입주개시일
   C_TO_DATE        	  H_SALE_ONTIME_AGREE.AGREE_DATE%TYPE;     -- 입주종료일
...
BEGIN
	BEGIN
	   SELECT NVL(MAX(daily_seq), 0) + 1
		...
		INSERT INTO H_SALE_ONTIME_INCOME_DAILY   --일자별 납입금 테이블에 INSERT한다
	...
	-- 계산된 값을 분양_세대별수입금에 넣어준다.
			INSERT INTO H_SALE_ONTIME_INCOME
				VALUES ( as_dept_code,as_sell_code,as_dongho,as_seq,C_LAST_DEGREE,C_LAST_SEQ,as_input_date,
							...
							as_receipt_class_code, as_receipt_number, as_card_app_num, as_card_bank_code ,C_DAILY_SEQ, 0,0 )  ;
	-- 기납입금액누계를 구한다.
			SELECT NVL(SUM(r_amt),0)
			  INTO C_R_AMT
			  FROM H_SALE_ONTIME_INCOME
