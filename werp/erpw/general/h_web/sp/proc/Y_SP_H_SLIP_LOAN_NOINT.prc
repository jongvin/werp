CREATE OR REPLACE PROCEDURE Y_Sp_H_Slip_Loan_Noint (ai_save_num IN NUMBER,
...
CURSOR DETAIL_CUR IS
  SELECT a.dongho, 
   ...
   WHERE (LENGTH(trim(b.work_no)) IS NULL OR             --전표생성 안？거나
     ...
     AND a.chg_div <> '00'
     AND b.loan_interest_tag = '01'                            --무이자
...
-- User Define Error
   ...
   C_ACNTNO_DUI        VARCHAR2(50)  DEFAULT '442615';--지급수수료(이자)
   C_ACNTNO_ACC        VARCHAR2(50)  DEFAULT '111114';--보통예금(원화)
    
   
   C_SOURCE            VARCHAR2(50)  DEFAULT '분양';
   C_CATEGORY          VARCHAR2(50)  DEFAULT '대출이자 대납(무이자)';
