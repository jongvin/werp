CREATE OR REPLACE PROCEDURE y_sp_h_slip_list_bp02(as_dept_code    IN   VARCHAR2,
   ...
   BEGIN
      ...
      v_w_cnt := 999;    -- 전표 write  건수가 99 건이 넘으면 새로운 전표를 만든다.
		...
		LOOP
			...
			END IF;
-- 가전표_디테일을 입력한다
			select  c.code_name, a.agree_amt, sum(b.r_amt)
			  into  v_code_name, v_sell_amt, v_r_amt
			  from h_sale_ontime_agree a, h_sale_ontime_income b, h_code_common c
		    where a.dept_code   = as_dept_code
		      ...
		      and c.code_div(+) = '02'
		      and c.code(+)     = v_degree
          group by c.code_name, a.agree_amt  ;
			v_dongho_tmp := substr(v_dongho, 1, 4) || '-' || substr(v_dongho, 5, 4);
			v_desc := ' 별도품목 ' || v_code_name || '-' || v_cust_name || ' ' || v_dongho_tmp;
			if v_tag = '9' then
         ...
         v_amt := v_amt1;
	      ...
	      if v_tag = '3' then
			...
			if v_tag = '9' then
				...
				if v_contract_code = '02' then
					v_acnt_tmp2 := v_acntcode_02_3;     -- 임대미수금
				else
					if v_sell_code = '03' then
