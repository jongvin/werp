CREATE OR REPLACE PROCEDURE y_sp_h_income_grt_cmpt_tempbk(as_spec_unq_num IN NUMBER,
...
-------------------------------------------------------------
-- 공통 변수
   C_LAST_DEGREE       H_LEASE_GRT_AGREE_TEMP.DEGREE_CODE%TYPE;    -- 마지막약정차수
   C_MAX_DEGREE        H_LEASE_GRT_AGREE_TEMP.DEGREE_CODE%TYPE;    -- 마지막약정차수(잔금,인상분 포함)
   C_LAST_SEQ          H_LEASE_GRT_INCOME_TEMP.DEGREE_SEQ%TYPE;    -- 마지막회수
   C_AGREE_DATE        H_LEASE_GRT_AGREE_TEMP.AGREE_DATE%TYPE;     -- 약정일자
   C_LEASE_AMT         H_LEASE_GRT_AGREE_TEMP.AMT%TYPE;       -- 약정금액
   C_DELAY_AMT         H_LEASE_GRT_AGREE_TEMP.AMT%TYPE;       -- 연체료
   C_DISCOUNT_AMT      H_LEASE_GRT_AGREE_TEMP.AMT%TYPE;       -- 할인료
   C_DELAY_DAY         H_LEASE_GRT_INCOME_TEMP.DISCOUNT_DAYS%TYPE;       -- 연체일자
   C_DISCOUNT_DAY      H_LEASE_GRT_INCOME_TEMP.DISCOUNT_DAYS%TYPE;       -- 할인일자
   C_R_AMT             H_LEASE_GRT_AGREE_TEMP.AMT%TYPE;       -- 납입금액
   C_WORK_AMT          H_LEASE_GRT_AGREE_TEMP.AMT%TYPE;       -- 약정대상금액
   C_TEMP_AMT          H_LEASE_GRT_AGREE_TEMP.AMT%TYPE;       -- 약정계산금액
   C_S_DATE        	  H_LEASE_GRT_AGREE_TEMP.AGREE_DATE%TYPE;     -- 시작일자
   C_E_DATE        	  H_LEASE_GRT_AGREE_TEMP.AGREE_DATE%TYPE;     -- 종료일자
   C_FR_DATE        	  H_LEASE_GRT_AGREE_TEMP.AGREE_DATE%TYPE;     -- 입주개시일
   C_TO_DATE        	  H_LEASE_GRT_AGREE_TEMP.AGREE_DATE%TYPE;     -- 입주종료일
...
-- LOOP 시작
		LOOP
	-- 일수 및 약정금액을 구한다
			select nvl((agree_date - as_input_date) * -1,0),nvl(AMT_TMP,0),agree_date
...
--							C_TEMP_AMT  := c_input_amt;
	...
	-- 계산된 값을 분양_보증금납입에 넣어준다.
	      if C_LAST_SEQ < 70 THEN
				INSERT INTO H_LEASE_GRT_INCOME_TEMP
					VALUES ( as_spec_unq_num,as_dept_code,as_sell_code,as_cont_date,as_cont_seq,C_LAST_DEGREE,C_LAST_SEQ,
								as_input_date, as_input_deposit,C_TEMP_AMT,
								C_DELAY_DAY,C_DELAY_AMT,C_DISCOUNT_DAY,C_DISCOUNT_AMT,null,null,as_user,sysdate,'Y')  ;
			end if;
	-- 기납입금액누계를 구한다.
			select nvl(sum(r_amt),0)
			  into C_R_AMT
...
END y_sp_h_income_grt_cmpt_tempbk;
/
