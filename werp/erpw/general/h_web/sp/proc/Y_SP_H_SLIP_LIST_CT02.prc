CREATE OR REPLACE PROCEDURE y_sp_h_slip_list_ct02(as_dept_code    IN   VARCHAR2,
...
-- 변수선언
-------------------------------------------------------------
-- 계약금 입금 사항  (임대는 제외함 : 보통예금 수작업 전표 입력)
...
BEGIN
   BEGIN
      ...
      v_w_cnt := 999;    -- 전표 write  건수가 99 건이 넘으면 새로운 전표를 만든다.
		OPEN	DETAIL_CUR;
		LOOP
			FETCH DETAIL_CUR INTO v_dongho, v_contract_code, v_occ_date, v_cust_code, v_cust_name, v_amt;
...
-- 가전표_디테일을 입력한다
			...
			v_desc := '계약금 입금-' || v_cust_name || ' ' || v_dongho_tmp;
			-- 차변 시작
