CREATE OR REPLACE PROCEDURE Y_Sp_H_Slip_Loan_Fin (ai_save_num IN NUMBER,
...
-- User Define Error
   ...
   C_ACNTNO_DUI        VARCHAR2(50)  DEFAULT '111821';--미수금(유이자)
   C_ACNTNO_ACC        VARCHAR2(50)  DEFAULT '111131';--보통예금(원화)
    
   
   C_SOURCE            VARCHAR2(50)  DEFAULT '분양';
   C_CATEGORY          VARCHAR2(50)  DEFAULT '대출이자 정산';
 ...
 BEGIN
   ...
   e_msg      := 'EFIN_INVOICE_HEADER_ITF Insert 실패 '||C_GROUP_ID||','||ai_save_num||','||'A';
	...
	LOOP
      ...
      v_amt  := ABS(rec_slip_loan.amt);  --정산금액은 분양에서 (-)금액으로 입금되기 때문에 (+) 로 바꿔서 전표발행  
