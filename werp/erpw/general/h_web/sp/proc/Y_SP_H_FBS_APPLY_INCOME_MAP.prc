CREATE OR REPLACE PROCEDURE Y_Sp_H_Fbs_Apply_Income_Map(AS_FB_SEQ IN NUMBER,
    ...
    V_PRINT_DEPOSIT_NO H_CODE_DEPOSIT.PRINT_DEPOSIT_NO%TYPE;
...
BEGIN
   ...
   ***************************/
   IF rec_h_fb_intf_income.FB_GUBUN_CD = '1' THEN  --CMS
      IF LENGTH(TRIM(rec_h_fb_intf_income.FB_CMS)) IS NULL THEN --구분이 cms 이지만 cms 코드가 없는 경우 일반계좌 
         ...
         BEGIN
           ...
           SELECT MAX(dept_code) , MAX(sell_code)
             INTO V_DEPT_CODE, V_SELL_CODE
             FROM H_CODE_DEPOSIT
            WHERE deposit_no = V_DEPOSIT_NO
              AND sell_code = (SELECT  MIN(SELL_CODE)
                                 FROM H_CODE_DEPOSIT
                                WHERE deposit_no = V_DEPOSIT_NO);

            
           SELECT MAX(PRINT_DEPOSIT_NO)
             INTO V_PRINT_DEPOSIT_NO --'-'가들어간 계좌번호를 가져온다 
             FROM H_CODE_DEPOSIT
            WHERE deposit_no = V_DEPOSIT_NO;
         EXCEPTION
           WHEN NO_DATA_FOUND THEN
           AS_ERROR_CODE := '00095';
           AS_ERROR_MSG  := '일반입금(등록안된계좌 입금처리불가)';
           GOTO ERROR_ROUTINE;
         END;
         ...
         BEGIN
           SELECT m.dept_code, 
             ...
             FROM H_SALE_MASTER M,
                  H_CODE_CUST C
            WHERE M.CONTRACT_YN = 'Y'
              AND M.CUST_CODE = C.CUST_CODE
              AND trim(c.cust_name) = trim(rec_h_fb_intf_income.FB_ACCOUNT_NAME)
              AND M.DEPT_CODE||M.SELL_CODE IN (SELECT dept_code||sell_code
                                                 FROM H_CODE_DEPOSIT
                                                WHERE deposit_no = v_deposit_no)
           ...
           AS_DONGHO       := V_DONGHO;
           AS_SEQ          := V_SEQ;
           AS_DEPOSIT_NO   := V_PRINT_DEPOSIT_NO;
           AS_ERROR_CODE := '00094';
           AS_ERROR_MSG  := '일반입금';
           GOTO ERROR_ROUTINE;
         EXCEPTION
      ...
      END IF;
      --CMS 입금 처리
      IF LENGTH(TRIM(rec_h_fb_intf_income.FB_CMS)) <> 16 THEN
         AS_ERROR_CODE := '00023';
        AS_ERROR_MSG  := 'CMS입금(잘못된CMS 코드-'||TRIM(rec_h_fb_intf_income.FB_CMS)|| ')';
        GOTO ERROR_ROUTINE;
      END IF;
      ...
      EXCEPTION
        WHEN NO_DATA_FOUND THEN
        AS_ERROR_CODE := '00022';
        AS_ERROR_MSG  := 'CMS입금(등록안된계좌 입금처리불가)';
        GOTO ERROR_ROUTINE;
      END;
   ...
   ELSIF rec_h_fb_intf_income.FB_GUBUN_CD = '2' THEN  --가상계좌
      ...
      EXCEPTION
        ...
        AS_ERROR_CODE := '00001';
        AS_ERROR_MSG  := '해당가상계좌 H_CODE_V_ACCOUNT 에 등록 안됨/등록안된모계좌 '||'('||rec_h_fb_intf_income.FB_ACCOUNT_NUM||')';
        GOTO ERROR_ROUTINE;
      END;
   ...
   BEGIN
     SELECT SMS_YN  -- 추가 맵핑된동호의 문자 수신여부를 구해서 넘김....입금처리시 문자송신 여부 결정...
       INTO AS_EX_COL1
       FROM H_SALE_MASTER
      WHERE DEPT_CODE = V_DEPT_CODE
        AND SELL_CODE = V_SELL_CODE
   ...
   */----------------입금처리 막습니다....
   SELECT SYSDATE INTO AS_APPLY_DATE FROM DUAL;
   --AS_APPLY_OK     := 'Y';  --입금처리 막았으니까 처리 구분을 'N' 으로 해준다
   AS_APPLY_OK     := 'N';  --입금처리 막았으니까 처리 구분을 'N' 으로 해준다  
   AS_DEPT_CODE    := V_DEPT_CODE;
   AS_SELL_CODE    := V_SELL_CODE;
   AS_DONGHO       := V_DONGHO;
   AS_SEQ          := V_SEQ;
...
END Y_Sp_H_Fbs_Apply_Income_Map;
/
