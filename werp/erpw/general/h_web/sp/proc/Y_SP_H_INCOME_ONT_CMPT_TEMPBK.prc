CREATE OR REPLACE PROCEDURE y_sp_h_income_ont_cmpt_tempbk(as_spec_unq_num  IN   NUMBER,
...
-- LOOP 시작
		LOOP
	-- 일수 및 약정금액을 구한다
			select nvl((agree_date - as_input_date) * -1,0),nvl(AGREE_AMT_TMP,0),agree_date
...
END y_sp_h_income_ont_cmpt_tempbk;
/
