...
BEGIN
   BEGIN
		...
		select substr(code_name, 1, 8)
		  ...
		  from h_code_common
		 where code_div = '25'
		  and  CODE     = '11' ;        -- 분양선수금(아파트)  계정
		if v_acntcode_02_1 is null then
			e_msg := '분양선수금(아파트) 계정 없음 ot01';
			Wk_errflag := -20020;
         GOTO EXIT_ROUTINE;
      end if;
		select substr(code_name, 1, 8)
		  ...
		  from h_code_common
		 where code_div = '25'
		  and  CODE     = '12' ;        -- 분양선수금(상가) 계정
		if v_acntcode_02_2 is null then
			e_msg := '분양선수금(상가) 계정 없음 ot01';
			Wk_errflag := -20020;
         GOTO EXIT_ROUTINE;
      end if;
		select substr(code_name, 1, 8)
		...
		select substr(code_name, 1, 8)
		  ...
		  from h_code_common
		 where code_div = '25'
		  and  CODE     = '23' ;        -- 미지급미용(해약분) 계정
		if v_acntcode_02_4 is null then
			e_msg := '미지급미용(해약분) 계정 없음 ot01';
			Wk_errflag := -20020;
         GOTO EXIT_ROUTINE;
      end if;
		select substr(code_name, 1, 8)
		  ...
		  from h_code_common
		 where code_div = '25'
		  and  CODE     = '88' ;        -- 잡이익(위약금) 계정
		if v_acntcode_02_5 is null then
			e_msg := '잡이익(위약금) 계정 없음 ot01';
			Wk_errflag := -20020;
         GOTO EXIT_ROUTINE;
      end if;
		select substr(code_name, 1, 8)
		  ...
		  from h_code_common
		 where code_div = '25'
		  and  CODE     = '80' ;        -- 단기대여금(기타) 계정
		if v_acntcode_02_6 is null then
			e_msg := '단기대여금(기타) 계정 없음 ot01';
			Wk_errflag := -20020;
         GOTO EXIT_ROUTINE;
      end if;
		select substr(code_name, 1, 8)
...
-- 가전표_디테일을 입력한다
			...
			v_desc := '해약-' || v_dongho_tmp || ' ' || v_cust_name ;
			-- 차변 시작(임대는 차변 없음)
			...
			if v_contract_code = '02' then
				if v_process_code = '01' then
					...
					v_acnt_tmp2 := v_acntcode_02_31;     -- 표준임대보증금
				end if;
			else
				if v_sell_code = '03' then
         ...
         v_amt := v_r_amt * (-1) ;
		   ...
		   e_msg      := '가전표 디테일 생성 실패! ot01 [Line No: 8]'|| C_SEQ || ' 1';
			...
			-- 미지급비용(해약분) 처리
			if v_refund <> 0 then
				v_acnt_tmp2 := v_acntcode_02_4;     -- 미지급비용(해약분)
	         v_amt := v_refund ;
				v_vat := 0;
				v_tot := v_amt;
			...
			-- 잡이익(위약금) 처리
			if v_other <> 0 then
				v_acnt_tmp2 := v_acntcode_02_5;     -- 잡이익(위약금)
	         v_amt := v_other ;
				v_vat := 0;
				v_tot := v_amt;
			...
			-- 단기대여금  : 대여금 공제 처리
			if v_loan <> 0 then
				v_acnt_tmp2 := v_acntcode_02_6;     -- 단기대여금
	         v_amt := v_loan ;
				v_vat := 0;
				v_tot := v_amt;
			...
			-- 임대미수금  : 임대료 공제 처리
			if v_lease <> 0 then
				v_acnt_tmp2 := v_acntcode_02_7;     -- 임대미수금
	         v_amt := v_lease ;
				v_vat := 0;
				v_tot := v_amt;
			   e_msg      := '가전표 디테일 생성 실패! ot01 [Line No: 12]'|| C_SEQ || ' 1';
