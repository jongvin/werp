...
include file ="../../../comm_function/conn_tr_pool.jsp"%><%
     GauceDataSet dSet = req.getGauceDataSet("h_contract_lease_detail_6tr"); gpstatement.gp_dataset = dSet;
	...
	if(rows.getJobType() == GauceDataRow.TB_JOB_INSERT){
		String sSql = " INSERT INTO H_LEASE_DISCOUNT_RATE ( dept_code," + 
                    "sell_code," + 
                    "cont_date," + 
                    "cont_seq," + 
                    "rate_kind," + 
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "update h_lease_discount_rate set " + 
                            "dept_code=?,  " + 
                            "sell_code=?,  " + 
                            "cont_date=?,  " + 
                            "cont_seq=?,  " + 
 ...
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
      ...
      gpstatement.bindColumn(13, idx_cont_date);
      gpstatement.bindColumn(14, idx_cont_seq);
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "delete from h_lease_discount_rate where dept_code=? " + 
                            "                 and sell_code=? " +
                            "                 and cont_date=? " +
                            "                 and cont_seq=? " +
                            "                 and rate_kind=? " +
