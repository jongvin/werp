...
include file ="../../../comm_function/conn_tr_pool.jsp"%><%
     GauceDataSet dSet = req.getGauceDataSet("h_leas_receipt_detail_3tr"); gpstatement.gp_dataset = dSet;
   	...
   	int idx_guarantee_receipt_date = dSet.indexOfColumn("guarantee_receipt_date");
   ...
   int  rowCnt = dSet.getDataRowCnt();
	...
	if(rows.getJobType() == GauceDataRow.TB_JOB_INSERT){
		String sSql = " INSERT INTO h_leas_guarantee_income ( dept_code," + 
                    "sell_code," + 
                    "dongho," + 
                    "seq," + 
                    "contract_date," +
      ...
      gpstatement.bindColumn(8, idx_guarantee_receipt_date);
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "update h_leas_guarantee_income set " + 
                            "dept_code=?,  " + 
                            "sell_code=?,  " + 
                            "dongho=?,  " + 
                            "seq=?,  " + 
 ...
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
      ...
      gpstatement.bindColumn(24, idx_contract_date);
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "delete from h_leas_guarantee_income where dept_code=? " + 
                            "            and sell_code=? " +
                            "            and dongho=? " +
                            "            and seq=? " +
                            "            and contract_date=? " +
