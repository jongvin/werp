<%@ page session="true" import="com.gauce.*,com.gauce.io.*,com.gauce.common.*,com.gauce.log.*,com.gauce.db.*,java.sql.*"%><%@
 include file ="../../../comm_function/conn_tr.jsp"%><%
     GauceDataSet dSet = req.getGauceDataSet("g_mis_dept_site_result_1tr");
     gpstatement.gp_dataset = dSet;
   	...
   	int idx_pr_div = dSet.indexOfColumn("pr_div");
	...
	if(rows.getJobType() == GauceDataRow.TB_JOB_INSERT){
		String sSql = " INSERT INTO g_mis_dept_site_result ( work_year," + 
                    "spec_no_seq," + 
                    "no_seq," + 
                    "dept_name," + 
                    "pr_div," + 
      ...
      gpstatement.bindColumn(4, idx_dept_name);
      gpstatement.bindColumn(5, idx_pr_div);
      gpstatement.bindColumn(6, idx_m_1);
      gpstatement.bindColumn(7, idx_m_2);
      gpstatement.bindColumn(8, idx_m_3);
      gpstatement.bindColumn(9, idx_m_4);
      gpstatement.bindColumn(10, idx_m_5);
      gpstatement.bindColumn(11, idx_m_6);
      gpstatement.bindColumn(12, idx_m_7);
      gpstatement.bindColumn(13, idx_m_8);
      gpstatement.bindColumn(14, idx_m_9);
      gpstatement.bindColumn(15, idx_m_10);
      gpstatement.bindColumn(16, idx_m_11);
      gpstatement.bindColumn(17, idx_m_12);
      stmt.executeUpdate();
      stmt.close();
 
   }else if(rows.getJobType() ==  GauceDataRow.TB_JOB_UPDATE){
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "update g_mis_dept_site_result set " + 
                            "work_year=?,  " + 
                            "spec_no_seq=?,  " + 
                            "no_seq=?,  " + 
                            "dept_name=?,  " + 
 ...
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
      gpstatement.bindColumn(18, idx_work_year);
      gpstatement.bindColumn(19, idx_spec_no_seq);
      stmt.executeUpdate();
      stmt.close();
   }else if(rows.getJobType() ==  GauceDataRow.TB_JOB_DELETE){
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "delete from g_mis_dept_site_result where work_year=? and spec_no_seq=? "; 
      stmt = conn.prepareStatement(sSql); 
      gpstatement.gp_stmt = stmt;
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
      gpstatement.bindColumn(1, idx_work_year);
