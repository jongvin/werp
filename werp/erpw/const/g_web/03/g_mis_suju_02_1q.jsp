<%@ page session="true" contentType="text/html;charset=EUC_KR" import="com.gauce.*,com.gauce.io.*,com.gauce.common.*,com.gauce.log.*,com.gauce.db.*,java.sql.*"%><%@
 ...
 //---------------------------------------------------------- 
    ...
    String query = "  " + 
                   ...
                   " from (                                                " + 
                   "     SELECT  trunc(a.recive_date,'mm') mm,nvl(a.business_method,'00') gubun, nvl(b.detail_name,' ') detail_name,  count(nvl(a.business_method,1)) cnt " + 
                   "        from r_proj a, r_code_etc_detail b,r_proj_view_business_form c   " + 
                   "        where b.class_tag(+) = '003'                                                                                                                " + 
                   "          and a.business_method = b.etc_code (+)                                                                                                     " + 
                   "          and a.proj_unq_key = c.proj_unq_key                                                                                                    " + 
                   "          and a.step = c.step                                                                                                   " + 
                   "          and a.recive_date >= '" + arg_work_year + "'                                                                                     " + 
                   "          and a.recive_date < add_months('" + arg_work_year + "',12)      " + 
                   "        group by trunc(a.recive_date,'mm'),nvl(a.business_method,'00'),nvl(b.detail_name,' ')                                         " + 
                   "       ) a,                                                                                                                                        " + 
                   "    (                                                                                                                                              " + 
