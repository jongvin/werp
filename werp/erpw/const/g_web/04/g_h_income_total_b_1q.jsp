<%@ page session="true" import="com.gauce.*,com.gauce.io.*,com.gauce.common.*,com.gauce.log.*,com.gauce.db.*,java.sql.*"%><%@
 ...
 //---------------------------------------------------------- 
     ...
     dSet.addDataColumn(new GauceDataColumn("sunnap_incomeamt",GauceDataColumn.TB_DECIMAL,18,0));
     dSet.addDataColumn(new GauceDataColumn("dore_incomeamt",GauceDataColumn.TB_DECIMAL,18,0));
    String query = "" + 
     ...
     "     TRUNC(NVL(agreemidore.midore_amt,0)/1000000,0) midore_amt," + 
     "     TRUNC(NVL(agreemidore.dore_amt,0)/1000000,0) dore_amt," + 
     "     TRUNC(NVL(agreemidore.midore_incomeamt,0)/1000000,0) sunnap_incomeamt," + 
     "     TRUNC(NVL(agreemidore.dore_incomeamt,0)/1000000,0) dore_incomeamt" + 
     "  FROM" + 
     ...
     "						(" + 
     "						SELECT MASTER.dept_code, MASTER.sell_code, " + 
     "                     SUM(CASE WHEN agree.agree_date <= '"+arg_work_date+"' THEN NVL(income.r_amt,0) ELSE 0 END ) dore_incomeamt," + 
     "                     SUM(CASE WHEN agree.agree_date > '"+arg_work_date+"' THEN NVL(income.r_amt,0) ELSE 0 END ) midore_incomeamt	,			 " + 
     ...
     "	 AND sale.dept_code = agreemidore.dept_code(+)" + 
     "    AND sale.sell_code = agreemidore.sell_code(+)" + 
