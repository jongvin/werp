...
"		 						   (select max(dept_code) dept_code,nvl(sum(decode(extablish_tag,'2',supply_amt+vat_amt,0)),0) supply_amt " +
"		    							from r_contract_time_extablished " +
"		   						  where to_char(issue_date,'yyyy-mm') <= '" + arg_yyyymm + "' " +
"									  group by dept_code ) c, " +
"		 							(select max(dept_code) dept_code,nvl(sum(decode(extablish_tag,'1',supply_amt+vat_amt,0)),0) pre_amt, " +
"				   						  nvl(sum(decode(extablish_tag,'2',supply_amt+vat_amt,0)),0) tot_rcv_amt,  " +
"				   						  nvl(sum(decode(extablish_tag,'2',decode(to_char(received_date,'YYYY'),substr('" + arg_yyyymm + "',0,4),supply_amt+vat_amt,0),0)),0) this_year_amt, " +
"				   						  nvl(sum(decode(extablish_tag,'2',decode(to_char(received_date,'YYYY'),substr('" + arg_yyyymm + "',0,4),0,supply_amt+vat_amt),0)),0) pre_year_amt, " +
"				   						  nvl(sum(decode(extablish_tag,'2',decode(to_char(received_date,'YYYY-MM'),'" + arg_yyyymm + "',supply_amt+vat_amt,0),0)),0) this_mm_amt " +
