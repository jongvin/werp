...
include file="../../../comm_function/conn_q_pool.jsp"%><%
 // 다음 문장은 수정 하시요 (파라메터 갯수만큼 (없으면 //로 막으세요)//
	 ...
	 String arg_masterdept = req.getParameter("arg_masterdept");
	 String arg_new = req.getParameter("arg_new");
	 String arg_receive = req.getParameter("arg_receive");
	 String arg_pq = req.getParameter("arg_pq");
	 String arg_const = req.getParameter("arg_const");
	 String arg_constkind = req.getParameter("arg_constkind");
	 dSet.addDataColumn(new GauceDataColumn("dept_name",GauceDataColumn.TB_STRING,50));
	 
	 dSet.addDataColumn(new GauceDataColumn("masterdept_name",GauceDataColumn.TB_STRING,10));
	 ...
	 dSet.addDataColumn(new GauceDataColumn("constkind_name",GauceDataColumn.TB_STRING,50));
...
"            decode (m.masterdept_tag , '1', '영업1팀', '2', '영업2팀', '미지정' ) masterdept_name,"   +
"            DECODE (m.new_tag, 'Y', '신규', '이월') new_name,"   +
"            nvl((SELECT child_name FROM z_code_etc_child WHERE class_tag = '011' AND etc_code = m.receive_tag),'미지정') receive_name,"   +
"            nvl((SELECT child_name FROM z_code_etc_child WHERE class_tag = '010' AND etc_code = m.pq_tag),'미지정') pq_name,"   +
"            nvl((SELECT child_name FROM z_code_etc_child WHERE class_tag = '008' AND etc_code = m.const_tag),'미지정') const_name,"   +
"            nvl((SELECT child_name FROM z_code_etc_child WHERE class_tag = '007' AND etc_code = m.constkind_tag),'미지정') constkind_name,"   +
...
"        AND d.YEAR = '"+arg_year+"'"   +
"        AND nvl(m.masterdept_tag, ' ')   like decode('"+arg_masterdept+"', 'ALL', '%', '"+arg_masterdept+"')"   +
"        AND nvl(trim(m.new_tag), ' ')          like decode('"+arg_new+"', 'ALL', '%', '"+arg_new+"')"   +
"        AND nvl(m.receive_tag, ' ')      like decode('"+arg_receive+"', 'ALL', '%', '"+arg_receive+"')"   +
"        AND nvl(m.pq_tag, ' ')           like decode('"+arg_pq+"', 'ALL', '%', '"+arg_pq+"')"   +
"        AND nvl(m.const_tag, ' ')        like decode('"+arg_const+"', 'ALL', '%', '"+arg_const+"')"   +
"        AND nvl(m.constkind_tag, ' ')    like decode('"+arg_constkind+"', 'ALL', '%', '"+arg_constkind+"')"   +
