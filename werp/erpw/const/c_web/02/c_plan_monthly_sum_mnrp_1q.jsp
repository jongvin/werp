...
include file="../../../comm_function/conn_q_pool.jsp"%><%
 // 다음 문장은 수정 하시요 (파라메터 갯수만큼 (없으면 //로 막으세요)//
	 ...
	 dSet.addDataColumn(new GauceDataColumn("masterdept_tag",GauceDataColumn.TB_STRING,10));
	 dSet.addDataColumn(new GauceDataColumn("new_tag",GauceDataColumn.TB_STRING,10));
	 dSet.addDataColumn(new GauceDataColumn("receive_tag",GauceDataColumn.TB_STRING,10));
    ...
    String query ="  SELECT   m.masterdept_tag, m.new_tag, m.receive_tag, m.pq_tag, "+
"           decode (m.masterdept_tag , '1', '영업1팀', '2', '영업2팀', '미지정' ) masterdept_name,"   +
"           DECODE (m.new_tag, 'Y', '신규', '이월') new_name,  "   +
"           (SELECT child_name FROM z_code_etc_child WHERE class_tag = '011' AND etc_code = m.receive_tag) receive_name,"   +
"           (SELECT child_name FROM z_code_etc_child WHERE class_tag = '010' AND etc_code = m.pq_tag) pq_name,"   +
...
"       AND d.YEAR = '"+arg_year+"'"   +
"  GROUP BY m.masterdept_tag, m.new_tag, m.receive_tag, m.pq_tag, d.line_no, line_desc, line_desc2"   +
"  ORDER BY m.masterdept_tag, m.new_tag, m.receive_tag, m.pq_tag, d.line_no"  ; 
%><%@ 
