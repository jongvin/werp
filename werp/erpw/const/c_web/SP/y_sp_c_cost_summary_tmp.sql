...
/* 작성일 : 2005.05.26                                                           */
/* ============================================================================= */
CREATE OR REPLACE PROCEDURE y_sp_c_cost_summary_tmp(as_dept_code    IN   VARCHAR2,
                                                adt_yymm        IN   DATE ) IS
-------------------------------------------------------------
-- 변수선언
-------------------------------------------------------------
...
-- 외주비 자료 집계        
						  SELECT c.dept_code,
						         c.spec_unq_num, 
						         nvl(sum(decode(c.unit,'식',0,b.tm_prgs_qty)),0) cost_qty, 
						         nvl(sum(decode(e.order_class,'2',b.tm_prgs_amt,0)),0) cost_mat_amt,
						         0 cost_lab_amt,
						         0 cost_equ_amt,
						         nvl(sum(decode(e.order_class,'1',b.tm_prgs_amt,0)),0) cost_sub_amt,
                           0 cost_exp_amt, 
									nvl(sum(decode(c.unit,'식',0,b.tm_prgs_qty)),0) result_qty,
