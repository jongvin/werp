 ...
 -- 당월분 원가 집계 테이블 삭제..
      ...
      IF C_CHK = 'Y' THEN           -- 준공추정수량 (현실행수량 과 전월의 실행수량이 같으면 전월의 추정수량이고 같지않으면 현실행수량) 
         INSERT INTO c_spec_const_detail
              select as_dept_code,adt_yymm,a.spec_no_seq,a.spec_unq_num,
                     ...
                     NVL(b.ls_cost_equ_amt,0)       + NVL(b.cost_equ_amt,0),
                     NVL(b.ls_cost_exp_amt,0)       + NVL(b.cost_exp_amt,0),      
                     decode(sign((NVL(b.ls_cost_rate,0) + NVL(b.cost_rate,0)) - 999),1,999,NVL(b.ls_cost_rate,0) + NVL(b.cost_rate,0)),
                     case when a.qty = b.qty then b.sup_qty else a.qty end,  -- 준공 추정 수량 
                     Decode(b.sup_unit_fix,'T',b.sup_unit_amt,a.price),      -- 준공 추정 단가 
                     b.sup_unit_fix,
                     a.qty,
                     a.price,
   ...
   --위에서 복사된 당월의 단가고정및 금액기준단가 고정 당월에 다시 보관   
		...
		UPDATE c_spec_const_detail  a
		  SET (a.sup_unit_fix,a.mat_unit_fix) = 
            ( SELECT b.sup_unit_fix,b.mat_unit_fix
     ...
     -- 준공추정단가 복귀 
		UPDATE c_spec_const_detail  a
		  SET (a.sup_unit_amt) = 
            ( SELECT b.cost_mat_amt
							  from c_spec_const_cal_tmp b
							 where a.spec_unq_num = b.spec_unq_num 
                        and b.dept_code = as_dept_code
								and b.yymm         = adt_yymm 
								and b.detail_unq_num = C_UNQ_NUMBER 
								and (b.sup_unit_fix = 'T' or b.mat_unit_fix = 'T'))
		 WHERE a.dept_code = as_dept_code
			AND a.yymm      = adt_yymm
