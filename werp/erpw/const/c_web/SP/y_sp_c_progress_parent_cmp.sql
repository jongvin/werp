...
--  하위을 집계하여 상위에 반영 보할율과 실행금액 단 (건축일겨우에만 상위집계)
       update c_progress_parent a set (a.plan_per,a.plan_amt,a.real_per,a.real_amt) = 
                    (select sum(b.plan_mm_per),sum(b.plan_mm_amt),sum(b.real_mm_per),sum(b.real_mm_amt)
          ...
          where  a.wbs_code = 'A'
            ...
            and  exists (select sum(b.plan_mm_per),sum(b.plan_mm_amt)
                         from c_progress_detail b
                         where b.dept_code = as_dept_code
                           and b.chg_no_seq = ai_chg_no_seq
                           and a.dept_code = b.dept_code
