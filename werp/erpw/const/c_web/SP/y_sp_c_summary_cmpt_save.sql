...
BEGIN
  BEGIN
		...
		UPDATE c_spec_const_parent  a
		  SET	 a.cost_rate       = decode(a.amt,0,0,decode(sign(trunc((nvl(a.cost_amt,0) / a.amt ) * 100,2) - 999),
																				1,999,trunc((nvl(a.cost_amt,0) / a.amt ) * 100,2))),
				 a.cnt_result_rate =	decode(a.cnt_amt,0,0,decode(sign(trunc((nvl(a.cnt_result_amt,0) / a.cnt_amt ) * 100,2) - 999),
																				1,999,trunc((nvl(a.cnt_result_amt,0) / a.cnt_amt ) * 100,2))),
				 a.result_rate     =	decode(a.amt,0,0,decode(sign(trunc((nvl(a.result_amt,0) / a.amt ) * 100,2) - 999),
																				1,999,trunc((nvl(a.result_amt,0) / a.amt ) * 100,2))),
				 a.ls_cost_rate	 =	decode(a.amt,0,0,decode(sign(trunc((nvl(a.ls_cost_amt,0) / a.amt ) * 100,2) - 999),
																				1,999,trunc((nvl(a.ls_cost_amt,0) / a.amt ) * 100,2))),
				 a.ls_cnt_result_rate = decode(a.cnt_amt,0,0,decode(sign(trunc((nvl(a.ls_cnt_result_amt,0) / a.cnt_amt ) * 100,2) - 999),
																				1,999,trunc((nvl(a.ls_cnt_result_amt,0) / a.cnt_amt ) * 100,2))),
				 a.ls_result_rate = 	decode(a.amt,0,0,decode(sign(trunc((nvl(a.ls_result_amt,0) / a.amt ) * 100,2) - 999),
																				1,999,trunc((nvl(a.ls_result_amt,0) / a.amt ) * 100,2)))
