...
BEGIN
   lb_yn          := FALSE;
	...
	SELECT spec_no_seq,name,ssize
	  INTO li_key,ls_detail_name,ls_size
     FROM y_budget_detail
	 WHERE dept_code = as_dept_code
		AND spec_unq_num = ai_spec_unq_num;

	SELECT parent_sum_code,llevel
   ...
   ELSE
      IF li_level > 1 THEN
         ls_name := ls_name || '/' || ltrim(ls_detail_name) || '-' || ltrim(ls_size);
      ELSE
         ls_name := '';
      END IF;
   END IF;
