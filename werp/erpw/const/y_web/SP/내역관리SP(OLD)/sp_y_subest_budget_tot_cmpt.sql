/* ============================================================================= */
/* 함수명 : sp_y_subest_budget_tot_cmpt                                          */
/* 기  능 : 외주견적금액조정시 입력시 상위로 재계산한다.(전체일괄)               */
/* ----------------------------------------------------------------------------- */
...
/* 작성일 : 2001.09.04                                                           */
/* ============================================================================= */
CREATE OR REPLACE PROCEDURE sp_y_subest_budget_tot_cmpt(as_proj_code    IN   VARCHAR2,
                                                        ai_no           IN   INTEGER,
...
-- User Define Error 
   ...
   C_CTRL_AMT          Y_SUBEST_BUDGET_DETAIL.CTRL_AMT%TYPE;           -- 조정금액
   C_CTRL_MAT_AMT      Y_SUBEST_BUDGET_DETAIL.CTRL_MAT_AMT%TYPE;       -- 조정자재비
   C_CTRL_LAB_AMT      Y_SUBEST_BUDGET_DETAIL.CTRL_LAB_AMT%TYPE;       -- 조정노무비
   C_CTRL_EXP_AMT      Y_SUBEST_BUDGET_DETAIL.CTRL_EXP_AMT%TYPE;       -- 조정경비
   C_CTRL_EQU_AMT      Y_SUBEST_BUDGET_DETAIL.CTRL_EQU_AMT%TYPE;       -- 조정중기비
   C_CTRL_SUB_AMT      Y_SUBEST_BUDGET_DETAIL.CTRL_SUB_AMT%TYPE;       -- 조정외주비
   C_LEVEL             NUMBER(20,5);  -- 
   C_CNT               NUMBER(20,5);  -- 
  ...
  BEGIN
	...
	LOOP
      ...
      SELECT NVL(SUM(AMT),0),NVL(SUM(MAT_AMT),0),NVL(SUM(LAB_AMT),0),
             ...
             NVL(SUM(CTRL_AMT),0),NVL(SUM(CTRL_MAT_AMT),0),NVL(SUM(CTRL_LAB_AMT),0),
        ...
        INTO C_AMT,C_MAT_AMT,C_LAB_AMT,C_EXP_AMT,C_EQU_AMT,C_SUB_AMT,
             C_CTRL_AMT,C_CTRL_MAT_AMT,C_CTRL_LAB_AMT,C_CTRL_EXP_AMT,C_CTRL_EQU_AMT,C_CTRL_SUB_AMT
      ...
      UPDATE Y_SUBEST_BUDGET_SUMMARY
         SET AMT          = NVL(C_AMT,0),
             ...
             CTRL_MAT_AMT = NVL(C_CTRL_MAT_AMT,0),
             CTRL_LAB_AMT = NVL(C_CTRL_LAB_AMT,0),
             CTRL_EXP_AMT = NVL(C_CTRL_EXP_AMT,0),
             CTRL_EQU_AMT = NVL(C_CTRL_EQU_AMT,0),
             CTRL_SUB_AMT = NVL(C_CTRL_SUB_AMT,0)
       WHERE proj_code   = as_proj_code   AND
             no          = ai_no          AND
             estimate_no = ai_estimate_no AND
             custcode    = as_custcode    AND
   ...
   IF C_LEVEL <> 1 THEN
   	LOOP
         SELECT NVL(SUM(a.AMT),0),NVL(SUM(a.MAT_AMT),0),NVL(SUM(a.LAB_AMT),0),
                ...
                NVL(SUM(a.CTRL_AMT),0),NVL(SUM(a.CTRL_MAT_AMT),0),NVL(SUM(a.CTRL_LAB_AMT),0),
...
END sp_y_subest_budget_tot_cmpt;
