...
include file ="../../../comm_function/conn_tr_pool.jsp"%><%
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "update m_input_detail set " + 
                            ...
                            "est_unq_num=?,  " + 
                            "approval_unq_num=?,  " + 
                            "bigo=?,noout_qty=?,tmat_unq_num=?  where dept_code=? " +  
                            " and yymmdd=?  " + 
                            " and seq=?  " + 
                            " and input_unq_num=?  " ;
      stmt = conn.prepareStatement(sSql); 
 ...
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
      ...
      gpstatement.bindColumn(29, idx_yymmdd);
      gpstatement.bindColumn(30, idx_seq);
      gpstatement.bindColumn(31, idx_input_unq_num);
      stmt.executeUpdate();
      stmt.close();
      ...
      stmt = conn.prepareStatement(sSql); 
      gpstatement.gp_stmt = stmt;
      gpstatement.bindColumn(1, idx_rcomp_qty);
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "delete from m_input_detail where dept_code=? " +
                            " and yymmdd=?  " + 
                            " and seq=?  " + 
                            " and input_unq_num=?  " ;
      stmt = conn.prepareStatement(sSql); 
 ...
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
      ...
      stmt = conn.prepareStatement(sSql); 
      gpstatement.gp_stmt = stmt;
      gpstatement.bindColumn(1, idx_comp_qty);
      gpstatement.bindColumn(2, idx_dept_code);
      gpstatement.bindColumn(3, idx_approval_unq_num);
      stmt.executeUpdate();
      stmt.close();
