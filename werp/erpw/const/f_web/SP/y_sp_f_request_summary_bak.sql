...
CURSOR MAT_CUR IS
SELECT a.vender_code, c.vender_name, 
       decode(substr(a.inputtitle,1,2), '철근', 1, 0)   chol,  
       decode(substr(a.inputtitle,1,3), '레미콘', 1, 0) remicon,  
       nvl(sum(b.amt),0),nvl(sum(b.vatamt),0)
  FROM m_input  a,
       m_input_detail b,
		 m_code_collaborator c
 WHERE a.dept_code   = b.dept_code       and 
       a.yymmdd      = b.yymmdd          and  
       a.seq         = b.seq             and  
       a.vender_code = c.vender_code (+) and
		 a.dept_code    = as_dept_code AND
       to_char(a.yymmdd,'YYYY.MM') = to_char(ad_date,'YYYY.MM') AND
		 a.inouttypecode = 'A'
GROUP BY a.vender_code, c.vender_name, 
       decode(substr(a.inputtitle,1,2), '철근', 1, 0),  
       decode(substr(a.inputtitle,1,3), '레미콘', 1, 0) 
...
-- User Define Error 
   UserErr         EXCEPTION;                  -- Select Data Not Found
	C_DATE			 DATE;
   C_CODE			 VARCHAR2(13);
   C_VATCODE       VARCHAR2(2);
   C_NAME          VARCHAR2(50);
   C_ACNT_CODE     VARCHAR2(50);
   C_CNT           NUMBER(10,3);
   C_AMT           NUMBER(20,3);
   C_FREE_RATE     NUMBER(10,3);
...
BEGIN
   BEGIN
		...
		C_SEQ := C_SEQ + 1;
	-- 외주비 집계
	   e_msg      := '자금청구 외주비 집계작업 실패! [Line No: 611]';
		IF as_s_tag = 'Y' THEN	
			OPEN	SUB_CUR;
			LOOP
			  ...
			  e_msg      := '자금청구 외주비 집계작업 실패! [Line No: 612]';
			  SELECT b.SBCR_CODE
				 INTO v_vender_code
				 FROM S_PAY a,   
						S_CN_LIST b  
			   ...
			   e_msg      := '자금청구 외주비 집계작업 실패! [Line No: 613]';
				...
				if C_CODE <> NULL THEN
				   e_msg      := '자금청구 외주비 집계작업 실패! [Line No: 614]';
					select count(*)
					  into C_CNT
					...
					IF C_CNT < 1 THEN
					   e_msg      := '자금청구 외주비 집계작업 실패! [Line No: 615]';
			   ...
			   e_msg      := '자금청구 외주비 집계작업 실패! [Line No: 616]';
				select child_name
				  into C_ACNT_CODE
			   ...
			   e_msg      := '자금청구 외주비 집계작업 실패! [Line No: 617]';
				INSERT INTO F_REQUEST  
				  SELECT a.DEPT_CODE,ad_date,C_SEQ,'S',v_sbc_name || ' ' || to_char(v_yymm,'yyyy.mm') || '월 외주비',a.ORDER_NUMBER,   
							C_CODE,c.SBCR_NAME,decode(a.tm_vat,0,'',C_VATCODE),a.TM_PAY,a.TM_VAT,   
							C_ACNT_CODE,'3','1','2',decode(a.tm_vat,0,null,ad_date),'','',0,0,NULL,0, a.check_dt  
					 FROM S_PAY a,   
							S_CN_LIST b,   
							S_SBCR  c
					WHERE ( b.DEPT_CODE    = a.DEPT_CODE ) and  
				   ...
				   e_msg      := '자금청구 외주비 집계작업 실패! [Line No: 618]';
					C_SEQ := C_SEQ + 1;
			END LOOP;
			CLOSE SUB_CUR;
		END IF;	
	-- 자재비 집계
	   e_msg      := '자금청구 자재비 집계작업 실패! [Line No: 62]';
		IF as_m_tag = 'Y' THEN
			OPEN	MAT_CUR;
			LOOP
				FETCH MAT_CUR INTO v_vender_code, v_vender_name, v_chol, v_remicon, v_amt,v_vat;
				EXIT WHEN MAT_CUR%NOTFOUND;
	
			   e_msg      := '자금청구 자재비 집계작업 실패! [Line No: 621]';
				select businessman_number
				  into C_CODE
				  from m_code_collaborator
				 where vender_code = v_vender_code;
	
			   e_msg      := '자금청구 자재비 집계작업 실패! [Line No: 622]';
				select count(*)
				  into C_CNT
				  from z_code_cust_code
				 where cust_code = C_CODE;
	
				IF C_CNT < 1 AND C_CODE <> NULL THEN
				   e_msg      := '자금청구 자재비 집계작업 실패! [Line No: 623]';
					insert into z_code_cust_code
			   ...
			   e_msg      := '자금청구 자재비 집계작업 실패! [Line No: 624]';
				select MAX(name)
				  into C_NAME
				  from m_input_detail
				 where dept_code    = as_dept_code AND
						 ...
						 amt = ( SELECT max(amt)
									 ...
									 WHERE dept_code    = as_dept_code AND
											 ...
											 to_char(yymmdd,'YYYY.MM') = to_char(ad_date,'YYYY.MM') AND
											 inouttypecode = 'A');
			   e_msg      := '자금청구 자재비 집계작업 실패! [Line No: 625]';
				select count(*)
				  into C_CNT
				  from m_input_detail
				 WHERE dept_code    = as_dept_code AND
...
-- etc_code    MC:철근   MR:레미콘     MZ:기타자재
				...
				END IF;						
				IF v_remicon = 1 then
				   e_msg      := '자금청구 자재비 집계작업 실패! [Line No: 627]';
					select child_name
					  into C_ACNT_CODE
					  from z_code_etc_child
					 where class_tag = '980'
						and etc_code = 'MR';
				END IF;						
				IF v_chol + v_remicon = 0 then
				   e_msg      := '자금청구 자재비 집계작업 실패! [Line No: 628]';
					select child_name
					  into C_ACNT_CODE
					  from z_code_etc_child
					 where class_tag = '980'
			   ...
			   e_msg      := '자금청구 자재비 집계작업 실패! [Line No: 629]';
				INSERT INTO F_REQUEST  
					VALUES ( as_dept_code,ad_date,C_SEQ,'M',C_NAME,0,   
								C_CODE,v_vender_name,decode(v_vat,0,'',C_VATCODE),v_amt,v_vat,   
								C_ACNT_CODE,'1','1','2',decode(v_vat,0,null,ad_date),'','',0,0,NULL,0, null    );
		...
		END IF;	
	-- 노무비 집계
	   e_msg      := '자금청구 노무비 집계작업 실패! [Line No: 63]';
		IF as_l_tag = 'Y' THEN
			...
			IF C_AMT <> 0 THEN
				...
				INSERT INTO F_REQUEST  
				  VALUES ( as_dept_code,ad_date,C_SEQ,'L',to_char(ad_date,'YYYY.MM') || '월 노임',0,   
							'','','',C_AMT,0,   
							C_ACNT_CODE,'3','1','2',null,'','',0,0,NULL,0, null   );
			
				C_SEQ := C_SEQ + 1;
			END IF;
		END IF;	
	-- 장비비 집계
	   e_msg      := '자금청구 장비비 집계작업 실패! [Line No: 64]';
		IF as_q_tag = 'Y' THEN
			OPEN	EQU_CUR;
			LOOP
				...
				INSERT INTO F_REQUEST  
					VALUES ( as_dept_code,ad_date,C_SEQ,'E',v_vender_name || ' ' || to_char(ad_date,'yyyy.mm') || '월 장비비',0,   
								v_regist_no,v_vender_name,decode(v_vat,0,'',C_VATCODE),v_amt,v_vat,   
								C_ACNT_CODE,'3','1','2',decode(v_vat,0,null,ad_date),'','',0,0,NULL,0, null    );
