...
/* 기  능 : 자동전표생성마스터(노무비, 외주비 제외한 모든것)                     */
/*          노무/외주비는 y_sp_f_slip_list_ls 에서 처리한다.			            */
/* ----------------------------------------------------------------------------- */
...
BEGIN
   BEGIN
	...
	select COUNT(*)
	  into C_CNT
	  from erpc.sm_code_cust
	 where cust_code = as_dept_code;
	...
	IF C_CNT < 1 THEN
		...
		e_msg      := '거래처-부서코드 생성 실패! [Line No: -88]';
		INSERT INTO ERPC.SM_CODE_CUST  
			  VALUES ( as_dept_code,'Y','Y','003',v_dept_name,'','','',null,   
...
--			VALUES ( adt_slip_date,C_SEQ,C_SEQ,'1',as_dept_code,C_KIND_CODE,'002',as_slip_name,sysdate,'1',v_cust_code,   
...
-- 가전표_디테일을 입력한다 (노무비는 무조건 부가세와 거래처코드가 없는것으로 한다.)
		   e_msg      := '가전표 마스타 생성 실패! [Line No: 1]'|| C_SEQ || v_res_type;
			IF v_vatcode is null and v_cust_code is not null THEN
				Y_SP_F_SLIP_DETAIL1(as_dept_code,adt_from_date,adt_to_date,adt_slip_date,v_cust_code,
				           v_res_type,C_KIND_CODE,C_SEQ, as_slip_name);
			END IF;
			
		   e_msg      := '가전표 마스타 생성 실패! [Line No: 2]' || C_SEQ;
			IF v_vatcode is null and v_cust_code is null THEN
				Y_SP_F_SLIP_DETAIL2(as_dept_code,adt_from_date,adt_to_date,adt_slip_date,v_res_type,
				           C_KIND_CODE,C_SEQ, as_slip_name);
			END IF;
			
		   e_msg      := '가전표 마스타 생성 실패! [Line No: 3]';
			IF v_vatcode is not null THEN
				Y_SP_F_SLIP_DETAIL3(as_dept_code,v_rqst_date,v_seq,adt_slip_date,C_KIND_CODE,C_SEQ, as_slip_name);
			END IF;
...
-- 외주/노무 가전표 생성
		...
		Y_SP_F_SLIP_LIST_LS(as_dept_code, adt_from_date, adt_to_date, adt_slip_date, as_slip_name);
