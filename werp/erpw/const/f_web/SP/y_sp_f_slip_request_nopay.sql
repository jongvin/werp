 /* ============================================================================ */
/* 함수명 : y_sp_f_slip_request_nopay(CJ개발)                                    */
/* 기  능 : 자금청구-자금청구전표 발행(미지불)                                   */
/* ----------------------------------------------------------------------------- */
...
/* 작성일 : 2006.04.12                                                           */
/* ============================================================================= */
CREATE OR REPLACE PROCEDURE y_sp_f_slip_request_nopay (as_dept_code    IN   VARCHAR2,
																	  as_date         IN   DATE,
                                                     as_user         IN   VARCHAR2,
...
SELECT amt,vat,receipt_date,rqst_detail,res_class,dr_class,acntcode,
		 replace(cust_code,'-',''),nvl(cash_amt,0),vatcode,pay_date,income_type,nvl(income_amt,0),nvl(civi_amt,0)
  from f_request
...
SELECT amt,vat,receipt_date,rqst_detail,res_class,dr_class,acntcode,
		 replace(cust_code,'-',''),income_type,nvl(income_amt,0),nvl(civi_amt,0),pay_date
  from f_request
 where dept_code = as_dept_code
	and trunc(rqst_date,'MM')= as_date
	and res_class = 'L'
...
-- 부문을 구한다.
	...
	-- 전표내역을 만들기 위한 커서
			OPEN	DETAIL_CUR;
			LOOP
				FETCH DETAIL_CUR INTO V_AMT,V_VAT,V_YYMMDD,V_CONT,V_RES_CLASS,V_DR_CLASS,V_ACNTCODE,V_CUST_CODE,V_CASH_AMT,V_VATCODE,V_CHECK_DT,
				...
				IrBody.PAY_CON_CASH      := V_AMT + V_VAT;       -- 현금
				IrBody.PAY_CON_BILL      := 0;       -- 어음
				IrBody.PAY_CON_BILL_DAYS := 0;   -- 어음만기일수
...
--				IrBody.RESET_SLIP_IDSEQ  := C_SLIP_IDSEQ;
	...
	-- 전표내역을 만들기 위한 커서
			OPEN	DETAIL_CUR1;
			LOOP
				FETCH DETAIL_CUR1 INTO V_AMT,V_VAT,V_YYMMDD,V_CONT,V_RES_CLASS,V_DR_CLASS,V_ACNTCODE,V_CUST_CODE,V_INCOME_TYPE,V_INCOME_AMT,V_CIVI_AMT,V_CHECK_DT;
...
--              e_msg      := '전도금전표생성 실패! [Line No: 2]' || C_CUST_SEQ || C_CUST_NAME || C_AMT;
--              Wk_errflag := -20020;
