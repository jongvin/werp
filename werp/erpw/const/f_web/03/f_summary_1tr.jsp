...
include file ="../../../comm_function/conn_tr.jsp"%><%
     GauceDataSet dSet = req.getGauceDataSet("f_summary_1tr"); gpstatement.gp_dataset = dSet;
	...
	if(rows.getJobType() == GauceDataRow.TB_JOB_INSERT){
      ...
      gpstatement.bindColumn(5, idx_l_close);
      gpstatement.bindColumn(6, idx_q_close);
      gpstatement.bindColumn(7, idx_s_close);
      gpstatement.bindColumn(8, idx_m_close);
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "delete from c_spec_const_closing where dept_code=? " +
                            " and yymm=? ";  
      stmt = conn.prepareStatement(sSql); 
      gpstatement.gp_stmt = stmt;
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
