...
include file ="../../../comm_function/conn_tr_pool.jsp"%><%
     GauceDataSet dSet = req.getGauceDataSet("s_req_sbcr_6tr");
	...
	if(rows.getJobType() == GauceDataRow.TB_JOB_INSERT){
		String sSql = " INSERT INTO S_REQ_SCHOLARSHIP ( sbcr_unq_num," + 
                    "class," + 
                    "spec_seq," + 
                    "scholarship_class," + 
                    "sch_date," +
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "update s_req_scholarship set " + 
                            "sbcr_unq_num=?,  " + 
                            "class=?,  " + 
                            "spec_seq=?,  " + 
                            "scholarship_class=?,  " + 
                            ...
                            "subject_name=?,  " + 
                            "graduation_class=?,  " + 
                            "remark=? ,sch_date=? where sbcr_unq_num=? " +
                            "            and class=? " +
                            "            and spec_seq=? ";  
      stmt = conn.prepareStatement(sSql); 
      gpstatement.gp_stmt = stmt;
      ...
      gpstatement.bindColumn(6, idx_subject_name);
      gpstatement.bindColumn(7, idx_graduation_class);
      gpstatement.bindColumn(8, idx_remark);
      gpstatement.bindColumn(9, idx_sch_date);
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
      gpstatement.bindColumn(10, idx_sbcr_unq_num);
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "delete from s_req_scholarship where sbcr_unq_num=? " +
                                         "           and class=? " +
                                         "           and spec_seq=? "; 
      stmt = conn.prepareStatement(sSql); 
      gpstatement.gp_stmt = stmt;
