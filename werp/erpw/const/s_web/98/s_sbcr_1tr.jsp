<%@ page import="com.gauce.*,com.gauce.io.*,com.gauce.common.*,com.gauce.log.*,com.gauce.db.*,java.sql.*"%><%@
 include file ="../../../comm_function/conn_tr_pool.jsp"%><%
     GauceDataSet dSet = req.getGauceDataSet("s_sbcr_1tr");
     gpstatement.gp_dataset = dSet;
   	...
   	int idx_chg_unq = dSet.indexOfColumn("chg_unq");
	...
	if(rows.getJobType() == GauceDataRow.TB_JOB_INSERT){
		String sSql = " INSERT INTO S_SBCR ( sbcr_code," + 
                    ...
                    "c_grade3," + 
                    "input_dt," + 
                    "input_name,mat_sbcr_class,chg_degree,bill_dis_pay,sbcr_class,use_tag )      ";
      ...
      gpstatement.bindColumn(74, idx_use_tag);
      stmt.executeUpdate();
      stmt.close();
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "update s_sbcr set " + 
                            ...
                            "c_grade3=?,  " + 
                            "input_dt=?,  " + 
                            "input_name=?,mat_sbcr_class=?,chg_degree=?,bill_dis_pay=?,sbcr_class=?,use_tag=?  where sbcr_code=? ";  
      stmt = conn.prepareStatement(sSql); 
      gpstatement.gp_stmt = stmt;
      gpstatement.bindColumn(1, idx_sbcr_code);
      gpstatement.bindColumn(2, idx_sbcr_name);
 ...
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
      gpstatement.bindColumn(75, idx_sbcr_code);
      stmt.executeUpdate();
      stmt.close();
	   double ls_class = rows.getDouble(idx_chg_cnt);
	   if (ls_class != 0 ) {	
 	     sSql = "update s_sbcr_chg set " + 
	                            "sbcr_name=?," + 
	                            "businessman_number=?," + 
	                            "corp_no=?," + 
	                            "zip_number1=?," + 
	      ...
	      gpstatement.bindColumn(10, idx_chg_unq);
	      stmt.executeUpdate();
	      stmt.close();
	   }
