<%@ page import="com.gauce.*,com.gauce.io.*,com.gauce.common.*,com.gauce.log.*,com.gauce.db.*,java.sql.*"%><%@
 include file ="../../../comm_function/conn_tr_pool.jsp"%><%
     GauceDataSet dSet = req.getGauceDataSet("s_sbcr_8tr");
	...
	if(rows.getJobType() == GauceDataRow.TB_JOB_INSERT){
		String sSql = " INSERT INTO S_CONST ( sbcr_code," + 
                    "othercompany_seq," + 
                    "owner," + 
                    "profession_wbs_name," + 
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "update s_const set " + 
                            "sbcr_code=?,  " + 
                            "othercompany_seq=?,  " + 
                            "owner=?,  " + 
                            "profession_wbs_name=?,  " + 
                            ...
                            "othercompany_amt=?,  " + 
                            "sale_amt=?,  " + 
                            "bigo=?,year_class=?  where sbcr_code=? " +
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "delete from s_const where sbcr_code=? " +
      											"       and othercompany_seq=? "; 
      stmt = conn.prepareStatement(sSql); 
      gpstatement.gp_stmt = stmt;
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
