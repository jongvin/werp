...
/* 작성일 : 2003.04.28                                                           */
/* ============================================================================= */
CREATE OR REPLACE PROCEDURE y_sp_s_prgs_cmpt(as_dept_code    IN   VARCHAR2,
                                             adt_yymm        IN   DATE,
                                             ai_seq          IN   INTEGER,
...
BEGIN
  BEGIN
      ...
      SELECT nvl(sub_qty,0),nvl(sub_amt,0)
        INTO C_SUB_QTY,C_SUB_AMT
        FROM S_CN_PARENT
       WHERE dept_code    = as_dept_code
         AND order_number = ai_order_number
         AND spec_no_seq  = ai_spec_no_seq;
      ...
      UPDATE S_PRGS_PARENT
       ...
       WHERE dept_code    = as_dept_code  
		...
		IF C_LEVEL <> 1 THEN
			LOOP
				SELECT NVL(SUM(b.TM_PRGS_AMT),0)
				  INTO C_TM_PRGS_AMT
				  FROM S_CN_PARENT a,
						 S_PRGS_PARENT b
				...
				SELECT nvl(sub_qty,0),nvl(sub_amt,0),spec_no_seq
				  INTO C_SUB_QTY,C_SUB_AMT,C_SPEC_NO_SEQ
				  FROM S_CN_PARENT
				 WHERE dept_code    = as_dept_code
					AND order_number = ai_order_number
...
--						 tot_prgs_qty = NVL(C_TM_PRGS_QTY,0) + pre_prgs_qty
	...
	--기성총누계
		SELECT nvl(sum(b.tot_prgs_amt),0)
		  INTO C_TMP_TOT_PRGS_AMT
		  FROM s_cn_parent a,
				 s_prgs_parent b
		 WHERE a.dept_code    = b.dept_code
			AND a.order_number = b.order_number
			...
			AND a.llevel       = 1;
	--기성매입세
		SELECT nvl(sum(b.tot_prgs_amt),0),nvl(sum(b.tm_prgs_amt),0)
		  INTO C_PRE_PURCHASE_VAT,C_TMP_PURCHASE_VAT
		  FROM s_cn_parent a,
				 s_prgs_parent b
		 WHERE a.dept_code    = b.dept_code
			AND a.order_number = b.order_number
