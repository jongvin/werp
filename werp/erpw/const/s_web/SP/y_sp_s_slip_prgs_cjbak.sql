...
select a.tm_prgs,a.tm_vat,a.tm_prgs_notax + a.tm_purchase_vat,a.tm_cash,a.tm_bill,a.tm_pre_tax,a.tm_pre_vat,a.tm_pre_notax,a.pay_dt,a.check_dt,
		 ...
		 c.businessman_number,c.sbcr_name,c.rep_name1,c.kind_bussinesstype,c.kinditem,c.zip_number1,c.address1,c.tel_number1,c.chrg_name1,c.chrg_email,d.invoice_num,d.invoice_num1
  from s_pay a,
       s_cn_list b,
       s_sbcr c,
		 (select max(order_number) order_number,max(invoice_num) invoice_num, max(invoice_num1) invoice_num1
          from s_guarantee_accept
			where dept_code = as_dept_code
			  and guarantee_class = '2'
			  and invoice_num is not null
			group by dept_code,order_number ) d
 where b.order_number = d.order_number (+)
...
-- User Define Error
   ...
   V_CHRG_EMAIL        s_sbcr.chrg_email%type;
	V_SLIP_ID           t_acc_slip_head.slip_id%type;
	V_SLIP_IDSEQ        t_acc_slip_head.slip_id%type;
	C_SLIP_DT           DATE;
	C_MAKE_DT           DATE;
...
-- 전표내역을 만들기 위한 커서
		...
		LOOP
			FETCH DETAIL_CUR INTO V_TM_PRGS,V_TM_VAT,V_TM_PRGS_NOTAX,V_CASH,V_BILL,V_TM_PRE_TAX,V_TM_PRE_VAT,
										 ...
										 V_ZIP_NUM,V_ADDR,V_TEL_NUM,V_CHRG_NAME,V_CHRG_EMAIL,V_SLIP_ID,V_SLIP_IDSEQ;
...
-- 선급금공제처리
			...
			IF V_TM_PRE_TAX <> 0 THEN -- 과세처리
				...
				-- Body에 내역을 넣어준다.
				IF V_VATCODE = '3' THEN
					IrBody.DB_AMT            := C_AMT + V_TM_PRE_VAT;        -- 차변금액
				ELSE
					IrBody.DB_AMT            := C_AMT;        -- 차변금액
				END IF;
				IrBody.TAX_COMP_CODE     := C_COMP_CODE;  -- 세무사업장코드
				...
				IF V_VATCODE = '3' THEN
					IrBody.DB_AMT            := 0;        -- 차변금액
				ELSE
					IrBody.DB_AMT            := V_TM_PRE_VAT;        -- 차변금액
				END IF;
				IrBody.TAX_COMP_CODE     := C_COMP_CODE;  -- 세무사업장코드
				IrBody.COMP_CODE         := C_COMP_CODE;  -- 귀속사업장코드
				IrBody.DEPT_CODE         := as_dept_code; -- 귀속부서
				IrBody.VAT_DT            := V_PAY_DT;     -- 증빙일자
				IrBody.SUPAMT            := C_AMT;        -- 공급가(부가세 라인에서만 입력)
				IrBody.VATAMT            := V_TM_PRE_VAT;        -- 부가세(부가세 라인에서만 입력)
				IrBody.CUST_SEQ          := InCustSeq;    -- 거래처순번
				IrBody.CUST_NAME         := IsCustName;   -- 거래처명칭
				IrBody.SUMMARY1          := V_CONT1;       -- 적요
				...
				-- Body에 내역을 넣어준다.
				IrBody.CR_AMT            := C_AMT + V_TM_PRE_VAT; -- 대변금액
				IrBody.TAX_COMP_CODE     := C_COMP_CODE;   -- 세무사업장코드
				IrBody.COMP_CODE         := C_COMP_CODE;   -- 귀속사업장코드
				IrBody.DEPT_CODE         := as_dept_code;  -- 귀속부서
				IrBody.VAT_DT            := V_PAY_DT;      -- 증빙일자
				...
				IrBody.CUST_NAME         := IsCustName;    -- 거래처명칭
				IrBody.SUMMARY1          := V_CONT1;        -- 적요
				IrBody.RESET_SLIP_ID     := V_SLIP_ID;
				IrBody.RESET_SLIP_IDSEQ  := V_SLIP_IDSEQ;
...
--              e_msg      := '외주기성전표생성 실패! [Line No: 2]' || IsCustName || '-'  ;
--              Wk_errflag := -20020;
