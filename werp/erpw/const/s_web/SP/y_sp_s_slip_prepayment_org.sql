...
-- Interface테이블에 거래처를 생성한다.
	      y_sp_comm_slip_supplier(C_COMP_CODE,C_SBCR_NAME,C_CUST_CODE,C_CUST_TYPE,as_dept_code,'211111',C_ZIP,C_ADDR,C_UPTAE,C_UPJONG,C_TREADE);
		END IF;
...
-- AP Interface 테이블에 자료를 생성한다.
	-- 선급금(면세)
	      y_sp_comm_slip_ap_invoice(C_COMP_CODE,C_GROUP_ID,C_SBCR_NAME || ' 선급금 전표',C_INVOICE,ad_date,ad_date,
				  							  C_SBCR_NAME,C_CUST_CODE,C_DR_AMT,C_CR_AMT,'선급비용',C_TEMP_VATNAME,as_dept_code,
	...
	-- 매입면세(계산서)		
	      y_sp_comm_slip_ap_invoice(C_COMP_CODE,C_GROUP_ID,C_SBCR_NAME || ' 선급금 전표',C_INVOICE,ad_date,ad_date,
											  C_SBCR_NAME,C_CUST_CODE,0,C_CR_AMT,'선급비용',C_TEMP_VATNAME,as_dept_code,
...
-- AP Interface 테이블에 자료를 생성한다.
	-- 선급금(과세)
	      y_sp_comm_slip_ap_invoice(C_COMP_CODE,C_GROUP_ID,C_SBCR_NAME || ' 선급금 전표',C_INVOICE,ad_date,ad_date,
											  C_SBCR_NAME,C_CUST_CODE,C_DR_AMT,C_CR_AMT,'선급비용',C_VATNAME,as_dept_code,
	...
	-- 매입과세OR매입공통OR매입불공제(선택된 부가세코드와 연계된계정과목을 가지고 간다)
			IF C_ACNTCODE = '112421' THEN
				y_sp_comm_slip_ap_invoice(C_COMP_CODE,C_GROUP_ID,C_SBCR_NAME || ' 선급금 전표',C_INVOICE,ad_date,ad_date,
												  C_SBCR_NAME,C_CUST_CODE,C_TEMP_VAT,C_CR_AMT,'선급비용',C_VATNAME,as_dept_code,
