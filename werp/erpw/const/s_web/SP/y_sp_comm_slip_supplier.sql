 /* ============================================================================================= */
/* 함수명 : y_sp_comm_slip_supplier                                                               */
/* 기  능 : 전표발생시 인터페이스 거래처코드에 생성시킨다.                                        */
/* ---------------------------------------------------------------------------------------------- */
/* 인  수 : 사업장코드                                     ==> as_comp_code (string)              */
/*        : 거래처명                                       ==> as_vendor_name(string)             */
/*        : 사업자번호                                     ==> as_vendor_site_code(string)        */
/*        : 거래처유형(개인,사업자)                        ==> as_vendor_type_lookup_code(string) */
/*        : 현장코드                                       ==> as_dept_code(string)               */
/*        : 계정(외주,자재->외상매입금계정,기타->'211519') ==> as_accts_code(string)              */
/*        : 우편번호('373883')                             ==> as_zip(string)                     */
/*        : 주소(번지)                                     ==> as_addr(string)                    */
/*        : 업태                                           ==> as_uptae(string)                   */
/*        : 업종                                           ==> as_upjong(string)                  */
/*        : 대표자명                                       ==> as_tread(string)                   */
/* ===========================[ 변   경   이   력 ]============================================== */
/* 작성자 : 김동우                                                                                */
/* 작성일 : 2005.04.25                                                                            */
/* ============================================================================================== */
...
-- 현장의 사업장코드로 ORG-ID 및 ORG-Name를 구한다.
		...
		C_PAY_CODE_ACCOUNTS    := as_comp_code || '.' || as_comp_code || '9999' || '.' || as_accts_code || '.' || as_comp_code || '99999.0000';
		C_PREPAY_CODE_ACCOUNTS := as_comp_code || '.' || as_comp_code || '9999' || '.112212.' || as_comp_code || '99999.0000';
		C_FUTURE_ACCOUNTS      := as_comp_code || '.' || as_comp_code || '9999' || '.211115.' || as_comp_code || '99999.0000';
      ...
      END IF;
		...
		VALUES( efin_sup_itf_s.NEXTVAL,TO_CHAR(SYSDATE,'yyyy-mm-dd'),C_ORG_ID,C_ORG_NAME,as_vendor_name,as_vendor_name,
				 ...
				 '현금즉시지급','KRW','KRW','invoice','Due',
 				 'Check',C_ZIP,NVL(C_SIDO,'.'), NVL(C_GUGUN,'.'),NVL(as_addr,'.'),NVL(as_uptae,'.'),
