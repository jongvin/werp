 /* ============================================================================ */
/* 함수명 : y_sp_s_slip_prepayment(CJ개발)                                       */
/* 기  능 : 외주관리-선급금지불전표 발행                                         */
/* ----------------------------------------------------------------------------- */
...
SELECT c.guarantee_amt,c.guarantee_vat,c.guarantee_issueday,b.sbcr_name || '의 ' || a.sbc_name || ' 계약 선급금 지불',c.pay_dt,
...
-- 전표내역을 만들기 위한 커서
		...
		LOOP
			...
			IrBody.RESET_SLIP_ID     := IrBody.SLIP_ID; -- 설정전표
			IrBody.RESET_SLIP_IDSEQ  := IrBody.SLIP_IDSEQ; -- 설정전표
			IrBody.ANTICIPATION_DT   := V_PAY_DT; -- 지급희망일
