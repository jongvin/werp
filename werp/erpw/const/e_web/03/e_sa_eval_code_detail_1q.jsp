...
include file="../../../comm_function/conn_q_pool.jsp"%><%
 ...
 // 다음 문장은 수정 하시요 (파라메터 갯수만큼 (없으면 //로 막으세요)--r
     String arg_opinion = req.getParameter("arg_opinion");
 ...
 //---------------------------------------------------------- 
     ...
     dSet.addDataColumn(new GauceDataColumn("shadow",GauceDataColumn.TB_DECIMAL,18,0));
    String query = "  SELECT a.opinion_type,   " + 
     ...
     "                       (select max(or_point) " +
	  ...
	  "								where opinion_type=a.opinion_type and " +
	  "										seq=a.seq and " +
	  "										item_code=a.item_code " +
	  "								group by opinion_type, seq, item_code) max_point, " +
	  "                        '' shadow " +
     "                   FROM  e_opinion_register_detail a " + 
     "                   WHERE a.opinion_type = '" + arg_opinion + "' " + 
     "                    and  a.seq = " + arg_seq + 
     "                    and  a.item_code = '" + arg_item + "' " +
     "                   ORDER BY a.d_seq ASC        ";
