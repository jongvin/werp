<%@ page session="true" import="com.gauce.*,com.gauce.io.*,com.gauce.common.*,com.gauce.log.*,com.gauce.db.*,java.sql.*"%><%@
 include file ="../../../comm_function/conn_tr_pool.jsp"%><%
     GauceDataSet dSet = req.getGauceDataSet("d_efin_invoice_header_save_itf_1tr");
	...
	if(rows.getJobType() == GauceDataRow.TB_JOB_INSERT){
		String sSql = " INSERT INTO efin_invoice_header_save_itf  ( invoice_group_id," + 
                    "approval_name," + 
                    "complete_flag," + 
                    "interface_flag," + 
                    "current_approval_id," + 
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "update efin_invoice_header_save_itf  set " + 
                            "invoice_group_id=?,  " + 
                            "approval_name=?,  " + 
                            "complete_flag=?,  " + 
                            "interface_flag=?,  " + 
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "delete from efin_invoice_header_save_itf  where invoice_group_id=? "; 
      stmt = conn.prepareStatement(sSql); 
      gpstatement.gp_stmt = stmt;
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
      gpstatement.bindColumn(1, idx_invoice_group_id);
