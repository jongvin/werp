 /* ============================================================================ */
/* 함수명 : y_sp_f_slip_prerequest(ap,GL)                                        */
/* 기  능 : 자금청구-예비비청구전표 발행                                         */
/* ----------------------------------------------------------------------------- */
...
/* 작성일 : 2005.04.28                                                           */
/* ============================================================================= */
CREATE OR REPLACE PROCEDURE y_sp_f_slip_prerequest (as_dept_code    IN   VARCHAR2,
																	 ad_fr_date		  IN   DATE ,
																	 ad_to_date		  IN   DATE ,
...
SELECT a.request_date,a.seq,a.cont,nvl(a.cash_amt,0),nvl(a.bill_amt,0),b.folder_id,a.acntcode
  FROM f_preamt_request a,
		 ( select max(folder_id) folder_id,max(dept_code) dept_code
			  from f_dept_account 
...
-- 전표일자를 구한다...
	...
	-- 전표번호를 구하기위한 TEMP
		...
		LOOP
			FETCH DETAIL_CUR INTO V_DATE,V_SEQ,V_CONT,V_CASH,V_BILL,V_FOLDER_ID,V_ACNTCODE;
	...
	-- Interface테이블에 거래처를 생성한다.
			...
			--			and vat_registration_num = V_CUST_CODE; 
					...
					-- GL Interface 테이블에 자료를 생성한다.
						y_sp_const_slip_gl_invoice(C_COMP_CODE,C_GROUP_ID,C_DEPT_NAME || ' 예비비청구',C_DATE,V_DATE,
														  C_DEPT_NAME,as_dept_code,C_AMT,'','',as_dept_code,
			...
			-- AP Interface 테이블에 자료를 생성한다.
						...
						--								  C_DEPT_NAME,as_dept_code,C_DR_AMT,C_CR_AMT,V_CONT,'',as_dept_code,
