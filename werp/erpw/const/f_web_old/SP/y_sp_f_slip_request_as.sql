 /* ============================================================================ */
/* 함수명 : y_sp_f_slip_request_as(GL)                                           */
/* 기  능 : 자금청구 - 예비비정산 전표 발행(품질안전팀)                          */
/* ----------------------------------------------------------------------------- */
...
/* 작성일 : 2005.04.27                                                           */
/* ============================================================================= */
CREATE OR REPLACE PROCEDURE y_sp_f_slip_request_as (as_dept_code    IN   VARCHAR2,
																 ad_fr_date		  IN   DATE ,
																 ad_to_date		  IN   DATE ,
																 as_wbs_code     IN   VARCHAR2,
																 ai_unq_num      IN   NUMBER,
																 as_yn           IN   VARCHAR2) IS
-------------------------------------------------------------
-- 변수선언
-------------------------------------------------------------
...
SELECT a.receipt_date,a.slip_spec_unq_num,a.res_type,a.cr_class,
		 a.cont,a.acntcode,a.vatcode,nvl(a.amt,0),nvl(a.vat,0),nvl(a.profit_amt,0),
...
-- 현장별프로젝트코드를 구한다.
	...
	-- 전표 마스터번호를 구한다.
		...
		LOOP
			...
			-- 공급처 테이블에 거래처가 존재하는지 Check하여 미존재시 insert한다.
					IF F_COMM_SLIP_CHECK_SUPPLIER(C_ORG_ID,V_CUST_CODE) = False THEN
						y_sp_comm_slip_supplier(C_COMP_CODE,V_SBCR_NAME,V_CUST_CODE,V_CUST_TYPE,as_wbs_code,'211111',V_ZIP,V_ADDR,V_UPTAE,V_UPJONG,V_TREADE);
					END IF;
		...
		-- 부가세명,부가세계정과목코드를 가지고 온다.
			...
			-- GL Interface 테이블에 자료를 생성한다.
				IF V_CR_CLASS = '1' THEN  -- 차변일 경우 처리
					C_TOT_DR_AMT := C_TOT_DR_AMT + V_AMT + V_VAT ;
					C_TOT_CR_AMT := C_TOT_CR_AMT + V_PROFIT_AMT;
					y_sp_const_slip_gl_invoice(C_COMP_CODE,C_GROUP_ID,C_DEPT_NAME || ' 예비비정산',V_RQST_DATE,V_DATE,
					...
					IF V_VATCODE IS NOT NULL THEN
						IF C_ACNTCODE = '112421' THEN -- 불공제일경우
							...
							--								  C_PROJ,'',V_ACNTCODE,C_CAT_NAME,'자금청구',C_TAX_SEQ,C_TAX_TAG,C_FOLDER_ID,
							--								  '',C_ORG_ID,C_DEPT_NAME,C_VATNAME,'V',
							--								  V_FIX_QTY,V_ST_DT,V_END_DT,V_REACNT);
							--적요수정 경일
							y_sp_const_slip_gl_invoice(C_COMP_CODE,C_GROUP_ID,C_DEPT_NAME || ' 예비비정산',V_RQST_DATE,V_DATE,
															  V_SBCR_NAME,V_CUST_CODE,V_VAT,'','',as_wbs_code,
					...
					IF V_PROFIT_AMT <> 0 THEN
						y_sp_const_slip_gl_invoice(C_COMP_CODE,C_GROUP_ID,C_DEPT_NAME || ' 예비비정산',V_RQST_DATE,V_DATE,
														  V_SBCR_NAME,V_CUST_CODE,'',V_PROFIT_AMT,'',as_wbs_code,
														  C_PROJ,'','462519',C_CAT_NAME,'자금청구','','',C_FOLDER_ID,
														  '',C_ORG_ID,C_DEPT_NAME,V_CONT,'V',
														  V_FIX_QTY,V_ST_DT,V_END_DT,V_REACNT);
					END IF;
				ELSE
					C_TOT_DR_AMT := C_TOT_DR_AMT + V_PROFIT_AMT;
					C_TOT_CR_AMT := C_TOT_CR_AMT + V_AMT + V_VAT;
					y_sp_const_slip_gl_invoice(C_COMP_CODE,C_GROUP_ID,C_DEPT_NAME || ' 예비비정산',V_RQST_DATE,V_DATE,
													  V_SBCR_NAME,V_CUST_CODE,'',V_AMT,C_VATNAME,as_wbs_code,
													  C_PROJ,C_TAX_CNT,V_ACNTCODE,C_CAT_NAME,'자금청구',C_TAX_SEQ,C_TAX_TAG,C_FOLDER_ID,
													  '',C_ORG_ID,C_DEPT_NAME,V_CONT,'V',
		...
		CLOSE DETAIL_CUR1;
		y_sp_const_slip_gl_invoice(C_COMP_CODE,C_GROUP_ID,C_DEPT_NAME || ' 예비비정산',V_RQST_DATE,V_DATE,
										  '','','',C_TOT_DR_AMT - C_TOT_CR_AMT,'',as_wbs_code,
