 /* ============================================================================ */
/* 함수명 : y_sp_f_slip_nopay_as(ap)                                             */
/* 기  능 : 자금청구-미지급청구전표 발행(품질안전팀)                             */
/* ----------------------------------------------------------------------------- */
...
/* 작성일 : 2005.04.28                                                           */
/* ============================================================================= */
CREATE OR REPLACE PROCEDURE y_sp_f_slip_nopay_as (as_dept_code    IN   VARCHAR2,
																	 ad_fr_date		  IN   DATE ,
																	 ad_to_date		  IN   DATE ,
	...
	-- AP Interface 테이블에 자료를 생성한다.
		-- 공급가
				y_sp_comm_slip_ap_invoice(C_COMP_CODE,C_GROUP_ID,to_char(C_DATE,'yyyy-mm') || '월 미지급청구',C_INVOICE,V_DATE,V_DATE,
												  V_SBCR_NAME,V_CUST_CODE,C_DR_AMT,C_CR_AMT,V_CONT,C_VATNAME,as_wbs_code,
		...
		-- 매입과세OR매입공통OR매입불공제(선택된 부가세코드와 연계된계정과목을 가지고 간다)
				IF C_ACNTCODE = '112421' THEN
					--y_sp_comm_slip_ap_invoice(C_COMP_CODE,C_GROUP_ID,to_char(C_DATE,'yyyy-mm') || '월 미지급청구',C_INVOICE,V_DATE,V_DATE,
					--								  V_SBCR_NAME,V_CUST_CODE,V_VAT,C_CR_AMT,V_CONT,C_VATNAME,as_wbs_code,
