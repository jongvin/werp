CREATE OR REPLACE PROCEDURE y_sp_f_slip_profit (as_dept_code    IN   VARCHAR2,
																	ad_fr_date		  IN   DATE ,
																	ad_to_date		  IN   DATE,
	...
	LOOP
		...
		-- 부가세명,부가세계정과목코드를 가지고 온다.
			...
			-- Interface테이블에 거래처를 생성한다.
						y_sp_comm_slip_customer(C_COMP_CODE,V_SBCR_NAME,V_CUST_CODE,V_CUST_TYPE,as_dept_code,V_ZIP,V_ADDR,'',V_UPTAE,V_UPJONG);
					END IF;
				END IF;
