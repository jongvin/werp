<%@ page session="true" import="com.gauce.*,com.gauce.io.*,com.gauce.common.*,com.gauce.log.*,com.gauce.db.*,java.sql.*"%><%@
 include file ="../../../comm_function/conn_tr_pool.jsp"%><%
     GauceDataSet dSet = req.getGauceDataSet("f_bank_code_1tr");
	...
	if(rows.getJobType() == GauceDataRow.TB_JOB_INSERT){
		String sSql = " INSERT INTO F_DEPT_ACCOUNT ( dept_code," + 
			        "wbs_code,"+
                    "bank_account_id," + 
                    "bank_account_number," + 
                    "bank_branch_name," + 
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "update F_DEPT_ACCOUNT set " + 
                            "dept_code=?,  " + 
                            "wbs_code=?,  " + 
                            "bank_account_id=?,  " + 
                            "bank_account_number=?,  " + 
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "delete from f_dept_account where dept_code=? " +
                            "               and bank_account_id=? " +
                            "               and bank_account_number=? ";
      stmt = conn.prepareStatement(sSql);
      gpstatement.gp_stmt = stmt;
