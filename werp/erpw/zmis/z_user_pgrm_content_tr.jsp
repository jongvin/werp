...
include file ="../comm_function/conn_tr_pool.jsp"%><%
     GauceDataSet dSet = req.getGauceDataSet("z_user_pgrm_content_tr"); gpstatement.gp_dataset = dSet;
	...
	if(rows.getJobType() == GauceDataRow.TB_JOB_INSERT){
		String sSql = " INSERT INTO z_user_pgrm_content ( user_key, user_seq_key," + 
                    "pgrm_seq_key," + 
                    "no_seq," + 
                    "rw_tag )      ";
 ...
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "update z_user_pgrm_content set " + 
                            "user_key=?,  " + 
                            "user_seq_key=?,  " + 
                            "pgrm_seq_key=?,  " + 
                            "no_seq=?,  " + 
                            "rw_tag=?  where ( user_key=? and user_seq_key=? and pgrm_seq_key=?) ";  
      stmt = conn.prepareStatement(sSql); 
      gpstatement.gp_stmt = stmt;
      gpstatement.bindColumn(1, idx_user_key);
      gpstatement.bindColumn(2, idx_user_seq_key);
 ...
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
      gpstatement.bindColumn(6, idx_user_key);
      gpstatement.bindColumn(7, idx_user_seq_key);
      gpstatement.bindColumn(8, idx_pgrm_seq_key);
      stmt.executeUpdate();
      stmt.close();
   }else if(rows.getJobType() ==  GauceDataRow.TB_JOB_DELETE){
 /* 다음 문장은 반드시 수정하시요 (where 아래의 변수 갯수만큼 */ 
      String sSql = "delete from z_user_pgrm_content where ( user_key=? and user_seq_key=? and pgrm_seq_key=?) "; 
      stmt = conn.prepareStatement(sSql); 
      gpstatement.gp_stmt = stmt;
 /* 다음 문장은 반드시 수정하시요(변수갯수 만큼) */ 
      gpstatement.bindColumn(1, idx_user_key);
